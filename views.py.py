...
def _dg_build_model(
    ...
    if not billing.empty:
        ...
        pending_billing_period = pending_billing.loc[
            _dg_period_mask(pending_billing["Fecha factura"], start, end)
        ].copy()
    else:
    ...
    for column, company in zip(columns, companies):
        with column:
            ...
            if not company_billing.empty:
                ...
                pending = float(company_billing.loc[invoice_mask, "Pendiente"].sum())
...
def _dg_render_alerts(model: dict[str, Any]) -> None:
    ...
    billing = model["billing"]
    if not billing.empty:
        period_billing_mask = _dg_period_mask(billing["Fecha factura"], model["period_start"], model["period_end"])
