...
def _pp_prepare(df: pd.DataFrame | None, source: str = "Google Sheets") -> pd.DataFrame:
    """Normaliza una base ya tabular al esquema profesional."""
    if df is None or df.empty:
    ...
    for position, index in enumerate(result.index):
        ...
        if state_norm in {"pagada", "pagado", "cancelada", "cancelado", "saldada", "saldado"} or (amount > 0 and balance <= 0.01):
            ...
            # Una planilla histórica no demuestra que la cuota esté impaga.
            result.at[index, "estado"] = "A conciliar"
        else:
    ...
    result = result[meaningful].copy()
    result = result[result["importe_cuota"].abs().gt(0.0001)].copy()
    result["periodo"] = pd.to_datetime(result["periodo"], errors="coerce").dt.to_period("M").dt.to_timestamp()
    result["vencimiento"] = pd.to_datetime(result["vencimiento"], errors="coerce")

    # Completa numeración de cuotas por plan cuando la planilla no la informa.
    if not result.empty:
        grouping = ["hoja_origen", "empresa", "acreedor", "identificador"]
        result = result.sort_values(grouping + ["periodo", "fila_origen", "columna_origen"], na_position="last")
        calculated_number = result.groupby(grouping, dropna=False).cumcount() + 1
        calculated_total = result.groupby(grouping, dropna=False)["id_registro"].transform("size")
        result["cuota_numero"] = result["cuota_numero"].where(result["cuota_numero"] > 0, calculated_number)
        result["cuotas_totales"] = result["cuotas_totales"].where(result["cuotas_totales"] > 0, calculated_total)
...
def _pp_detect_identifier(values: list[str]) -> tuple[str, str, str]:
    ...
    for text in values:
        ...
        if not normalized:
            continue
        if any(token in normalized for token in ["pp arca", "arca", "credito macro", "credito galicia", "prestamo", "banco"]):
        ...
        identifier_like = (
            ...
            and not re.search(r"\b20\d{2}\b", compact)
        )
        if identifier_like:
...
def _pp_parse_legacy_matrix(matrix: pd.DataFrame, sheet_name: str = "Principal") -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Convierte el cuadro horizontal en una fila por cuota mensual.
    Devuelve la base normalizada y una tabla de controles contra los totales
    declarados que aparecen a la derecha de la planilla.
    """
    if matrix is None or matrix.empty:
    ...
    section_by_row: dict[int, str] = {}
    current_section = "Deudas asumidas mes a mes"
    for row_index in range(len(raw)):
        row_text = " ".join(_pp_text(value) for value in raw.iloc[row_index].tolist() if _pp_text(value))
        normalized = _pp_norm(row_text)
        if "deudas asumidas mes a mes" in normalized:
            current_section = "Deudas asumidas mes a mes"
        elif "adelanto de cuotas" in normalized:
            current_section = "Adelanto de cuotas"
        elif normalized == "prestamos" or normalized.startswith("prestamos "):
            current_section = "Préstamos"
        section_by_row[row_index] = current_section
    ...
    if not header_rows:
        return pd.DataFrame(columns=_PLANES_COLUMNS), pd.DataFrame([
            {"Hoja": sheet_name, "Control": "Estructura", "Resultado": "No se detectaron encabezados mensuales"}
        ])
    ...
    for block_position, header in enumerate(header_rows):
        ...
        end_row = header_rows[block_position + 1]["row"] if block_position + 1 < len(header_rows) else len(raw)
        first_month_col = int(header["first_month_col"])
        month_columns: dict[int, pd.Timestamp] = header["months"]
        total_column = header["total_col"]
        ...
        for row_index in range(start_row, end_row):
            ...
            descriptor_norm = _pp_norm(descriptor_text)
            if not descriptor_values and all(abs(_pp_number(raw.iat[row_index, column])) <= 0.0001 for column in month_columns):
                continue
            if descriptor_norm.startswith("total") or "totales" in descriptor_norm:
                ...
                validations.append({
                    "Hoja": sheet_name,
                    "Control": f"Total de bloque · fila {row_index + 1}",
                    "Resultado": "Detectado",
            ...
            unit = detected_unit or carried_unit

            # Algunas hojas tienen la entidad y el identificador en columnas fijas,
            # pero la unidad se encuentra en una celda combinada anterior.
            if not creditor:
                for text in descriptor_values:
                    normalized = _pp_norm(text)
                    if normalized in {"pp arca", "arca"}:
                        creditor = text
                        break
            if not creditor and identifier:
                creditor = "Plan / préstamo"
            if not identifier:
                # No crea registros para rótulos o subtítulos sin código.
                values_with_amount = [
                    _pp_number(raw.iat[row_index, column])
                    for column in month_columns
                    if abs(_pp_number(raw.iat[row_index, column])) > 0.0001
                ]
                if not values_with_amount:
                    continue
                identifier = f"FILA-{row_index + 1}"

            section = section_by_row.get(row_index, "Deudas asumidas mes a mes")
            company = _pp_company("", unit, sheet_name)
            financing_type = _pp_type("", creditor, section)
            row_total = _pp_number(raw.iat[row_index, total_column]) if total_column is not None and total_column < len(raw.columns) else 0.0
            monthly_sum = 0.0
            row_record_indexes: list[int] = []

            for column_index, period in month_columns.items():
                ...
                record = {
                    ...
                    "seccion": section,
                    "empresa": company,
                    "unidad": unit or ("Reproductiva" if company == "VMR" else "Medical" if company == "VM" else "Corporativo"),
                    "tipo_financiacion": financing_type,
                    "acreedor": creditor or "Sin acreedor",
                    ...
                    "observaciones": "Vencimiento estimado al cierre del mes según cronograma importado.",
                    "fila_origen": row_index + 1,
                ...
                row_record_indexes.append(len(records))
                records.append(record)
            ...
            if row_record_indexes:
                ...
                difference = row_total - monthly_sum if row_total else 0.0
                validations.append({
                    "Hoja": sheet_name,
                    "Control": f"{company} · {creditor or 'Sin acreedor'} · {identifier}",
                    "Resultado": "Coincide" if not row_total or abs(difference) <= 1.0 else "Revisar",
                    "Importe": monthly_sum,
...
def _pp_prepare_sheet(df: pd.DataFrame) -> pd.DataFrame:
    ...
    numeric_columns = [
        ...
        "tasa_mensual", "capital_original", "total_plan_declarado", "fila_origen",
    ]
...
def _pp_enrich(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if data.empty:
        for column in ["_periodo", "_vencimiento", "_importe", "_pagado", "_saldo", "_situacion"]:
...
def _pp_plan_summary(data: pd.DataFrame) -> pd.DataFrame:
    if data is None or data.empty:
        return pd.DataFrame()
    grouping = [
        "hoja_origen", "empresa", "unidad", "tipo_financiacion",
        "acreedor", "identificador", "moneda",
    ]
    ...
    current_month = pd.Timestamp.today().replace(day=1).normalize()
    remaining = (
        data[(data["_saldo"] > 0.01) & (data["_periodo"] >= current_month)]
        .groupby(grouping, dropna=False)["id_registro"]
        .size()
    ...
    summary["Diferencia_control"] = summary["Total_declarado"].where(summary["Total_declarado"] > 0, summary["Total_cronograma"]) - summary["Total_cronograma"]
    summary["Avance_%"] = (summary["Pagado"] / summary["Total_cronograma"].replace(0, pd.NA) * 100).fillna(0).clip(0, 100)
    return summary.sort_values(["Saldo", "Hasta"], ascending=[False, True]).reset_index(drop=True)
...
def _pp_merge(existing: pd.DataFrame, incoming: pd.DataFrame, replace_sheets: list[str] | None = None) -> pd.DataFrame:
    ...
    if combined.empty:
        return pd.DataFrame(columns=_PLANES_COLUMNS)
    # Mismo plan, mes y celda de origen: la última importación reemplaza la anterior.
    keys = ["hoja_origen", "empresa", "acreedor", "identificador", "periodo", "fila_origen", "columna_origen"]
    combined["_period_key"] = pd.to_datetime(combined["periodo"], errors="coerce").dt.strftime("%Y-%m")
    dedupe_keys = ["hoja_origen", "empresa", "acreedor", "identificador", "_period_key", "fila_origen", "columna_origen"]
...
def render_planes_pagos_prestamos_pro(
    ...
    # La hoja puede estar ya normalizada o conservar el cuadro horizontal.
    fallback_matrix = pd.DataFrame()
    validation_from_sheet = pd.DataFrame()
    if _pp_is_canonical(df_original):
        base = _pp_prepare(df_original, source="Google Sheets")
    else:
    ...
    with tabs[0]:
        if data.empty:
            ...
            # Totales generales: siempre visibles, aunque luego se apliquen filtros.
            ars = data[data["moneda"].eq("ARS")]
            usd = data[data["moneda"].eq("USD")]
            ...
            active_plans = int(plan_summary[plan_summary["Saldo"] > 0.01].shape[0]) if not plan_summary.empty else 0
            final_month = data.loc[(data["_saldo"] > 0.01) & data["_periodo"].notna(), "_periodo"].max()

            st.markdown("#### Totales generales de toda la cartera")
            c1, c2, c3, c4, c5 = st.columns(5)
            with c1:
                _pp_metric_card("Compromiso histórico ARS", _pp_money(general_total_ars), f"{len(ars)} cuotas registradas")
            with c2:
                _pp_metric_card("Pendiente desde hoy", _pp_money(future_balance_ars), "No incluye cuotas históricas sin conciliar")
            with c3:
    ...
    with tabs[1]:
        if data.empty:
            ...
            view = schedule[[
                "hoja_origen", "empresa", "unidad", "tipo_financiacion", "acreedor",
                "identificador", "_periodo", "cuota_numero", "cuotas_totales",
                "_importe", "_pagado", "_saldo", "moneda", "estado", "_situacion",
            ]].rename(columns={
                "hoja_origen": "Hoja", "empresa": "Empresa", "unidad": "Unidad",
                "tipo_financiacion": "Tipo", "acreedor": "Acreedor", "identificador": "Identificador",
                "_periodo": "Período", "cuota_numero": "Cuota", "cuotas_totales": "Total cuotas",
                "_importe": "Importe", "_pagado": "Pagado", "_saldo": "Saldo",
                "moneda": "Moneda", "estado": "Estado", "_situacion": "Situación",
            })
            st.caption(f"{len(view)} cuotas visibles · Total saldo ARS: {_pp_money(schedule[schedule['moneda'].eq('ARS')]['_saldo'].sum())}")
            st.dataframe(
                view,
                use_container_width=True,
                hide_index=True,
    ...
    with tabs[2]:
        ...
        open_rows = data[data["_saldo"] > 0.01].copy() if not data.empty else pd.DataFrame()
        if open_rows.empty:
                ...
                if selected_import_sheets:
                    ...
                    for sheet_name in selected_import_sheets:
                        ...
                        status_rows.append({
                            ...
                            "Cuotas detectadas": len(parsed),
                            "Planes detectados": parsed["identificador"].nunique() if not parsed.empty else 0,
                            "Total ARS detectado": parsed.loc[parsed["moneda"].eq("ARS"), "importe_cuota"].sum() if not parsed.empty else 0.0,
                        })
                    ...
                    st.dataframe(
                        ...
                        use_container_width=True,
                        hide_index=True,
                        column_config={"Total ARS detectado": st.column_config.NumberColumn(format="$ %.2f")},
                    )
                    if imported.empty:
                        st.error("No se detectaron cuotas mensuales. Revisá que la hoja tenga encabezados como jun-26, jul-26, etc.")
                    else:
                        i1, i2, i3, i4 = st.columns(4)
                        i1.metric("Cuotas", len(imported))
                        i2.metric("Planes / préstamos", imported["identificador"].nunique())
                        i3.metric("VM", _pp_money(imported.loc[imported["empresa"].eq("VM") & imported["moneda"].eq("ARS"), "importe_cuota"].sum()))
                        i4.metric("VMR", _pp_money(imported.loc[imported["empresa"].eq("VMR") & imported["moneda"].eq("ARS"), "importe_cuota"].sum()))
                        ...
                        confirm_import = st.checkbox(
                            f"Confirmo guardar {len(imported)} cuotas en Google Sheets",
