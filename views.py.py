...
def _dg_build_model(
    ...
    total_payables = obligation_balance + account_payables
...
def _dg_render_ai(dfs: dict[str, pd.DataFrame]) -> None:
    st.markdown("### Director IA")
    st.caption("Consulta consolidada sobre los módulos incluidos en Dashboard Global. No modifica datos.")

    if st.button("Generar informe ejecutivo automático", type="primary", key="dg_generate_briefing"):
        with st.spinner("Analizando la información consolidada..."):
            try:
                st.session_state["dg_briefing"] = generar_briefing_automatico(dfs=dfs)
            except Exception as exc:
                st.session_state["dg_briefing_error"] = str(exc)
    ...
    if st.session_state.get("dg_briefing_error"):
        st.error(f"No se pudo generar el informe: {st.session_state['dg_briefing_error']}")

    st.divider()
    question = st.text_area(
        "Pregunta ejecutiva",
        placeholder="Ej.: ¿Dónde está hoy la principal brecha financiera y qué debería priorizar esta semana?",
        key="dashboard_ia",
        height=110,
    )
    if st.button("Consultar", key="consultar_dashboard", use_container_width=True):
        if not question.strip():
            ...
            with st.spinner("Analizando Dashboard Global..."):
                try:
                    st.session_state["dg_ai_answer"] = preguntar_dashboard(dfs, question)
                    st.session_state.pop("dg_ai_error", None)
                except Exception as exc:
                    st.session_state["dg_ai_error"] = str(exc)
    if st.session_state.get("dg_ai_answer"):
        st.success(st.session_state["dg_ai_answer"])
    if st.session_state.get("dg_ai_error"):
        st.error(f"No se pudo consultar al Director IA: {st.session_state['dg_ai_error']}")
