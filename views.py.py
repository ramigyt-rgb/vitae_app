...
# Paleta visual corporativa VITAE · rosa de marca, sin azules ni verdes.
# Solo afecta presentación de gráficos; no modifica datos ni cálculos.
VITAE_PLOTLY_COLORS = [
...
def render_vitae_login() -> None:
    ...
    with center:
        ...
        st.markdown('<div class="vitae-security-note">Cada persona usa su propia cuenta. El Administrador puede activar, desactivar y limitar accesos por empresa y por módulo.</div>', unsafe_allow_html=True)
...
def _auth_render_session_bar() -> None:
    ...
    with c2:
        if st.button("Salir", key=f"vitae_logout_{st.session_state.get('vitae_current_module','home')}", use_container_width=True):
            _auth_logout()
            st.rerun()
