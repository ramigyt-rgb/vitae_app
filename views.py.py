...
from assistant import preguntar_copiloto, obtener_historial_copiloto, limpiar_historial_copiloto
...
def render_vitae_copilot(modulo_actual: str | None = None) -> None:
    """Copiloto persistente: acompaña al usuario en cualquier módulo de la app."""
    actual = str(
        modulo_actual
    ...
    with st.sidebar:
        st.markdown(
            """
            <div style="margin-top:.6rem;padding:.85rem .9rem;border-radius:16px;"
                 "background:linear-gradient(135deg,rgba(16,36,62,.96),rgba(28,78,128,.94));"
                 "box-shadow:0 12px 30px rgba(16,36,62,.16);color:white;">
                <div style="font-size:.72rem;letter-spacing:.12em;text-transform:uppercase;opacity:.72;">
                    Vitae Intelligence
                </div>
                <div style="font-size:1.08rem;font-weight:800;margin-top:.15rem;">✦ Copiloto Vitae</div>
                <div style="font-size:.78rem;opacity:.82;margin-top:.25rem;line-height:1.35;">
                    Te acompaña por toda la plataforma, entiende dónde estás y puede cruzar información de la empresa.
                </div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        st.caption(f"📍 Estás en: **{actual}**")

        q1, q2 = st.columns(2)
        quick_question = None
        if q1.button("¿Qué hago acá?", key=f"copilot_help_{actual}", use_container_width=True):
            quick_question = "¿Qué puedo hacer en esta pantalla y qué debería mirar primero?"
        if q2.button("Analizá esto", key=f"copilot_analyze_{actual}", use_container_width=True):
        ...
        if history:
            with st.expander("Conversación", expanded=False):
                for item in history[-4:]:
                    st.markdown(f"**Vos:** {item.get('pregunta','')}")
                    st.markdown(item.get("respuesta", ""))
                    st.divider()

        question = st.text_area(
            "Preguntame lo que quieras",
            key="vitae_copilot_question",
            placeholder="Ej.: ¿Dónde veo lo que vence esta semana? · ¿Cómo está VMR? · ¿Qué debería revisar ahora?",
            height=82,
            label_visibility="collapsed",
        )

        c_send, c_clear = st.columns([3, 1])
        send = c_send.button("Preguntar al copiloto", key="vitae_copilot_send", use_container_width=True)
        clear = c_clear.button("↺", key="vitae_copilot_clear", help="Limpiar conversación", use_container_width=True)
        ...
        final_question = quick_question or (question.strip() if send and question.strip() else "")
        if final_question:
            try:
                with st.spinner("El copiloto está revisando Vitae..."):
                    answer = preguntar_copiloto(
                        final_question,
                        modulo_actual=actual,
                        ruta_actual=actual,
                    )
                st.session_state["vitae_copilot_last_answer"] = answer
            except Exception as error:
                st.session_state["vitae_copilot_last_answer"] = (
                    "No pude completar la consulta del copiloto. "
                    f"Detalle técnico: {error}"
                )

        last_answer = st.session_state.get("vitae_copilot_last_answer", "")
        if last_answer:
