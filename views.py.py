    ...
    with tab_panel:
        ...
        if table in ["facturacion_vm", "facturacion_vmr"]:
            ...
            if table in ["caja_vm", "caja_vmr"]:
                ...
                # Cuenta Corriente VMR usa un centro financiero exclusivo de doble moneda.
                # Evitamos métricas y tablas genéricas duplicadas para que ARS y USD no se mezclen.
...
def render_cuenta_corriente_vmr_pro(df):
    ...
    st.markdown(
        ...
        .ccvmr-hero {
            ...
            border-radius: 22px;
            background: linear-gradient(120deg, rgba(15,23,42,.98), rgba(30,41,59,.96));
            border: 1px solid rgba(148,163,184,.22);
            box-shadow: 0 18px 42px rgba(2,6,23,.18);
            margin-bottom: 1rem;
        }
        .ccvmr-kicker {font-size:.76rem; letter-spacing:.18em; text-transform:uppercase; opacity:.72;}
        .ccvmr-title {font-size:2rem; font-weight:800; margin:.25rem 0 .3rem 0;}
        .ccvmr-sub {font-size:.94rem; opacity:.78; margin:0;}
        .ccvmr-note {
            padding:.8rem 1rem; border-radius:14px; background:rgba(148,163,184,.08);
            border:1px solid rgba(148,163,184,.16); margin:.4rem 0 .8rem 0;
        }
        </style>
        <div class="ccvmr-hero">
            <div class="ccvmr-kicker">CENTRO FINANCIERO · DOBLE MONEDA</div>
            <div class="ccvmr-title">Cuenta Corriente VMR</div>
            <p class="ccvmr-sub">Control ejecutivo de compromisos, pagos, vencimientos y exposición en pesos y dólares.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )

    control1, control2, control3, control4 = st.columns([1.1, 1.35, 1.2, 1.2])
    with control1:
        situations = [
    ...
    with control4:
        search = st.text_input(
            "Buscar", placeholder="Entidad, factura, concepto…", key=f"{key}_buscar"
        ).strip().lower()

    horizon1, horizon2 = st.columns([1.2, 2.8])
    with horizon1:
    ...
    with horizon2:
        exchange_rate = st.number_input(
            "Tipo de cambio para consolidar USD a ARS (opcional)",
            min_value=0.0,
            value=max(suggested_rate, 0.0),
            step=10.0,
            format="%.2f",
            key=f"{key}_tc",
            help="No modifica tu Sheet. Se usa únicamente para la lectura consolidada del panel.",
        )
