...
def _di_vm_header_kind(value: Any) -> str | None:
    ...
    if not norm:
        return None
    if "fecha" in norm and ("vto" in norm or "venc" in norm):
    ...
    if "contribucion" in norm and "convenc" in norm:
        return "contribucion_convencional"
    if norm in {"total", "deuda total", "total deuda"} or ("deuda" in norm and "total" in norm):
        return "deuda_total"
    return None
    ...
    for header_position, (header_row, period_columns) in enumerate(header_rows):
        ...
        for period_index, period_col in enumerate(period_columns):
            ...
            for row_index in range(header_row + 1, min(next_header_row, nrows)):
                ...
                if not group_ffill or _di_vm_norm(tax) == _di_vm_norm(organism):
...
def _di_vm_from_sheet(df: pd.DataFrame, table: str) -> pd.DataFrame:
    prepared = _di_vm_prepare(df, source=table)
    if not prepared.empty:
        return prepared
    if df is None or not isinstance(df, pd.DataFrame) or df.empty:
        return prepared

    # Si la hoja fue pegada conservando filas en blanco y encabezados internos,
    # get_df puede haber tomado la primera fila como encabezado. La reinsertamos
    # como matriz para detectar el cuadro exactamente como está organizado.
    ...
    return _di_vm_parse_matrix(raw, sheet_name=table, filename="Google Sheet")
...
def _di_vm_render_dashboard(data: pd.DataFrame, table: str) -> None:
    ...
    with state_left:
        ...
        by_state = by_state[by_state["saldo"].abs().gt(0.0001)]
        if by_state.empty:
...
def _di_vm_read_uploaded(uploaded_file: Any) -> tuple[dict[str, pd.DataFrame], str]:
    ...
    if uploaded_file is None:
        return {}, ""
    filename = _di_vm_text(getattr(uploaded_file, "name", "archivo"))
    ...
    if lower.endswith(".csv"):
        ...
        frame = pd.read_csv(StringIO(text), header=None, sep=None, engine="python", dtype=object)
...
def _di_vm_render_import(data: pd.DataFrame, table: str) -> None:
    ...
    mode = st.radio(
        ...
        [
            ...
            "Reemplazar toda la base de Deudas Impositivas VM",
        ],
        horizontal=True,
...
def _di_vm_render_export(data: pd.DataFrame, table: str) -> None:
    from io import BytesIO
...
def render_deudas_impositivas_vm_pro(
    ...
    """Centro integral de deuda tributaria y sindical de Vitae Medical."""
    st.markdown(
        """
        <style>
        .di-vm-hero {
            ...
            border-radius: 18px;
            background: linear-gradient(135deg, rgba(15, 45, 75, .98), rgba(20, 89, 111, .92));
            color: white;
            margin: .25rem 0 1rem 0;
            box-shadow: 0 14px 38px rgba(9, 30, 55, .16);
        }
        .di-vm-hero h2 { margin: 0 0 .25rem 0; font-size: 1.45rem; }
        .di-vm-hero p { margin: 0; opacity: .88; }
        .di-vm-pill {
            display: inline-block;
            padding: .25rem .62rem;
            margin-top: .7rem;
        ...
        <div class="di-vm-hero">
            <h2>Centro de Deudas Impositivas VM</h2>
            <p>Control integral de AFIP/ARCA, IVA, F.931, tasas municipales, FATSA, ATSA y demás obligaciones.</p>
            <span class="di-vm-pill">Google Sheets como fuente única · Importación Excel por hoja · Seguimiento de pagos</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with tabs[2]:
        _di_vm_render_import(data, table)
    with tabs[3]:
        _di_vm_render_actions(data, table)
    with tabs[4]:
        _di_vm_render_export(data, table)
...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    # DEUDAS IMPOSITIVAS VM: lector especializado de Google Sheets e importador
    # de Excel con selección de hojas, cuadros heterogéneos y control de pagos.
    identity_deuda_vm = _di_vm_norm(f"{table} {module_name}")
    if (
        table == "deudas_impositivas_vm"
        or (
            "impositiv" in identity_deuda_vm
            and re.search(r"\bvm\b", identity_deuda_vm)
            and "vmr" not in identity_deuda_vm
        )
    ):
