...
def apply_filters(df: pd.DataFrame, module_name: str) -> pd.DataFrame:
    ...
    with c6:
        ...
        fecha_desde = st.date_input("Desde", value=primer_dia_mes, key=f"desde_{module_name}")
    with c7:
        fecha_hasta = st.date_input("Hasta", value=hoy, key=f"hasta_{module_name}")
    if search:
        mask = df.astype(str).apply(lambda col: col.str.contains(search, case=False, na=False)).any(axis=1)
        df = df[mask]
    if "estado" in df.columns and estado != "Todos":
        df = df[df["estado"].astype(str).str.strip().str.lower() == str(estado).strip().lower()]
        if "obra_social" in df.columns and obra_social != "Todos":
            df = df[df["obra_social"].astype(str).str.strip().str.lower() == str(obra_social).strip().lower()]
        if "procedimiento" in df.columns and procedimiento != "Todos":
            df = df[df["procedimiento"].astype(str).str.strip().str.lower() == str(procedimiento).strip().lower()]
        if "medico_responsable" in df.columns and medico != "Todos":
            df = df[df["medico_responsable"].astype(str).str.strip().str.lower() == str(medico).strip().lower()]   
    fecha_col = first_available_date_col(df, module_name)
    if fecha_col:
        s = df[fecha_col].astype(str).str.strip()
        
        fechas = pd.to_datetime(s, format="%d-%m%Y", errors="coerce")
        fechas = fechas.fillna(pd.to_datetime(s, dayfirst=True, errors="coerce"))
    ...
    st.write("DEBUG filtros:", estado, obra_social, procedimiento, medico, fecha_desde, fecha_hasta)

    st.write("DEBUG filas finales:", len(df))
