...
# IMPORTANTE:
# database.py sí tiene get_df(), pero no necesariamente add_balance_columns().
# Se importan por separado para que la ausencia de una función opcional
# NO inutilice la conexión completa a Google Sheets.
try:
...
try:
    from database import add_balance_columns
except Exception:  # pragma: no cover
...
def _profile_dataframe(df: pd.DataFrame) -> dict[str, Any]:
    ...
    # IMPORTANTE: usamos acceso POSICIONAL (iloc) porque algunas hojas de
    # Google Sheets pueden traer nombres de columnas duplicados. Con df[col],
    # pandas devuelve un DataFrame cuando el nombre está repetido y eso rompe
    # operaciones como int(...sum()) con: "int() argument ... not 'Series'".
...
def preguntar_copiloto(
    ...
    if not enterprise:
        if get_df is None:
            raise RuntimeError(
                "El Copiloto no pudo importar database.get_df(). "
                "La conexión de datos de la app no quedó disponible dentro de assistant.py."
            )
        raise RuntimeError(
            "El Copiloto pudo importar database.get_df(), pero no encontró módulos configurados "
            "para leer desde Google Sheets."
        )
