...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    if "mes" in tabla_limpia.columns:
        ...
        tabla_limpia["mes"] = pd.to_datetime(
        ...
        tabla_limpia["mes"] = tabla_limpia["mes"].dt.strftime("%d/%m/%Y")
...
def clave_orden_fecha(v):

    s = normalizar_fecha(v)

    return pd.to_datetime(s, format="%d/%m/%Y", errors="coerce")

if "mes" in tabla_limpia.columns:

    tabla_limpia["_orden_fecha"] = tabla_limpia["mes"].apply(clave_orden_fecha)
    ...
    tabla_limpia["mes"] = tabla_limpia["mes"].apply(normalizar_fecha)
