...
def render_analisis_anual_2026(df: pd.DataFrame):
    ...
    data["anio"] = data["mes"].dt.year

    data["numero_mes"] = data["mes"].dt.month

    datos_2026 = data[data["anio"] == 2026]
    ...
    # Toma el último mes cargado de 2026 para comparar

    # contra el mismo período de años anteriores.

    ultimo_mes_2026 = int(

        datos_2026["numero_mes"].max()
    ...
    nombre_ultimo_mes = nombres_meses.get(
    ...
    # Compara todos los años hasta el mismo mes alcanzado en 2026.

    comparacion = data[

        data["numero_mes"] <= ultimo_mes_2026

    ].copy()

    anual = (

        comparacion.groupby("anio")[monto_col]
    ...
    diferencia = facturacion_2026 - facturacion_2025

    if facturacion_2025 > 0:

        variacion = (

            diferencia / facturacion_2025

        ) * 100

        texto_variacion = f"{variacion:+.1f}%"

    else:

        texto_variacion = "Sin base 2025"
    ...
    col1.metric(
        ...
        fmt_money(facturacion_2026)

    )

    col2.metric(

        f"Facturación 2025 hasta {nombre_ultimo_mes}",

        fmt_money(facturacion_2025)
    ...
    fig = px.bar(
        ...
        title=(
            ...
            f"Facturación anual comparada hasta "
    ...
    st.plotly_chart(
        ...
        key="facturacion_anual_2026"

    )

    # Comparación mensual entre 2025 y 2026.

    comparacion_mensual = comparacion[

        comparacion["anio"].isin([2025, 2026])

    ].copy()

    comparacion_mensual = (

        comparacion_mensual.groupby(
        ...
        .sort_values(["anio", "numero_mes"])

    )

    comparacion_mensual["Mes"] = (

        comparacion_mensual["numero_mes"]
    ...
    comparacion_mensual["Año"] = (

        comparacion_mensual["anio"]
    ...
    fig2 = px.line(
        ...
        title="Comparación mensual 2025 vs 2026",
    ...
    st.plotly_chart(
        ...
        key="comparacion_anual_mensual_2026"
...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    with tab_panel:
        ...
        if df_panel.empty:
            ...
            render_analisis_anual_2026(filtered)
