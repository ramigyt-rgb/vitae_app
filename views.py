...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    # FECHA REAL PARA TABLA LIMPIA
    fecha_cols = [c for c in tabla_limpia.columns if "fecha" in c.lower()]
    fecha_cols = ["mes"] + fecha_cols + ["vencimiento"]
    fecha_cols = [c for c in fecha_cols if c in tabla_limpia.columns]
    tabla_limpia["_fecha_real"] = pd.NaT
    for col in fecha_cols:
        s = tabla_limpia[col].astype(str).str.strip()
        s = s.replace(["", "None", "none", "nan", "NaT"], pd.NA)
        fecha_parseada = pd.to_datetime(
            s,
            errors="coerce",
            dayfirst=False
        )
        tabla_limpia["_fecha_real"] = tabla_limpia["_fecha_real"].fillna(fecha_parseada)
    tabla_limpia = tabla_limpia.sort_values(
        by="_fecha_real",
        ascending=False,
        na_position="last"
    )
    tabla_limpia["mes"] = tabla_limpia["_fecha_real"].dt.strftime("%d/%m/%Y")
    tabla_limpia["mes"] = tabla_limpia["mes"].fillna("")
    tabla_limpia = tabla_limpia.drop(columns=["_fecha_real"], errors="ignore")
    st.dataframe(
        tabla_limpia,
        use_container_width=True,
        hide_index=True,
