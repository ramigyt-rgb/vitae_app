...
def format_facturacion_table(df: pd.DataFrame, labels: Dict[str, str]) -> pd.DataFrame:
    ...
    if "mes" in show.columns:
        show["mes"] = _facturacion_parse_fechas(
            show["mes"], permitir_futuras=False
...
def render_facturacion_industrial(
    ...
    # Procedimiento, factura y pago son hechos ya ocurridos: si una fecha
    # ambigua aparece en el futuro por inversión día/mes, se repara.
    # Vencimiento sí puede ser futuro y se conserva literalmente.
    data["_fecha_servicio"] = serie_fecha(col_mes, permitir_futuras=False)
