...
def render_facturacion_industrial(
    ...
    def serie_fecha(columna):
        """Lee fechas del Sheet sin invertir día/mes.

        Prioridad deliberada:
        1) ISO/año primero (YYYY-MM-DD, incluso si viene con hora o timezone),
        2) formato argentino DD/MM/YYYY o DD-MM-YYYY,
        3) serial Excel / Google Sheets.

        IMPORTANTE: no se aplica ``dayfirst=True`` sobre una cadena ISO completa.
        Pandas puede interpretar, por ejemplo, ``2026-01-09 00:00:00`` como
        01/09/2026. Ese era el origen de fechas futuras falsas en Facturación.
        """
        if columna is None or columna not in data.columns:
        ...
        # 1) Año primero / ISO. Se extrae únicamente la parte de fecha para que
        # también funcione con "2026-01-09 00:00:00" o "2026-01-09T00:00:00Z".
        iso_guion = raw.str.extract(
            r"^(\d{4}-\d{1,2}-\d{1,2})",
            expand=False,
        )
        mask_iso_guion = iso_guion.notna()
        if mask_iso_guion.any():
            fechas.loc[mask_iso_guion] = pd.to_datetime(
                iso_guion.loc[mask_iso_guion],
                format="%Y-%m-%d",
                errors="coerce",
            )

        iso_barra = raw.str.extract(
            r"^(\d{4}/\d{1,2}/\d{1,2})",
            expand=False,
        )
        mask_iso_barra = fechas.isna() & iso_barra.notna()
        if mask_iso_barra.any():
            fechas.loc[mask_iso_barra] = pd.to_datetime(
                iso_barra.loc[mask_iso_barra],
                format="%Y/%m/%d",
                errors="coerce",
            )

        # 2) Formato argentino explícito. También se toma sólo la parte de fecha
        # para tolerar celdas exportadas con una hora agregada.
        dmy_barra = raw.str.extract(
            r"^(\d{1,2}/\d{1,2}/\d{4})",
            expand=False,
        )
        mask_dmy_barra = fechas.isna() & dmy_barra.notna()
        if mask_dmy_barra.any():
            fechas.loc[mask_dmy_barra] = pd.to_datetime(
                dmy_barra.loc[mask_dmy_barra],
                format="%d/%m/%Y",
                errors="coerce",
            )

        dmy_guion = raw.str.extract(
            r"^(\d{1,2}-\d{1,2}-\d{4})",
            expand=False,
        )
        mask_dmy_guion = fechas.isna() & dmy_guion.notna()
        if mask_dmy_guion.any():
            fechas.loc[mask_dmy_guion] = pd.to_datetime(
                dmy_guion.loc[mask_dmy_guion],
                format="%d-%m-%Y",
                errors="coerce",
            )

        # 3) Fallback prudente para otros textos de fecha no ambiguos.
        ...
        # 4) Fechas serializadas de Excel / Google Sheets.
        ...
        return pd.to_datetime(fechas, errors="coerce").dt.normalize()
