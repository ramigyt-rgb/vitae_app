...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    fecha_origen = pd.Series(pd.NaT, index=tabla_limpia.index)
    ...
    for col in ["mes", "fecha", "fecha_factura", "vencimiento", "fecha_pago"]:
        ...
        if col in tabla_limpia.columns:
            ...
            s = s.replace(["", "None", "nan", "NaT"], pd.NA)

            fecha_iso = pd.to_datetime(s, format="%Y-%m-%d", errors="coerce")

            fecha_arg = pd.to_datetime(s, dayfirst=True, errors="coerce")

            fecha_parseada = fecha_iso.fillna(fecha_arg)

            fecha_origen = fecha_origen.fillna(fecha_parseada)
    ...
    tabla_limpia.insert(
        ...
        fecha_origen.dt.strftime("%d/%m/%Y").fillna("")

    )

    tabla_limpia["_orden_fecha"] = fecha_origen
