...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    if tabla_limpia is None or tabla_limpia.empty:
        ...
        fecha_col = None
        for c in ["mes", "fecha", "fecha_factura", "vencimiento", "created_at"]:
            if c in tabla_limpia.columns:
                fecha_col = c
                break
        if fecha_col:
            tabla_limpia[fecha_col] = pd.to_datetime(
                tabla_limpia[fecha_col],
                errors="coerce",
                dayfirst=True
            )
            ...
            tabla_limpia[fecha_col] = tabla_limpia[fecha_col].dt.strftime("%Y-%m-%d")
        st.dataframe(
            tabla_limpia,
            use_container_width=True,
            hide_index=True,
        ...
        if df.empty:
            ...
            if st.button("Guardar cambios", type="primary", key=f"guardar_editor_{table}"):
                ...
                try:
                    ...
                    limpio = limpio.drop(columns=["saldo", "saldo_usd", "saldo_movimiento"], errors="ignore")

                    sync_df_to_sheet(table, limpio)
                    total_guardado = len(limpio)

                    st.success(f"Cambios guardados correctamente. Registros procesados: {total_guardado}")
