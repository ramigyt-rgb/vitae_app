...
def preparar_tabla_editor(df, table):
    df_edit = add_balance_columns(df.copy())
    ...
    if "mes" in df_edit.columns:
        ...
        df_edit["mes"] = df_edit["mes"].dt.strftime("%d/%m/%Y")
    return df_edit
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    st.divider()
    st.markdown("##### Tabla limpia")
    tabla_limpia = preparar_tabla_editor(filtered, table)
    st.dataframe(
        tabla_limpia,
        use_container_width=True,
        hide_index=True,
    ...
    with tab_editar:
        ...
        if df.empty:
            ...
            df_edit = preparar_tabla_editor(df_base, table)
            if table == "cuenta_corriente_vm":
                df_edit = df_edit.drop(columns=["importe_usd", "pagado_usd"], errors="ignore")
