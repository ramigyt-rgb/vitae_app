...
def render_facturacion_industrial(
    ...
    with tab_resumen:
        ...
        top_obra = (
            activos.groupby("_obra_social")["_monto"].sum().sort_values(ascending=False)
        )
        top_proc = (
            activos.groupby("_procedimiento")["_monto"].sum().sort_values(ascending=False)
        )
        top_med = activos.groupby("_medico")["_monto"].sum().sort_values(ascending=False)

        mensajes = []
        if not top_obra.empty and texto_limpio(top_obra.index[0]):
            participacion = top_obra.iloc[0] / facturado * 100 if facturado else 0
            mensajes.append(
                f"La obra social con mayor facturación es **{top_obra.index[0]}**, "
                f"con {fmt_money(top_obra.iloc[0])} ({participacion:.1f}% del total)."
            )
        if not top_proc.empty and texto_limpio(top_proc.index[0]):
            mensajes.append(
                f"El procedimiento de mayor peso es **{top_proc.index[0]}**, "
                f"con {fmt_money(top_proc.iloc[0])}."
            )
        if not top_med.empty and texto_limpio(top_med.index[0]):
            mensajes.append(
                f"El médico con mayor facturación asociada es **{top_med.index[0]}**, "
                f"con {fmt_money(top_med.iloc[0])}."
            )
        if mensajes:
