...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    if "mes" in tabla_limpia.columns:
        ...
        tabla_limpia["mes"] = tabla_limpia["mes"].astype(str).str.strip()

        tabla_limpia["_orden_mes"] = pd.to_datetime(
        ...
        tabla_limpia["mes"] = tabla_limpia["_orden_mes"].dt.strftime("%d/%m/%Y")

        tabla_limpia["mes"] = tabla_limpia["mes"].fillna("")

        tabla_limpia = tabla_limpia.drop(columns=["_orden_mes"], errors="ignore")
