...
# =========================================================
# DESIGN SYSTEM + TELEMETRÍA LOCAL
# =========================================================
# Se mantiene la identidad VITAE y se evita introducir dependencias visuales
# externas. Plotly hereda una paleta consistente en todos los módulos.
VITAE_PLOTLY_COLORS = [
    "#B1567E", "#C56D92", "#8F3F66", "#D892B0",
    "#71324F", "#E8B8CC", "#9F4A70", "#F1D5E1",
]
px.defaults.color_discrete_sequence = VITAE_PLOTLY_COLORS
try:
    px.defaults.template = "plotly_white"
except Exception:
    pass

_VITAE_LOGGER = logging.getLogger("vitae.views")
_VITAE_DEBUG_ENV = str(os.getenv("VITAE_DEBUG", "")).strip().lower() in {
    "1", "true", "yes", "si", "sí", "on"
}

# =========================================================
# IA VITAE · IMPORTACIÓN TOLERANTE Y FALLBACKS REALES
# =========================================================
# La IA es una mejora opcional: nunca debe impedir que el sistema operativo
# principal cargue. Si assistant.py/director_ia.py no están disponibles, la
# aplicación conserva el resto de sus funciones y usa resúmenes locales sobre
# los datos reales cuando existe información suficiente.
try:
    import assistant as _vitae_assistant
    _VITAE_ASSISTANT_IMPORT_ERROR = ""
except Exception as _assistant_import_exc:
    _vitae_assistant = None
    _VITAE_ASSISTANT_IMPORT_ERROR = f"{type(_assistant_import_exc).__name__}: {_assistant_import_exc}"


def _ia_no_disponible(*args, **kwargs):
    return (
        "La integración de IA no está disponible en este momento. "
        "Los datos, cálculos, filtros y módulos de Vitae continúan operativos."
    )


def _copiloto_no_disponible(*args, **kwargs):
    return (
        "El Copiloto Vitae no está disponible en este momento. "
...
if _vitae_assistant is not None:
    preguntar_ia = getattr(_vitae_assistant, "preguntar_ia", _ia_no_disponible)
    preguntar_dashboard = getattr(_vitae_assistant, "preguntar_dashboard", _ia_no_disponible)
    ...
    preguntar_ia = _ia_no_disponible
    preguntar_dashboard = _ia_no_disponible
    preguntar_copiloto = _copiloto_no_disponible
    obtener_historial_copiloto = _historial_copiloto_vacio
    limpiar_historial_copiloto = _limpiar_historial_copiloto_fallback


def _director_resumen_local(global_df: pd.DataFrame | None = None, anio: Any = None, **kwargs) -> str:
    """Fallback ejecutivo basado únicamente en cifras presentes en global_df."""
    if global_df is None or not isinstance(global_df, pd.DataFrame) or global_df.empty:
        return "**Director IA no disponible.** No hay datos suficientes para generar un resumen local."

    def _sum(col: str) -> float:
        if col not in global_df.columns:
            return 0.0
        return float(pd.to_numeric(global_df[col], errors="coerce").fillna(0).sum())

    facturado = _sum("Facturado")
    cobrado = _sum("Cobrado")
    pendiente = _sum("Pendiente")
    egreso = _sum("Egreso")
    resultado = _sum("Resultado")
    periodo = f" {anio}" if anio else ""
    return (
        f"**Resumen local{periodo}.** Facturado: **${facturado:,.0f}** · "
        f"Cobrado: **${cobrado:,.0f}** · Pendiente: **${pendiente:,.0f}** · "
        f"Egresos: **${egreso:,.0f}** · Resultado: **${resultado:,.0f}**.  \n"
        "La integración externa de IA no está disponible; este resumen se calculó "
        "localmente sobre la información real cargada en Vitae."
    )


def _director_briefing_local(dfs: dict[str, pd.DataFrame] | None = None, **kwargs) -> dict[str, Any]:
    dfs = dfs or {}
    valid = {
        str(name): df for name, df in dfs.items()
        if isinstance(df, pd.DataFrame) and not df.empty
    }
    total_rows = int(sum(len(df) for df in valid.values()))
    now = pd.Timestamp.now().strftime("%d/%m/%Y %H:%M")
    return {
        "saludo": "Vitae",
        "emoji_nivel": "●",
        "nivel": "Análisis local",
        "contenido": (
            f"Hay **{len(valid)} módulos con datos** y **{total_rows:,} registros** disponibles. "
            "La integración externa de IA no está activa; el sistema mantiene operativos "
            "los análisis determinísticos y la información de cada módulo."
        ),
        "actualizado": now,
        "modulos_con_datos": len(valid),
        "modulos_analizados": len(dfs),
        "nombres_modulos": list(valid.keys()),
        "modulos_con_error": [],
    }
...
try:
    ...
    _VITAE_DIRECTOR_IMPORT_ERROR = ""
except Exception as _director_import_exc:
    _VITAE_DIRECTOR_IMPORT_ERROR = f"{type(_director_import_exc).__name__}: {_director_import_exc}"
    generar_resumen_ejecutivo = _director_resumen_local
    generar_briefing_automatico = _director_briefing_local


# =========================================================
# SEGURIDAD, USUARIOS Y PERMISOS VITAE
# =========================================================
# Este bloque vive en views.py para que pueda incorporarse sin modificar la
# estructura existente de la app. Persiste usuarios preferentemente en Google
# Sheets (hoja ``usuarios_vitae``) y usa un archivo local sólo como respaldo
# cuando la conexión todavía no está disponible.
import hashlib
import hmac
import json
import secrets
...
_VITAE_USERS_TABLE = "usuarios_vitae"
_VITAE_USERS_LOCAL_FILE = Path(__file__).resolve().parent / ".vitae_users.json"
_VITAE_USER_COLUMNS = [
    "id", "usuario", "nombre", "email", "rol", "activo", "empresas",
    "permisos", "password_hash", "password_salt", "creado_en",
    "actualizado_en", "ultimo_acceso",
]
_VITAE_ROLES = [
    "Administrador",
    "Dirección",
...
def _auth_bool(value: Any) -> bool:
    if isinstance(value, bool):
        return value
    if value is None or (isinstance(value, float) and pd.isna(value)):
        return False
    return str(value).strip().lower() in {"1", "true", "si", "sí", "yes", "activo", "active"}


def _auth_pack(values: Any) -> str:
    if values is None:
        return ""
...
def _auth_unpack(value: Any) -> list[str]:
    if value is None or (isinstance(value, float) and pd.isna(value)):
        return []
...
def _auth_hash_password(password: str, salt_hex: str | None = None) -> tuple[str, str]:
    salt = bytes.fromhex(salt_hex) if salt_hex else secrets.token_bytes(24)
    digest = hashlib.pbkdf2_hmac("sha256", password.encode("utf-8"), salt, 240_000)
    return digest.hex(), salt.hex()


def _auth_verify_password(password: str, expected_hash: str, salt_hex: str) -> bool:
    try:
        calculated, _ = _auth_hash_password(password, salt_hex)
        return hmac.compare_digest(calculated, str(expected_hash))
    except Exception:
        return False


def _auth_empty_users() -> pd.DataFrame:
    return pd.DataFrame(columns=_VITAE_USER_COLUMNS)


def _auth_normalize_users(df: pd.DataFrame | None) -> pd.DataFrame:
    if df is None or not isinstance(df, pd.DataFrame):
        return _auth_empty_users()
    result = df.copy()
    for col in _VITAE_USER_COLUMNS:
        if col not in result.columns:
            result[col] = ""
    result = result[_VITAE_USER_COLUMNS].copy()
    result["usuario"] = result["usuario"].fillna("").astype(str).str.strip().str.lower()
    result["nombre"] = result["nombre"].fillna("").astype(str).str.strip()
    result["email"] = result["email"].fillna("").astype(str).str.strip()
    result["rol"] = result["rol"].fillna("Personalizado").astype(str).str.strip()
    result["activo"] = result["activo"].apply(_auth_bool)
    result = result[result["usuario"] != ""].drop_duplicates(subset=["usuario"], keep="last").reset_index(drop=True)
    return result


def _auth_read_local() -> pd.DataFrame:
    try:
        if not _VITAE_USERS_LOCAL_FILE.exists():
            return _auth_empty_users()
        data = json.loads(_VITAE_USERS_LOCAL_FILE.read_text(encoding="utf-8"))
        return _auth_normalize_users(pd.DataFrame(data))
    except Exception:
        return _auth_empty_users()


def _auth_write_local(df: pd.DataFrame) -> None:
    try:
        records = df.fillna("").to_dict(orient="records")
        _VITAE_USERS_LOCAL_FILE.write_text(
...
def _auth_load_users(force: bool = False) -> pd.DataFrame:
    cache_key = "_vitae_users_cache"
    cache_time_key = "_vitae_users_cache_time"
    if not force and cache_key in st.session_state:
        age = time.time() - float(st.session_state.get(cache_time_key, 0) or 0)
        if age < 20:
            return _auth_normalize_users(st.session_state[cache_key])

    sheet_df = _auth_empty_users()
    try:
        sheet_df = _auth_normalize_users(get_df(_VITAE_USERS_TABLE))
    except Exception:
        sheet_df = _auth_empty_users()

    # Si la hoja todavía no existe, usa el respaldo local. En el primer guardado
    # se vuelve a intentar crear/sincronizar la hoja de Google Sheets.
    result = sheet_df if not sheet_df.empty else _auth_read_local()
    st.session_state[cache_key] = result.copy()
    st.session_state[cache_time_key] = time.time()
    return result


def _auth_save_users(df: pd.DataFrame) -> tuple[bool, str]:
    clean = _auth_normalize_users(df)
    sheet_ok = False
    detail = ""
    try:
        sync_df_to_sheet(_VITAE_USERS_TABLE, clean.copy())
        sheet_ok = True
        detail = "Google Sheets"
    except Exception as exc:
        detail = f"respaldo local ({exc})"
    _auth_write_local(clean)
    st.session_state["_vitae_users_cache"] = clean.copy()
    st.session_state["_vitae_users_cache_time"] = time.time()
    try:
        st.cache_data.clear()
    except Exception:
        pass
    return sheet_ok, detail


def _auth_permission_catalog() -> dict[str, str]:
    catalog = {
        "dashboard_global": "Dashboard Global",
        "configuracion": "Configuración y seguridad",
    }
    for module_name in MODULES.keys():
        catalog[f"mod::{module_name}"] = str(module_name)
    return catalog


def _auth_all_permissions() -> list[str]:
    return list(_auth_permission_catalog().keys())


def _auth_role_permissions(role: str) -> list[str]:
    role_norm = str(role or "").strip().lower()
    catalog = _auth_permission_catalog()
    all_perms = list(catalog.keys())
    if role_norm == "administrador":
        return all_perms
    if role_norm in {"dirección", "direccion"}:
        return [p for p in all_perms if p != "configuracion"]

    def matches(permission: str, words: tuple[str, ...]) -> bool:
        label = catalog.get(permission, permission).lower()
        return any(word in label for word in words)

    if role_norm == "finanzas":
        words = ("caja", "banco", "factur", "deuda", "pago", "préstamo", "prestamo", "cuenta corriente", "honorario", "gasto", "vencimiento", "alquiler")
        return ["dashboard_global"] + [p for p in all_perms if p.startswith("mod::") and matches(p, words)]
    if role_norm == "operaciones":
        words = ("agenda", "quir", "farmacia", "stock", "gine", "tarea", "convenio", "contrato")
        return ["dashboard_global"] + [p for p in all_perms if p.startswith("mod::") and matches(p, words)]
    if role_norm == "administración" or role_norm == "administracion":
        return [p for p in all_perms if p != "configuracion"]
    if role_norm == "solo lectura":
        return [p for p in all_perms if p != "configuracion"]
    return []


def _auth_current_user() -> dict[str, Any] | None:
    user = st.session_state.get("vitae_auth_user")
    return user if isinstance(user, dict) and user.get("usuario") else None


def _auth_is_admin(user: dict[str, Any] | None = None) -> bool:
    user = user or _auth_current_user()
    return bool(user and str(user.get("rol", "")).strip().lower() == "administrador")


# =========================================================
# DEFENSAS DE PRODUCCIÓN TRANSVERSALES
# =========================================================
def _vitae_debug_enabled() -> bool:
    """Habilita detalle técnico sólo en modo debug o para Administradores."""
    if _VITAE_DEBUG_ENV:
        return True
    try:
        return _auth_is_admin()
    except Exception:
        return False


def _vitae_exception_detail(exc: Exception, context: str = "") -> None:
    """Registra excepciones sin exponer trazas sensibles a usuarios comunes."""
    if exc is None:
        return
    try:
        _VITAE_LOGGER.error(
    ...
    if _vitae_debug_enabled():
        with st.expander("Detalle técnico", expanded=False):
            st.code(f"{type(exc).__name__}: {exc}", language="text")


def _vitae_render_error(message: str, exc: Exception | None = None, context: str = "") -> None:
    """Error visible, claro y aislado; el resto de la pantalla sigue operativo."""
    st.error(message)
    if exc is not None:
        _vitae_exception_detail(exc, context=context or message)


def _vitae_excel_bytes(df: pd.DataFrame, sheet_name: str = "Datos") -> bytes:
    """Genera Excel en memoria: evita archivos temporales y colisiones concurrentes."""
    buffer = io.BytesIO()
    safe_sheet = re.sub(r"[\\/*?:\[\]]", "_", str(sheet_name or "Datos"))[:31] or "Datos"
...
def _vitae_invalidate_data_cache() -> None:
    """Invalida caches de lectura después de escrituras o actualización manual."""
    try:
        st.cache_data.clear()
    except Exception:
        pass
...
def _vitae_lazy_selector(label: str, options: list[str], key: str) -> str:
    """Navegación de una sola sección por rerun; fallback compatible a st.radio."""
    if not options:
        return ""
...
def _auth_user_permissions(user: dict[str, Any] | None = None) -> set[str]:
    user = user or _auth_current_user()
    if not user:
        return set()
    if _auth_is_admin(user):
        return set(_auth_all_permissions())
    return set(_auth_unpack(user.get("permisos", "")))


def _auth_user_companies(user: dict[str, Any] | None = None) -> set[str]:
    user = user or _auth_current_user()
    if not user:
        return set()
    if _auth_is_admin(user):
        return set(_VITAE_EMPRESAS)
    return {v.upper() for v in _auth_unpack(user.get("empresas", ""))}


def _auth_permission_for_current_screen() -> tuple[str | None, str | None]:
    current = str(st.session_state.get("vitae_current_module", "") or "").strip()
    if current == "Dashboard Global":
        return "dashboard_global", None
    if current == "Configuración":
        return "configuracion", None
    if current in MODULES:
        company = str(MODULES.get(current, {}).get("empresa", "") or "").strip().upper()
        return f"mod::{current}", company or None
    return None, None


def _auth_can_access(permission: str | None, company: str | None = None, user: dict[str, Any] | None = None) -> bool:
    user = user or _auth_current_user()
    if not user:
        return False
    if _auth_is_admin(user):
        return True
    if permission and permission not in _auth_user_permissions(user):
        return False
    if company and company in _VITAE_EMPRESAS and company not in _auth_user_companies(user):
        return False
    return True


def _auth_login_css() -> None:
    st.markdown(
        """
        <style>
        .vitae-login-wrap{max-width:1120px;margin:3vh auto 0 auto;}
        .vitae-login-hero{background:linear-gradient(135deg,#3C1224 0%,#792348 58%,#B8527E 100%);border-radius:28px;padding:34px 38px;color:white;box-shadow:0 24px 70px rgba(60,18,36,.22);margin-bottom:18px;position:relative;overflow:hidden}
        .vitae-login-hero:after{content:"";position:absolute;width:290px;height:290px;border-radius:50%;right:-80px;top:-150px;background:rgba(255,255,255,.11)}
        .vitae-login-brand{font-size:14px;font-weight:800;letter-spacing:.18em;text-transform:uppercase;opacity:.78}
        .vitae-login-title{font-size:42px;font-weight:850;letter-spacing:-.04em;margin:7px 0 2px 0}
        .vitae-login-sub{font-size:16px;opacity:.88;max-width:700px;line-height:1.55}
        .vitae-security-note{border:1px solid rgba(121,35,72,.12);background:rgba(255,255,255,.82);border-radius:18px;padding:15px 18px;margin-top:12px;color:#4b2636}
        .vitae-user-chip{display:inline-flex;gap:8px;align-items:center;background:rgba(121,35,72,.08);border:1px solid rgba(121,35,72,.13);padding:7px 11px;border-radius:999px;color:#5C1B37;font-size:13px;font-weight:700}
        </style>
        """,
...
def _auth_set_session(user_row: pd.Series | dict[str, Any]) -> None:
    user = dict(user_row)
    user.pop("password_hash", None)
    user.pop("password_salt", None)
    st.session_state["vitae_auth_user"] = user
    st.session_state["vitae_auth_at"] = _auth_now()
    st.session_state["vitae_auth_failures"] = 0
    st.session_state.pop("vitae_auth_lock_until", None)


def _auth_logout() -> None:
    for key in ["vitae_auth_user", "vitae_auth_at", "vitae_auth_failures", "vitae_auth_lock_until"]:
        st.session_state.pop(key, None)


def render_vitae_login() -> None:
    _render_vitae_app_shell()
    _auth_login_css()
    # Oculta la navegación lateral mientras no exista una sesión válida.
    st.markdown(
        """
        <style>
    ...
    st.markdown(
        """
        <div class="vitae-login-wrap">
          <div class="vitae-login-hero">
            <div class="vitae-login-brand">Acceso seguro · Ecosistema Vitae</div>
            <div class="vitae-login-title">VITAE</div>
            <div class="vitae-login-sub">Gestión clínica, operativa y financiera con acceso individual, perfiles por función y permisos administrados desde un único centro de seguridad.</div>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    users = _auth_load_users(force=True)
    if users.empty:
        st.markdown("## Activación inicial")
        st.caption("Creá el primer Administrador. Después, todos los usuarios y permisos se manejan desde Configuración.")
        with st.form("vitae_bootstrap_admin", clear_on_submit=False):
            c1, c2 = st.columns(2)
            nombre = c1.text_input("Nombre y apellido", placeholder="Administrador Vitae")
            usuario = c2.text_input("Usuario", placeholder="admin")
            email = c1.text_input("Email", placeholder="nombre@vitae.com")
            c2.text_input("Rol", value="Administrador", disabled=True)
            p1, p2 = st.columns(2)
            password = p1.text_input("Contraseña", type="password")
            confirm = p2.text_input("Repetir contraseña", type="password")
            submitted = st.form_submit_button("Crear administrador y entrar", type="primary", use_container_width=True)
        if submitted:
            user_norm = usuario.strip().lower()
            if not nombre.strip() or not user_norm:
                ...
                pwd_hash, salt = _auth_hash_password(password)
                row = {
                    "id": uuid.uuid4().hex,
                    "usuario": user_norm,
                    "nombre": nombre.strip(),
                    ...
                    "rol": "Administrador",
                    "activo": True,
                    "empresas": _auth_pack(_VITAE_EMPRESAS),
                    "permisos": _auth_pack(_auth_all_permissions()),
                    "password_hash": pwd_hash,
                    "password_salt": salt,
                    "creado_en": _auth_now(),
                    "actualizado_en": _auth_now(),
                    "ultimo_acceso": _auth_now(),
                }
                new_df = pd.DataFrame([row], columns=_VITAE_USER_COLUMNS)
                _auth_save_users(new_df)
                _auth_set_session(row)
                st.success("Administrador creado correctamente.")
                st.rerun()
        st.markdown('<div class="vitae-security-note">🔐 Las contraseñas se almacenan con hash PBKDF2-SHA256 y salt individual; nunca se guarda la contraseña en texto plano.</div>', unsafe_allow_html=True)
        return

    lock_until = float(st.session_state.get("vitae_auth_lock_until", 0) or 0)
    if lock_until > time.time():
        remaining = int(lock_until - time.time()) + 1
        st.error(f"Acceso temporalmente bloqueado por intentos fallidos. Probá nuevamente en {remaining} segundos.")
        return

    left, center, right = st.columns([1.15, 1.7, 1.15])
    with center:
        st.markdown("## Iniciar sesión")
        st.caption("Ingresá con tu usuario personal de Vitae.")
        with st.form("vitae_login_form"):
            username = st.text_input("Usuario", autocomplete="username")
            password = st.text_input("Contraseña", type="password", autocomplete="current-password")
            submit = st.form_submit_button("Ingresar a Vitae", type="primary", use_container_width=True)
        if submit:
            key = username.strip().lower()
            match = users[users["usuario"] == key]
            valid = False
            if not match.empty:
                row = match.iloc[0]
                valid = _auth_bool(row.get("activo")) and _auth_verify_password(
                    password, row.get("password_hash", ""), row.get("password_salt", "")
                )
            if valid:
                idx = match.index[0]
                users.loc[idx, "ultimo_acceso"] = _auth_now()
                users.loc[idx, "actualizado_en"] = users.loc[idx, "actualizado_en"] or _auth_now()
                _auth_save_users(users)
                _auth_set_session(users.loc[idx])
                st.rerun()
            else:
                failures = int(st.session_state.get("vitae_auth_failures", 0) or 0) + 1
                st.session_state["vitae_auth_failures"] = failures
                if failures >= 5:
                    st.session_state["vitae_auth_lock_until"] = time.time() + 30
                    st.session_state["vitae_auth_failures"] = 0
                    st.error("Demasiados intentos fallidos. Acceso bloqueado por 30 segundos.")
                else:
                    st.error("Usuario o contraseña incorrectos, o cuenta desactivada.")


def _auth_require_login_and_permission() -> None:
    user = _auth_current_user()
    if not user:
        render_vitae_login()
        st.stop()

    # Revalida que la cuenta siga activa. Así una desactivación hecha por el
    # administrador corta el acceso en el siguiente cambio de pantalla.
    users = _auth_load_users()
    current_match = users[users["usuario"] == str(user.get("usuario", "")).lower()]
    if current_match.empty or not _auth_bool(current_match.iloc[0].get("activo")):
        _auth_logout()
        st.warning("Tu sesión ya no está habilitada.")
        render_vitae_login()
        st.stop()

    fresh = dict(current_match.iloc[0])
    fresh.pop("password_hash", None)
    fresh.pop("password_salt", None)
    st.session_state["vitae_auth_user"] = fresh
    permission, company = _auth_permission_for_current_screen()
    if not _auth_can_access(permission, company, fresh):
        _render_vitae_app_shell()
        st.error("No tenés permiso para ingresar a esta sección.")
        st.caption("El acceso se administra desde Configuración → Permisos.")
        if st.button("Cerrar sesión", key="vitae_no_access_logout"):
            _auth_logout()
            st.rerun()
        st.stop()


def _auth_render_session_bar() -> None:
    user = _auth_current_user() or {}
    name = str(user.get("nombre") or user.get("usuario") or "Usuario")
    role = str(user.get("rol") or "")
    current_module = str(st.session_state.get("vitae_current_module", "home") or "home")
    c1, c_refresh, c2 = st.columns([7.4, 1.25, 1.35])
    with c1:
        st.markdown(f'<span class="vitae-user-chip">● {name} · {role}</span>', unsafe_allow_html=True)
    with c_refresh:
        if st.button(
            "Actualizar",
            key=f"vitae_refresh_{current_module}",
            help="Descarta lecturas cacheadas y vuelve a consultar las fuentes de datos.",
            use_container_width=True,
        ):
            _vitae_invalidate_data_cache()
            st.rerun()
    with c2:
        if st.button("Salir", key=f"vitae_logout_{current_module}", use_container_width=True):
            _auth_logout()
            st.rerun()


def _auth_validate_new_password(password: str) -> str | None:
    if len(password) < 8:
        return "La contraseña debe tener al menos 8 caracteres."
    return None


def require_vitae_login() -> None:
    """Puerta de entrada global: llamar desde app.py antes de crear la navegación."""
    _auth_require_login_and_permission()


def _auth_user_safe_table(users: pd.DataFrame) -> pd.DataFrame:
    if users.empty:
        return pd.DataFrame(columns=["Nombre", "Usuario", "Rol", "Estado", "Empresas", "Último acceso"])
    result = users[["nombre", "usuario", "rol", "activo", "empresas", "ultimo_acceso"]].copy()
    result["activo"] = result["activo"].apply(lambda x: "Activo" if _auth_bool(x) else "Desactivado")
    result["empresas"] = result["empresas"].apply(lambda x: ", ".join(_auth_unpack(x)) or "—")
    result.columns = ["Nombre", "Usuario", "Rol", "Estado", "Empresas", "Último acceso"]
    return result


def _auth_render_user_management() -> None:
    users = _auth_load_users(force=True)
    total = len(users)
    active = int(users["activo"].apply(_auth_bool).sum()) if total else 0
    admins = int((users["rol"].astype(str).str.lower() == "administrador").sum()) if total else 0
    inactive = total - active

    st.markdown("### Centro de usuarios")
    st.caption("Altas, bajas lógicas, perfiles, credenciales y control de acceso desde un solo lugar.")
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Usuarios", total)
    m2.metric("Activos", active)
    m3.metric("Administradores", admins)
    m4.metric("Desactivados", inactive)

    st.markdown("#### Crear usuario")
    with st.container(border=True):
        with st.form("vitae_create_user_form"):
            c1, c2, c3 = st.columns(3)
            name = c1.text_input("Nombre y apellido", key="new_user_name")
            username = c2.text_input("Usuario", key="new_user_username")
            email = c3.text_input("Email", key="new_user_email")
            c4, c5, c6 = st.columns(3)
            role = c4.selectbox("Perfil inicial", _VITAE_ROLES, index=2, key="new_user_role")
            companies = c5.multiselect("Empresas", _VITAE_EMPRESAS, default=_VITAE_EMPRESAS, key="new_user_companies")
            active_new = c6.toggle("Cuenta activa", value=True, key="new_user_active")
            p1, p2 = st.columns(2)
            pwd = p1.text_input("Contraseña temporal", type="password", key="new_user_pwd")
            pwd2 = p2.text_input("Repetir contraseña", type="password", key="new_user_pwd2")
            create = st.form_submit_button("Crear usuario", type="primary", use_container_width=True)
        if create:
            uname = username.strip().lower()
            error = _auth_validate_new_password(pwd)
            if not name.strip() or not uname:
                st.error("Nombre y usuario son obligatorios.")
            elif uname in set(users["usuario"].tolist()):
                ...
                st.error("Asigná al menos una empresa.")
            else:
                pwd_hash, salt = _auth_hash_password(pwd)
                perms = _auth_all_permissions() if role == "Administrador" else _auth_role_permissions(role)
                row = {
                    "id": uuid.uuid4().hex, "usuario": uname, "nombre": name.strip(), "email": email.strip(),
                    "rol": role, "activo": bool(active_new), "empresas": _auth_pack(companies),
                    "permisos": _auth_pack(perms), "password_hash": pwd_hash, "password_salt": salt,
                    "creado_en": _auth_now(), "actualizado_en": _auth_now(), "ultimo_acceso": "",
                }
                users = pd.concat([users, pd.DataFrame([row])], ignore_index=True)
                _auth_save_users(users)
                st.success(f"Usuario {uname} creado correctamente.")
                st.rerun()

    st.markdown("#### Usuarios existentes")
    search = st.text_input("Buscar usuario", placeholder="Nombre, usuario, email o rol…", key="vitae_users_search")
    visible = users.copy()
    if search.strip():
        needle = search.strip().lower()
        mask = visible[["nombre", "usuario", "email", "rol"]].fillna("").astype(str).apply(
            lambda col: col.str.lower().str.contains(needle, regex=False)
        ).any(axis=1)
        visible = visible[mask]
    st.dataframe(_auth_user_safe_table(visible), use_container_width=True, hide_index=True, height=min(460, 90 + 38 * max(1, len(visible))))

    if users.empty:
        return
    labels = {row["usuario"]: f"{row['nombre'] or row['usuario']} · {row['rol']}" for _, row in users.iterrows()}
    selected = st.selectbox("Administrar cuenta", list(labels.keys()), format_func=lambda x: labels[x], key="manage_user_select")
    idx = users.index[users["usuario"] == selected][0]
    row = users.loc[idx].copy()
    with st.container(border=True):
        st.markdown(f"##### {row['nombre'] or row['usuario']}")
        e1, e2, e3 = st.columns(3)
        edit_name = e1.text_input("Nombre", value=str(row["nombre"]), key=f"edit_name_{selected}")
        edit_email = e2.text_input("Email", value=str(row["email"]), key=f"edit_email_{selected}")
        edit_role = e3.selectbox("Rol", _VITAE_ROLES, index=_VITAE_ROLES.index(row["rol"]) if row["rol"] in _VITAE_ROLES else len(_VITAE_ROLES)-1, key=f"edit_role_{selected}")
        edit_companies = st.multiselect("Empresas habilitadas", _VITAE_EMPRESAS, default=[c for c in _auth_unpack(row["empresas"]) if c in _VITAE_EMPRESAS], key=f"edit_companies_{selected}")
        edit_active = st.toggle("Cuenta activa", value=_auth_bool(row["activo"]), key=f"edit_active_{selected}")
        b1, b2 = st.columns(2)
        save_profile = b1.button("Guardar perfil", type="primary", use_container_width=True, key=f"save_profile_{selected}")
        disable_label = "Desactivar usuario" if _auth_bool(row["activo"]) else "Activar usuario"
        toggle_state = b2.button(disable_label, use_container_width=True, key=f"toggle_state_{selected}")
        if save_profile:
            if not edit_name.strip() or not edit_companies:
                st.error("El nombre y al menos una empresa son obligatorios.")
            else:
                users.loc[idx, "nombre"] = edit_name.strip()
                users.loc[idx, "email"] = edit_email.strip()
                users.loc[idx, "rol"] = edit_role
                users.loc[idx, "empresas"] = _auth_pack(edit_companies)
                users.loc[idx, "activo"] = bool(edit_active)
                if edit_role == "Administrador":
                    users.loc[idx, "permisos"] = _auth_pack(_auth_all_permissions())
                users.loc[idx, "actualizado_en"] = _auth_now()
                _auth_save_users(users)
                st.success("Perfil actualizado.")
                st.rerun()
        if toggle_state:
            if selected == str((_auth_current_user() or {}).get("usuario")) and _auth_bool(row["activo"]):
                st.error("No podés desactivar tu propia sesión de Administrador.")
            else:
                users.loc[idx, "activo"] = not _auth_bool(row["activo"])
                users.loc[idx, "actualizado_en"] = _auth_now()
                _auth_save_users(users)
                st.rerun()

        with st.expander("Cambiar contraseña"):
            with st.form(f"reset_pwd_{selected}"):
                np1 = st.text_input("Nueva contraseña", type="password", key=f"np1_{selected}")
                np2 = st.text_input("Repetir nueva contraseña", type="password", key=f"np2_{selected}")
                do_reset = st.form_submit_button("Actualizar contraseña", use_container_width=True)
            if do_reset:
                err = _auth_validate_new_password(np1)
                if np1 != np2:
                    st.error("Las contraseñas no coinciden.")
                elif err:
                    st.error(err)
                else:
                    pwd_hash, salt = _auth_hash_password(np1)
                    users.loc[idx, "password_hash"] = pwd_hash
                    users.loc[idx, "password_salt"] = salt
                    users.loc[idx, "actualizado_en"] = _auth_now()
                    _auth_save_users(users)
                    st.success("Contraseña actualizada.")

        with st.expander("Eliminar cuenta definitivamente"):
            st.warning("Esta acción elimina el usuario de la lista. Para conservar trazabilidad, normalmente conviene desactivarlo.")
            confirm_delete = st.checkbox(f"Confirmo eliminar a {selected}", key=f"delete_confirm_{selected}")
            if st.button("Eliminar usuario", disabled=not confirm_delete, key=f"delete_user_{selected}"):
                if selected == str((_auth_current_user() or {}).get("usuario")):
                    st.error("No podés eliminar tu propia cuenta mientras estás conectado.")
                else:
                    users = users[users["usuario"] != selected].reset_index(drop=True)
                    _auth_save_users(users)
                    st.success("Usuario eliminado.")
                    st.rerun()


def _auth_render_permissions_management() -> None:
    users = _auth_load_users(force=True)
    if users.empty:
        st.info("Creá al menos un usuario para administrar permisos.")
        return
    st.markdown("### Matriz de permisos")
    st.caption("Elegí exactamente qué puede ver cada persona. Los permisos se aplican al intentar abrir cada módulo.")
    catalog = _auth_permission_catalog()
    labels = {row["usuario"]: f"{row['nombre'] or row['usuario']} · {row['rol']}" for _, row in users.iterrows()}
    selected = st.selectbox("Usuario", list(labels.keys()), format_func=lambda x: labels[x], key="permission_user_select")
    idx = users.index[users["usuario"] == selected][0]
    row = users.loc[idx].copy()
    role = str(row["rol"])
    if role == "Administrador":
        st.success("Administrador · acceso total a todas las empresas, módulos y Configuración.")
        st.dataframe(pd.DataFrame({"Acceso": list(catalog.values()), "Estado": ["Permitido"] * len(catalog)}), use_container_width=True, hide_index=True)
        return

    current_permissions = set(_auth_unpack(row["permisos"]))
    current_companies = [c for c in _auth_unpack(row["empresas"]) if c in _VITAE_EMPRESAS]
    with st.container(border=True):
        p1, p2 = st.columns([1.1, 1.9])
        with p1:
            st.markdown("##### Perfil y alcance")
            new_role = st.selectbox("Perfil", _VITAE_ROLES[1:], index=_VITAE_ROLES[1:].index(role) if role in _VITAE_ROLES[1:] else len(_VITAE_ROLES[1:])-1, key=f"perm_role_{selected}")
            companies = st.multiselect("Empresas", _VITAE_EMPRESAS, default=current_companies, key=f"perm_companies_{selected}")
            apply_template = st.button("Aplicar plantilla del perfil", use_container_width=True, key=f"apply_template_{selected}")
            if apply_template:
                st.session_state[f"perm_selection_{selected}"] = _auth_role_permissions(new_role)
        with p2:
            st.markdown("##### Accesos habilitados")
            default_keys = [p for p in current_permissions if p in catalog]
            selected_labels_default = [catalog[p] for p in default_keys]
            if f"perm_selection_{selected}" in st.session_state:
                selected_labels_default = [catalog[p] for p in st.session_state[f"perm_selection_{selected}"] if p in catalog]
            selected_labels = st.multiselect(
                "Módulos y funciones",
                options=list(catalog.values()),
                default=selected_labels_default,
                key=f"perm_labels_{selected}",
            )
            reverse = {label: key for key, label in catalog.items()}
            selected_perms = [reverse[label] for label in selected_labels if label in reverse]
        save = st.button("Guardar permisos", type="primary", use_container_width=True, key=f"save_perms_{selected}")
        if save:
            if not companies:
                st.error("Asigná al menos una empresa.")
            else:
                users.loc[idx, "rol"] = new_role
                users.loc[idx, "empresas"] = _auth_pack(companies)
                users.loc[idx, "permisos"] = _auth_pack(selected_perms)
                users.loc[idx, "actualizado_en"] = _auth_now()
                _auth_save_users(users)
                st.success("Permisos guardados y aplicados.")
                st.rerun()

    rows = []
    for permission, label in catalog.items():
        if not permission.startswith("mod::"):
            company = "Sistema"
        else:
            module = permission.split("::", 1)[1]
            company = str(MODULES.get(module, {}).get("empresa", "—"))
        rows.append({"Función / módulo": label, "Empresa": company, "Acceso": "Sí" if permission in current_permissions else "No"})
    st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True, height=480)


def _auth_render_companies_management() -> None:
    users = _auth_load_users(force=True)
    st.markdown("### Acceso por empresa")
    st.caption("Vista ejecutiva de qué usuarios tienen habilitada cada unidad. El permiso por empresa se combina con el permiso específico del módulo.")
    if users.empty:
        st.info("No hay usuarios cargados.")
        return
    rows = []
    for _, row in users.iterrows():
        companies = set(_auth_unpack(row["empresas"]))
        rows.append({
            "Usuario": row["nombre"] or row["usuario"],
            "Rol": row["rol"],
            "VITAE": "✓" if _auth_is_admin(dict(row)) or "VITAE" in companies else "—",
            "VM": "✓" if _auth_is_admin(dict(row)) or "VM" in companies else "—",
            "VMR": "✓" if _auth_is_admin(dict(row)) or "VMR" in companies else "—",
            "Estado": "Activo" if _auth_bool(row["activo"]) else "Desactivado",
        })
    st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
    st.info("Para modificar el alcance de una persona, usá la pestaña Permisos o Usuarios.")


def _auth_render_security_status() -> None:
    users = _auth_load_users(force=True)
    st.markdown("### Seguridad del sistema")
    a, b, c = st.columns(3)
    a.metric("Cuentas activas", int(users["activo"].apply(_auth_bool).sum()) if not users.empty else 0)
    b.metric("Hash de contraseñas", "PBKDF2-SHA256")
    b.caption("240.000 iteraciones + salt individual")
    c.metric("Bloqueo de acceso", "5 intentos")
    c.caption("30 segundos por sesión")
    st.success("Las contraseñas nunca se muestran ni se almacenan en texto plano.")
...
def safe_panel(func_name, *args, **kwargs):
    """Aísla fallos de paneles opcionales para que una sección no derribe la app."""
    func = globals().get(func_name)
    if not callable(func):
        _VITAE_LOGGER.warning("Panel no disponible: %s", func_name)
        return None
    try:
        return func(*args, **kwargs)
    except Exception as exc:
        _vitae_render_error(
            "Esta sección no pudo cargarse, pero el resto de Vitae continúa disponible.",
            exc,
            context=f"safe_panel:{func_name}",
        )
        return None
...
def load_all_data():
    """Lectura consolidada con cache corta; cada tabla se consulta una sola vez."""
    data = {}
    for cfg in MODULES.values():
        ...
        try:
            ...
            _VITAE_LOGGER.warning("No se pudo precargar %s: %s", tabla, exc)
            data[tabla] = pd.DataFrame()
    return data
def _render_vitae_app_shell() -> None:
    ...
    st.markdown(
        ...
        :root {
            ...
            --vitae-text: #331723;
            --vitae-muted: #687386;
            --vitae-line: rgba(60,18,36, .11);
            --vitae-shadow: 0 14px 38px rgba(70,20,42, .08);
        }
        ...
        .stApp {
            background:
                radial-gradient(circle at 88% 4%, rgba(184,82,126, .10), transparent 24rem),
                radial-gradient(circle at 7% 18%, rgba(162,47,97, .08), transparent 28rem),
                linear-gradient(180deg, #EDD4DF 0%, #F9F3F6 100%);
        ...
        header[data-testid="stHeader"] {
            background: rgba(237,212,223, .82);
            backdrop-filter: blur(18px);
            border-bottom: 1px solid rgba(60,18,36, .06);
        }
        section[data-testid="stSidebar"] {
            background: linear-gradient(180deg, #3C1224 0%, #5C1B37 58%, #722144 100%);
        ...
        section[data-testid="stSidebar"] div[data-baseweb="select"] > div {
            ...
            box-shadow: 0 7px 18px rgba(30,9,18, .18) !important;
            min-height: 3rem;
        }
        ...
        section[data-testid="stSidebar"] div[data-baseweb="select"] [data-testid="stSelectboxVirtualDropdown"] {
            color: #3C1224 !important;
            -webkit-text-fill-color: #3C1224 !important;
            opacity: 1 !important;
        }
        section[data-testid="stSidebar"] div[data-baseweb="select"] svg {
            fill: #792348 !important;
            color: #792348 !important;
        }
        ...
        .vitae-app-header {
            ...
            border: 1px solid rgba(60,18,36, .10);
            border-radius: 24px;
            background: linear-gradient(125deg, rgba(255,255,255,.98), rgba(237,212,223, .96));
        ...
        .vitae-app-header::after {
            ...
            top: -145px;
            border-radius: 50%;
            background: linear-gradient(135deg, rgba(162,47,97, .22), rgba(184,82,126, .08));
        }
        .vitae-eyebrow {
            font-size: .69rem;
            letter-spacing: .16em;
        ...
        .vitae-live::before {
            ...
            border-radius: 50%;
            background: #BA3D73;
            box-shadow: 0 0 0 4px rgba(186,61,115, .13);
        }
        ...
        div[data-testid="stMetric"] {
            ...
            box-shadow: 0 8px 24px rgba(70,20,42, .055);
            min-height: 118px;
            transition: transform .18s ease, box-shadow .18s ease, border-color .18s ease;
        }
        div[data-testid="stMetric"]:hover {
            transform: translateY(-2px);
            border-color: rgba(162,47,97, .26);
            box-shadow: 0 14px 30px rgba(70,20,42, .09);
        }
        ...
        div[data-testid="stVerticalBlockBorderWrapper"] {
            ...
            box-shadow: 0 8px 28px rgba(70,20,42, .045);
        }
        ...
        .stButton > button, .stDownloadButton > button, .stFormSubmitButton > button {
            min-height: 2.65rem;
            border-radius: 12px;
            border: 1px solid rgba(121,35,72, .16);
            background: linear-gradient(135deg, #792348, #8C2853);
            color: white;
            font-weight: 760;
            box-shadow: 0 6px 16px rgba(121,35,72, .15);
            transition: transform .16s ease, box-shadow .16s ease, filter .16s ease;
        }
        .stButton > button:hover, .stDownloadButton > button:hover, .stFormSubmitButton > button:hover {
            transform: translateY(-1px);
            filter: brightness(1.04);
            box-shadow: 0 10px 22px rgba(121,35,72, .22);
            color: white;
        }
        ...
        .stNumberInput input {
            ...
            border-color: rgba(60,18,36, .14) !important;
        ...
        .stTextInput input:focus {
            border-color: rgba(162,47,97, .65) !important;
            box-shadow: 0 0 0 3px rgba(162,47,97, .10) !important;
        }
        div[data-testid="stDataFrame"], div[data-testid="stTable"] {
            ...
            overflow: hidden;
            background: white;
            box-shadow: 0 7px 22px rgba(70,20,42, .045);
        }
        div[data-testid="stPlotlyChart"] {
            ...
            padding: .25rem;
            box-shadow: 0 8px 24px rgba(70,20,42, .045);
        }
        div[data-testid="stTabs"] [data-baseweb="tab-list"] {
            gap: .35rem;
            background: rgba(242,229,235, .72);
            padding: .3rem;
            border-radius: 14px;
        }
        ...
        div[data-testid="stTabs"] button[aria-selected="true"] {
            ...
            box-shadow: 0 4px 12px rgba(70,20,42, .08);
        }
        div[data-testid="stAlert"] { border-radius: 15px; }
        hr { border-color: rgba(60,18,36, .09); }
        ...
        /* PALETA CORPORATIVA VITAE · ROSA DE MARCA */
        :root {
            --vitae-rose: #B1567E;
            --vitae-rose-deep: #71324F;
            --vitae-rose-dark: #3A1E2D;
            --vitae-rose-soft: #FFF6FA;
        }
        [data-testid="stTabs"] button[aria-selected="true"] {
            color: #B1567E !important;
            border-bottom-color: #B1567E !important;
        }
        [data-testid="stTabs"] [data-baseweb="tab-highlight"] {
            background-color: #B1567E !important;
        }
        .stButton > button, .stDownloadButton > button, .stFormSubmitButton > button {
            background: linear-gradient(135deg, #A94770 0%, #B1567E 55%, #C56D92 100%) !important;
            border-color: rgba(177,86,126,.30) !important;
            box-shadow: 0 8px 20px rgba(177,86,126,.20) !important;
        }
        .stButton > button:hover, .stDownloadButton > button:hover, .stFormSubmitButton > button:hover {
            box-shadow: 0 11px 26px rgba(177,86,126,.28) !important;
        }
        section[data-testid="stSidebar"] {
            background: linear-gradient(180deg, #3A1E2D 0%, #71324F 58%, #A94770 100%) !important;
        }
        ...
        /* =====================================================
           VITAE PRODUCT UI · CAPA FINAL DE CONSISTENCIA
           ===================================================== */
        :root {
            color-scheme: light;
            --vitae-radius-xs: 9px;
            --vitae-radius-sm: 12px;
            --vitae-radius-md: 16px;
            --vitae-radius-lg: 22px;
            --vitae-focus: 0 0 0 3px rgba(177,86,126,.14);
        }

        .block-container > div {
            animation: vitae-enter .18s ease-out both;
        }
        @keyframes vitae-enter {
            from { opacity: .01; transform: translateY(2px); }
            to { opacity: 1; transform: translateY(0); }
        }

        [data-testid="stForm"] {
            border: 1px solid rgba(60,18,36,.10);
            border-radius: var(--vitae-radius-lg);
            background: rgba(255,255,255,.86);
            padding: 1rem 1rem .35rem 1rem;
            box-shadow: 0 8px 26px rgba(70,20,42,.045);
        }
        [data-testid="stDataEditor"] {
            border: 1px solid rgba(60,18,36,.10);
            border-radius: var(--vitae-radius-md);
            /* Mantener visibles los controles flotantes nativos del editor. */
            overflow: visible !important;
            background: #fff;
            box-shadow: 0 7px 22px rgba(70,20,42,.04);
        }
        /* Streamlit oculta la barra de herramientas hasta hover en varias
           versiones. En iPad/touch no existe un hover real, por lo que los
           controles de tabla pueden quedar inaccesibles. Los dejamos siempre
           visibles cuando el frontend los renderiza. */
        ...
        [data-testid="stDataEditor"] [data-testid="stElementToolbarButton"],
        ...
        [data-testid="stFileUploader"] section {
            border-radius: var(--vitae-radius-md) !important;
            border-color: rgba(177,86,126,.22) !important;
        ...
        [data-testid="stSegmentedControl"] > div {
            border: 1px solid rgba(60,18,36,.09);
            border-radius: 14px;
            background: rgba(255,255,255,.72);
            padding: .22rem;
            box-shadow: 0 5px 16px rgba(70,20,42,.035);
        }
        ...
        [data-testid="stToast"] {
            border: 1px solid rgba(60,18,36,.10);
            border-radius: var(--vitae-radius-md);
            box-shadow: 0 16px 38px rgba(70,20,42,.12);
        }
        [data-testid="stProgress"] > div > div > div {
            background: linear-gradient(90deg, #9F4A70, #C56D92) !important;
        }
        ...
        a:hover { color: #71324F; }

        button:focus-visible, input:focus-visible, textarea:focus-visible,
        [role="combobox"]:focus-visible {
            outline: none !important;
            box-shadow: var(--vitae-focus) !important;
        }
        button:disabled {
        ...
        .vitae-status-dot {
            ...
            margin-top: .33rem; flex: 0 0 auto;
            background: #B1567E;
            box-shadow: 0 0 0 4px rgba(177,86,126,.10);
        }
        ...
        @media (max-width: 640px) {
            ...
            .vitae-app-header {
                margin-bottom: .8rem;
                box-shadow: 0 7px 22px rgba(70,20,42,.06);
            }
            div[data-testid="stMetric"] {
                padding: .78rem .82rem;
                min-height: 96px;
            ...
            div[data-testid="stMetricValue"] {
                font-size: clamp(1.22rem, 8vw, 1.75rem) !important;
            }
...
def _copilot_runtime_context(modulo_actual: str) -> dict[str, Any]:
    """Contexto UI seguro: módulo + períodos activos, sin copiar formularios ni datos sensibles."""
    context: dict[str, Any] = {"modulo_actual": str(modulo_actual or "Inicio")}
    allowed_exact = {"dg_period_option", "dg_custom_dates"}
    allowed_suffixes = ("_periodo", "_desde", "_hasta", "_mes", "_anio")
    filters: dict[str, Any] = {}
    try:
        for key in list(st.session_state.keys()):
            key_text = str(key)
            if key_text not in allowed_exact and not key_text.endswith(allowed_suffixes):
                continue
            value = st.session_state.get(key)
            if isinstance(value, (str, int, float, bool, date, pd.Timestamp)):
                filters[key_text] = str(value)
            elif isinstance(value, (tuple, list)) and len(value) <= 4:
                if all(isinstance(item, (str, int, float, bool, date, pd.Timestamp)) for item in value):
                    filters[key_text] = [str(item) for item in value]
    except Exception:
        filters = {}
    if filters:
        context["filtros_temporales"] = filters
    return context


def _preguntar_copiloto_contextual(pregunta: str, modulo_actual: str) -> Any:
    """Usa contexto enriquecido sólo si assistant.py lo soporta; mantiene retrocompatibilidad."""
    kwargs: dict[str, Any] = {
        "modulo_actual": modulo_actual,
        "ruta_actual": modulo_actual,
    }
    try:
        signature = inspect.signature(preguntar_copiloto)
        parameters = signature.parameters.values()
        accepts_kwargs = any(p.kind == inspect.Parameter.VAR_KEYWORD for p in parameters)
        if "contexto_ui" in signature.parameters or accepts_kwargs:
            kwargs["contexto_ui"] = _copilot_runtime_context(modulo_actual)
    except Exception:
        pass
    return preguntar_copiloto(pregunta, **kwargs)
            ...
            if final_question:
                try:
                    with st.spinner("Analizando Vitae…"):
                        answer = _preguntar_copiloto_contextual(
                            final_question,
...
def _dg_previous_period(
    period_option: str,
    start: pd.Timestamp,
    end: pd.Timestamp,
) -> tuple[pd.Timestamp, pd.Timestamp] | None:
    """Período comparable real, respetando semántica calendario cuando corresponde."""
    option = str(period_option or "").strip()
    if option == "Histórico":
        return None
    ...
    if option == "Mes actual":
        ...
        day_target = min(end.day, int(previous_month_end.days_in_month))
        previous_end = previous_start.replace(day=day_target)
        return previous_start, previous_end
    if option == "Mes anterior":
        previous_end = start - pd.Timedelta(days=1)
        return previous_end.replace(day=1), previous_end
    if option == "Año actual":
    ...
    if option == "Últimos 12 meses":
        return (start - pd.DateOffset(months=12)).normalize(), (end - pd.DateOffset(months=12)).normalize()

    # Rango personalizado: compara contra el bloque inmediatamente anterior
    # con idéntica cantidad de días.
    span_days = max(int((end - start).days), 0)
    previous_end = start - pd.Timedelta(days=1)
    previous_start = previous_end - pd.Timedelta(days=span_days)
    return previous_start, previous_end


def _dg_change_text(current: float, previous: float) -> str:
    current = float(current or 0.0)
    previous = float(previous or 0.0)
    difference = current - previous
    if abs(previous) > 1e-9:
        pct = difference / abs(previous) * 100.0
        sign = "+" if pct > 0 else ""
        return f"{sign}{pct:.1f}% · Δ {fmt_money(difference)}"
    if abs(current) > 1e-9:
        return f"Sin base comparable · Δ {fmt_money(difference)}"
    return "Sin variación"


def _dg_render_period_comparison(
    model: dict[str, Any],
    previous_model: dict[str, Any] | None,
    previous_period: tuple[pd.Timestamp, pd.Timestamp] | None,
    start: pd.Timestamp,
    end: pd.Timestamp,
    today: pd.Timestamp,
) -> None:
    if previous_model is not None and previous_period is not None:
        p_start, p_end = previous_period
        st.markdown("### Cambio contra período comparable")
        st.caption(
            f"Actual: {start.strftime('%d/%m/%Y')} — {end.strftime('%d/%m/%Y')} · "
            f"Comparación: {p_start.strftime('%d/%m/%Y')} — {p_end.strftime('%d/%m/%Y')}"
        )
        c1, c2, c3, c4 = st.columns(4)
        c1.metric(
            "Facturación",
            fmt_money(model["billed_period"]),
            delta=_dg_change_text(model["billed_period"], previous_model["billed_period"]),
        )
        c2.metric(
            "Cobrado",
            fmt_money(model["collected_period"]),
            delta=_dg_change_text(model["collected_period"], previous_model["collected_period"]),
        )
        c3.metric(
            "Flujo neto",
            fmt_money(model["cash_flow"]),
            delta=_dg_change_text(model["cash_flow"], previous_model["cash_flow"]),
        )
        current_patients = int(model.get("patients_period", 0) or 0)
        previous_patients = int(previous_model.get("patients_period", 0) or 0)
        patient_diff = current_patients - previous_patients
        patient_delta = (
            f"{patient_diff:+d} pacientes" if patient_diff else "Sin variación"
        )
        c4.metric("Pacientes", current_patients, delta=patient_delta)

    # Proyección explicable únicamente para el mes corriente y corte a hoy.
    if (
        start.year == today.year
        and start.month == today.month
        and start.day == 1
        and end.normalize() == today.normalize()
        and model.get("billed_period", 0) > 0
    ):
        elapsed = max(int(today.day), 1)
        days_in_month = int(today.days_in_month)
        projected = float(model["billed_period"]) / elapsed * days_in_month
        remaining = max(projected - float(model["billed_period"]), 0.0)
        st.markdown(
            f'<div class="vitae-system-note"><span class="vitae-status-dot"></span>'
            f'<div><strong>Proyección lineal de cierre</strong><br>'
            f'Con {elapsed} de {days_in_month} días transcurridos, el ritmo actual proyecta '
            f'<strong>{fmt_money(projected)}</strong> de facturación mensual. '
            f'Para alcanzar esa proyección restan <strong>{fmt_money(remaining)}</strong>. '
            f'Es una extrapolación lineal, no una predicción de IA.</div></div>',
            unsafe_allow_html=True,
        )
...
def _dg_build_model(
    ...
    for module_name, cfg in MODULES.items():
        ...
        # Estado descriptivo sin umbrales inventados: la completitud exacta
        # queda expresada en la columna numérica "Calidad %".
        coverage = "Sin datos" if df.empty else "Con datos"
        ...
        if category == "Facturación":
            ...
            if "mes" in df.columns:
                _mes_dashboard = df.loc[:, "mes"]
                if isinstance(_mes_dashboard, pd.DataFrame):
                    _mes_dashboard = _mes_dashboard.iloc[:, 0]
                service_date = parse_mes(_mes_dashboard)
            else:
                service_date = _dg_date_series(
                    df,
                    ["fecha", "fecha_procedimiento", "fecha práctica", "fecha_practica"],
                )
...
def _dg_render_css() -> None:
    st.markdown(
        ...
        <style>
        .dg-hero {
            border: 1px solid rgba(82,46,62, .12);
            border-radius: 22px;
            padding: 1.25rem 1.4rem;
            margin: .15rem 0 1rem 0;
            background: linear-gradient(135deg, rgba(241,208,222, .98), rgba(236,213,223, .96));
            box-shadow: 0 12px 34px rgba(48,24,34, .08);
        }
        .dg-kicker {font-size:.72rem; letter-spacing:.14em; text-transform:uppercase; color:#697386; font-weight:800;}
        .dg-title {font-size:2rem; line-height:1.08; color:#351E28; font-weight:850; margin:.2rem 0 .35rem 0;}
        .dg-subtitle {font-size:.94rem; color:#667085; max-width:920px;}
        .dg-period {display:inline-block; margin-top:.75rem; padding:.35rem .65rem; border-radius:999px; background:#fff; border:1px solid rgba(82,46,62, .12); color:#573141; font-size:.8rem; font-weight:700;}
        .dg-section-title {font-size:1.08rem; font-weight:820; color:#3B212C; margin:.3rem 0 .15rem 0;}
        .dg-section-copy {font-size:.84rem; color:#737b8c; margin-bottom:.75rem;}
        .dg-callout {border-radius:16px; padding:.9rem 1rem; border:1px solid rgba(82,46,62, .1); background:#FCFAFB; margin:.35rem 0; min-height:92px;}
        .dg-callout strong {display:block; color:#442734; font-size:.88rem; margin-bottom:.18rem;}
        .dg-callout span {color:#697386; font-size:.82rem; line-height:1.35;}
        .dg-status-good {background:#EED3DF; color:#651D3C;}
        ...
        div[data-testid="stMetric"] {
            border:1px solid rgba(82,46,62, .11);
            border-radius:17px;
            padding:.78rem .9rem;
            background:rgba(255,255,255,.92);
            box-shadow:0 7px 18px rgba(49,22,34, .045);
            min-height:116px;
        }
        div[data-testid="stMetricLabel"] {font-weight:700; color:#697386;}
        div[data-testid="stMetricValue"] {font-weight:820; color:#3B212C; font-variant-numeric:tabular-nums;}
...
def _dg_render_alerts(model: dict[str, Any]) -> None:
    """Alertas basadas en hechos observables; evita umbrales arbitrarios."""
    ...
    if model["billed_period"] > 0:
        signals.append(
            f"La facturación emitida en el período registra una cobranza de {model['collection_rate']:.1f}% al cierre seleccionado."
        )

    coverage = model["coverage_ratio"]
    if coverage is not None and coverage < 1:
        gap = max(model["total_payables_period"] - model["liquidity"], 0.0)
        alerts.append((
            "error",
            f"La liquidez al cierre no cubre las obligaciones del período: cobertura {coverage:.2f}x y brecha de {fmt_money(gap)}.",
        ))
    ...
    if len(model["expiring_contracts"]) > 0:
        alerts.append(("warning", f"Hay {len(model['expiring_contracts'])} contratos con vencimiento dentro del horizonte analizado."))
    ...
    if not billing.empty:
        ...
        if payer.sum() > 0 and not payer.empty:
            ...
            top_payer = str(payer.index[0])
            signals.append(
                f"La principal obra social del período es {top_payer} y representa {concentration:.1f}% de la facturación seleccionada."
            )

    if not alerts:
        st.success("No se detectaron incidencias ejecutivas concretas en los controles disponibles.")
    else:
        for severity, message in alerts:
            getattr(st, severity)(message)

    if signals:
        with st.expander("Señales de contexto", expanded=False):
            for item in signals:
                st.markdown(f"- {item}")
...
def _dg_render_priority_actions(model: dict[str, Any]) -> None:
    ...
    modules_without_data = (
        model["modules"][model["modules"]["Registros"].eq(0)]
        if not model["modules"].empty else pd.DataFrame()
    )
    if not modules_without_data.empty:
        actions.append((
            "Disponibilidad de información",
            f"Revisar {len(modules_without_data)} módulos que no muestran registros en la fuente estándar del tablero.",
        ))
    if not actions:
        actions.append((
            "Consolidación",
...
def _dg_render_overview(
    model: dict[str, Any],
    start: pd.Timestamp,
    end: pd.Timestamp,
    previous_model: dict[str, Any] | None = None,
    previous_period: tuple[pd.Timestamp, pd.Timestamp] | None = None,
    ...
    attention_reasons = []
    if model["overdue_amount"] > 0:
        critical_reasons.append("saldos vencidos")
    if model["coverage_ratio"] is not None and model["coverage_ratio"] < 1:
        critical_reasons.append("cobertura financiera insuficiente")
    if len(model["overdue_tasks"]) > 0:
        attention_reasons.append("tareas vencidas")
    if len(model["expiring_contracts"]) > 0:
        attention_reasons.append("contratos próximos a vencer")
    if model.get("convention_duplicates", 0) > 0:
        attention_reasons.append("duplicados en convenios")
    if model.get("convention_without_value", 0) > 0:
        attention_reasons.append("prácticas de convenio sin valor")

    if critical_reasons:
        label, css_class = "CRÍTICO", "dg-status-bad"
        reason_text = ", ".join(critical_reasons)
    elif attention_reasons:
        label, css_class = "ATENCIÓN", "dg-status-alert"
        reason_text = ", ".join(attention_reasons)
    else:
        label, css_class = "OK", "dg-status-good"
        reason_text = "sin incidencias concretas en los controles disponibles"

    st.markdown(
        f'<span class="dg-pill {css_class}">Estado ejecutivo: {label}</span>',
        unsafe_allow_html=True,
    )
    st.caption(
        f"Estado derivado de datos observables: {reason_text}. "
        "No utiliza ponderaciones ni puntajes inventados."
    )
    ...
    comparison_today = pd.Timestamp(today).normalize() if today is not None else pd.Timestamp.today().normalize()
    _dg_render_period_comparison(
        model,
...
def render_dashboard() -> None:
    ...
    # Defensa en profundidad: el Dashboard valida sesión/permisos por sí mismo,
    # incluso si en el futuro cambia el router principal de app.py.
    _auth_require_login_and_permission()
    _render_vitae_app_shell()
    _auth_render_session_bar()
    ...
    active_dashboard_section = _vitae_lazy_selector(
        "Sección del Dashboard",
        dashboard_sections,
        key="dg_active_section",
    )

    model = _dg_build_model(dfs, start, end, today)
    previous_period = None
    previous_model = None
    if active_dashboard_section == "Resumen ejecutivo":
        previous_period = _dg_previous_period(period_option, start, end)
        if previous_period is not None:
            previous_start, previous_end = previous_period
            previous_cutoff = min(today, previous_end)
            previous_model = _dg_build_model(dfs, previous_start, previous_end, previous_cutoff)
    ...
    elif active_dashboard_section == "Finanzas y cobranza":
        _dg_render_finance(model, start, end, today)
    elif active_dashboard_section == "Operaciones":
        _dg_render_operations(model, start, end)
    elif active_dashboard_section == "Riesgos y vencimientos":
        _dg_render_risks(model, today)
    elif active_dashboard_section == "Mapa de módulos":
        _dg_render_modules(model)
    elif active_dashboard_section == "Director IA":
        _dg_render_ai(dfs, model, start, end, today)

def _facturacion_hoy_argentina() -> pd.Timestamp:
    """Fecha operativa de Argentina para Facturación VM/VMR."""
    try:
        from datetime import datetime
        from zoneinfo import ZoneInfo
        return pd.Timestamp(datetime.now(ZoneInfo("America/Argentina/Salta")).date())
    except Exception:
        return pd.Timestamp(date.today()).normalize()


def _facturacion_parse_fechas(
    valores: pd.Series,
    *,
    permitir_futuras: bool = True,
    hoy: pd.Timestamp | None = None,
) -> pd.Series:
    """Parsea fechas de Facturación sin invertir día/mes.

    Google Sheets puede entregar una fecha argentina ya convertida a ISO con
    día/mes cruzados. Para hechos que necesariamente ya ocurrieron
    (prestación, factura y pago), una fecha posterior a hoy se reinterpreta
    intercambiando día y mes sólo cuando esa alternativa es válida y no futura.
    Si sigue siendo futura, se invalida en lugar de mostrar una fecha imposible.
    Los vencimientos pueden ser futuros y se conservan.
    """
    if valores is None:
        return pd.Series(dtype="datetime64[ns]")

    if isinstance(valores, pd.DataFrame):
        valores = valores.iloc[:, 0]
    original = pd.Series(valores).copy()
    raw = original.astype("string").str.strip().replace("", pd.NA)
    fechas = pd.Series(pd.NaT, index=original.index, dtype="datetime64[ns]")

    # Año primero / ISO. Tolera timestamps con hora o zona al final.
    iso_guion = raw.str.extract(r"^(\d{4}-\d{1,2}-\d{1,2})", expand=False)
    mask = iso_guion.notna()
    if mask.any():
        fechas.loc[mask] = pd.to_datetime(
            iso_guion.loc[mask], format="%Y-%m-%d", errors="coerce"
        )

    iso_barra = raw.str.extract(r"^(\d{4}/\d{1,2}/\d{1,2})", expand=False)
    mask = fechas.isna() & iso_barra.notna()
    if mask.any():
        fechas.loc[mask] = pd.to_datetime(
            iso_barra.loc[mask], format="%Y/%m/%d", errors="coerce"
        )

    # Formato argentino explícito: DD/MM/YYYY o DD-MM-YYYY.
    dmy_barra = raw.str.extract(r"^(\d{1,2}/\d{1,2}/\d{4})", expand=False)
    mask = fechas.isna() & dmy_barra.notna()
    if mask.any():
        fechas.loc[mask] = pd.to_datetime(
            dmy_barra.loc[mask], format="%d/%m/%Y", errors="coerce"
        )

    dmy_guion = raw.str.extract(r"^(\d{1,2}-\d{1,2}-\d{4})", expand=False)
    mask = fechas.isna() & dmy_guion.notna()
    if mask.any():
        fechas.loc[mask] = pd.to_datetime(
            dmy_guion.loc[mask], format="%d-%m-%Y", errors="coerce"
        )

    # Serial numérico de Excel / Google Sheets.
    ...
    # Último fallback, siempre con prioridad día/mes.
    faltantes = fechas.isna() & raw.notna()
    if faltantes.any():
        try:
            fallback = pd.to_datetime(
                raw.loc[faltantes], errors="coerce", dayfirst=True, format="mixed"
            )
        except (TypeError, ValueError):
            fallback = pd.to_datetime(
                raw.loc[faltantes], errors="coerce", dayfirst=True
            )
        fechas.loc[faltantes] = fallback

    fechas = pd.to_datetime(fechas, errors="coerce").dt.normalize().copy(deep=True)

    if not permitir_futuras:
        corte = pd.Timestamp(hoy if hoy is not None else _facturacion_hoy_argentina()).normalize()
        futuras = fechas.notna() & fechas.gt(corte)

        # Corrige únicamente la ambigüedad real día/mes.
        for fila in fechas.index[futuras]:
            valor = fechas.at[fila]
            if pd.isna(valor):
                continue
            dia = int(valor.day)
            mes = int(valor.month)
            if dia <= 12 and mes <= 12:
                try:
                    alternativa = pd.Timestamp(
                        year=int(valor.year), month=dia, day=mes
                    ).normalize()
                except ValueError:
                    alternativa = pd.NaT
                if pd.notna(alternativa) and alternativa <= corte:
                    fechas.at[fila] = alternativa

        # Una prestación/factura/pago no puede quedar fechada en el futuro.
        # Si no existe una reinterpretación inequívoca, se deja sin fecha para
        # que no contamine filtros, métricas ni el detalle ejecutivo.
        fechas = fechas.mask(fechas.gt(corte), pd.NaT)
...
def format_facturacion_table(df: pd.DataFrame, labels: Dict[str, str]) -> pd.DataFrame:
    ...
    if "mes" in show.columns:
        # Misma lectura que "Editar tabla"; no reinterpretar día/mes aquí.
        show["mes"] = parse_mes(show["mes"]).dt.strftime("%d/%m/%Y").fillna("")
    show = show.drop(
        columns=[
            "id",
            "created_at",
    ...
    for col in ["fecha_factura", "vencimiento", "fecha_pago"]:
        if col in show.columns:
            show[col] = _facturacion_parse_fechas(
                show[col],
                permitir_futuras=(col == "vencimiento"),
    ...
    return show
def _vitae_column_preferences_key(table: str) -> str:
    return f"_vitae_column_preferences::{str(table).strip()}"


def _vitae_apply_export_preferences(
    df: pd.DataFrame,
    table: str,
    labels: Dict[str, str],
) -> pd.DataFrame:
    """Aplica preferencias visuales sin cambiar columnas ni estructura fuente."""
    if df is None or not isinstance(df, pd.DataFrame):
        return pd.DataFrame()
    preferences = st.session_state.get(_vitae_column_preferences_key(table), {})
    selected = preferences.get("selected", []) if isinstance(preferences, dict) else []
    aliases = preferences.get("aliases", {}) if isinstance(preferences, dict) else {}

    base = df.copy()
    if selected:
        selected_existing = [col for col in selected if col in base.columns]
        if selected_existing:
            base = base[selected_existing].copy()

    export_labels = dict(labels or {})
    if isinstance(aliases, dict):
        for source_col, alias in aliases.items():
            alias_text = str(alias or "").strip()
            if source_col in base.columns and alias_text:
                export_labels[source_col] = alias_text
    return format_facturacion_table(base, export_labels)


def _render_column_preferences(
    df: pd.DataFrame,
    table: str,
    labels: Dict[str, str],
) -> None:
    """Gestor real de columnas para vista/exportación, sin alterar Google Sheets."""
    st.subheader("Configuración de columnas")
    st.caption(
        "Elegí qué campos incluir y cómo nombrarlos al exportar. "
        "Estas preferencias no modifican nombres, fórmulas ni estructura de Google Sheets."
    )
    if df is None or not isinstance(df, pd.DataFrame) or df.empty:
        st.info("No hay columnas disponibles hasta que el módulo tenga registros.")
        return

    pref_key = _vitae_column_preferences_key(table)
    saved = st.session_state.get(pref_key, {})
    saved_selected = set(saved.get("selected", list(df.columns))) if isinstance(saved, dict) else set(df.columns)
    saved_aliases = saved.get("aliases", {}) if isinstance(saved, dict) else {}

    rows = []
    total = max(len(df), 1)
    for position, col in enumerate(df.columns, start=1):
        non_null = int(df[col].notna().sum())
        completeness = (non_null / total) * 100.0
        rows.append({
            "Orden": position,
            "Campo origen": col,
            "Etiqueta exportación": str(saved_aliases.get(col) or labels.get(col, col.replace("_", " ").title())),
            "Incluir": col in saved_selected,
            "Tipo": str(df[col].dtype),
            "Completitud %": round(completeness, 1),
        })
    prefs_df = pd.DataFrame(rows)

    edited = st.data_editor(
        prefs_df,
        hide_index=True,
        use_container_width=True,
        disabled=["Campo origen", "Tipo", "Completitud %"],
        column_config={
            "Orden": st.column_config.NumberColumn("Orden", min_value=1, step=1),
            "Campo origen": st.column_config.TextColumn("Campo origen"),
            "Etiqueta exportación": st.column_config.TextColumn("Etiqueta exportación", max_chars=80),
            "Incluir": st.column_config.CheckboxColumn("Incluir"),
            "Tipo": st.column_config.TextColumn("Tipo"),
            "Completitud %": st.column_config.ProgressColumn(
                "Completitud", min_value=0.0, max_value=100.0, format="%.1f%%"
            ),
        },
        key=f"column_preferences_editor_{table}",
    )

    c_save, c_reset, c_quality = st.columns([1.3, 1.1, 2.4])
    with c_save:
        if st.button("Guardar preferencia", key=f"column_preferences_save_{table}", type="primary", use_container_width=True):
            clean = edited.copy()
            clean["Orden"] = pd.to_numeric(clean["Orden"], errors="coerce").fillna(9999)
            clean = clean.sort_values(["Orden", "Campo origen"], kind="stable")
            included = clean[clean["Incluir"].fillna(False).astype(bool)]
            selected = included["Campo origen"].astype(str).tolist()
            aliases = {
                str(row["Campo origen"]): str(row["Etiqueta exportación"] or "").strip()
                for _, row in included.iterrows()
            }
            normalized_aliases = [value.casefold() for value in aliases.values() if value]
            if not selected:
                st.warning("Seleccioná al menos una columna para exportar.")
            elif len(normalized_aliases) != len(set(normalized_aliases)):
                st.warning("Las etiquetas de exportación incluidas deben ser únicas.")
            else:
                st.session_state[pref_key] = {"selected": selected, "aliases": aliases}
                st.success("Preferencias de columnas guardadas para esta sesión.")
    with c_reset:
        if st.button("Restablecer", key=f"column_preferences_reset_{table}", use_container_width=True):
            st.session_state.pop(pref_key, None)
            st.session_state.pop(f"column_preferences_editor_{table}", None)
            st.rerun()
    with c_quality:
        duplicate_rows = int(df.duplicated().sum()) if len(df.columns) else 0
        cells = max(int(df.shape[0] * df.shape[1]), 1)
        missing_cells = int(df.isna().sum().sum())
        quality = max(0.0, 100.0 - (missing_cells / cells * 100.0))
        st.caption(
            f"Calidad estructural · {len(df):,} filas · {len(df.columns)} columnas · "
            f"{duplicate_rows:,} filas duplicadas · completitud {quality:.1f}%"
        )
...
def render_analisis_anual_2026(df: pd.DataFrame):
    ...
    # Facturación VM/VMR: misma interpretación que la tabla editable.
    data["mes"] = parse_mes(data["mes"])
        ...
        if not str(pregunta or "").strip():
            ...
            with st.spinner("Analizando información…"):
                try:
                    ...
                    _vitae_render_error(
                        "La IA no pudo responder. El análisis local y la tabla siguen disponibles.",
                        exc,
                        context="ia:tabla_generica",
                    )
...
def render_agenda_quirofano_pro(
    ...
    def badge_estado(estado: str) -> str:
        ...
        estilos = {
            ...
            "Consulta": ("#F1D0DE", "#922B58"),

            "Pendiente de autorización": ("#FFF4E5", "#A15C00"),

            "Autorizado": ("#ECD4DF", "#681E3E"),

            "Programado": ("#E8EAED", "#3C4043"),

            "Confirmado": ("#ECD4DF", "#581A35"),

            "En quirófano": ("#FCE8E6", "#B31412"),

            "Realizado": ("#F1D0DE", "#591A35"),
    ...
    # Las pestañas anteriores eran sólo visuales: su contenido se renderizaba
    # completo en cada rerun. Esta navegación ejecuta una única vista real.
    if "estado" not in df.columns:
        df["estado"] = "Programado"
    hoy = pd.Timestamp.today().date()
    agenda_section = _vitae_lazy_selector(
        "Vista de agenda",
        ["Flujo de pacientes", "Agenda del día", "Indicadores",
         "Próximas cirugías", "Ocupación", "Alertas"],
        key="agenda_quirofano_active_section",
    )
    # ==========================================================
    ...
    if agenda_section == "Flujo de pacientes":
        st.markdown("---")
    ...
    if agenda_section == "Agenda del día":
        st.markdown("---")
    ...
    if agenda_section == "Próximas cirugías":
        st.markdown("---")
...
# =========================================================
# AGENDA QUIRÓFANO X1000 · CENTRO OPERATIVO VITAE
# =========================================================
def render_agenda_quirofano_ultra_pro(
    df_original: pd.DataFrame,
    guardar_callback=None,
) -> pd.DataFrame:
    """Agenda quirúrgica operativa, visual y editable sobre el mismo Google Sheet.

    La vista trabaja por posición de columna/fila para tolerar encabezados
    repetidos, columnas numéricas y esquemas históricos. Ningún dato se borra:
    sólo se escriben los campos que el usuario modifica explícitamente.
    """
    ...
    ESTADOS_PROGRAMACION = {
        "Autorizado", "Programado", "Confirmado", "En quirófano", "Reprogramado"
    }
    ETAPAS = [
        ("Consulta", {"Consulta"}, "#3B82F6"),
        ("Autorización", {"Pendiente de autorización", "Autorizado"}, "#F59E0B"),
        ("Programación", {"Programado", "Confirmado", "Reprogramado"}, "#7C3AED"),
        ("En quirófano", {"En quirófano"}, "#DC2626"),
        ("Finalizado", {"Realizado"}, "#16A34A"),
        ("Suspendido", {"Suspendido", "Cancelado"}, "#475569"),
    ]

    ALIASES = {
        "fecha": ["fecha", "fecha_cirugia", "fecha_procedimiento", "fecha_turno", "dia"],
        "hora_inicio": ["hora_inicio", "hora", "inicio", "horario", "hora_desde"],
        "hora_fin": ["hora_fin", "fin", "hora_hasta"],
        "duracion_min": ["duracion_min", "duracion", "duracion_minutos", "minutos"],
        "sala": ["sala", "quirofano", "quirofano_sala", "sala_quirofano"],
        "paciente": ["paciente", "nombre_paciente", "apellido_y_nombre", "apellido_nombre", "afiliado"],
        "procedimiento": ["procedimiento", "practica", "cirugia", "prestacion"],
        "medico": ["medico", "medico_responsable", "profesional", "cirujano"],
        "estado": ["estado", "estado_agenda", "situacion"],
        "anestesista": ["anestesista", "anestesiologo", "anestesia"],
        "obra_social": ["obra_social", "obra_social_prepaga", "cobertura", "financiador"],
        "numero_afiliado": ["numero_afiliado", "n_afiliado", "nro_afiliado", "numero_de_afiliado"],
        "autorizacion": ["autorizacion", "n_autorizacion", "numero_autorizacion", "autorizado"],
        "telefono": ["telefono", "celular", "contacto", "whatsapp"],
        "observaciones": ["observaciones", "observacion", "notas", "nota"],
        "prioridad": ["prioridad", "urgencia"],
        "confirmado": ["confirmado", "confirmacion", "paciente_confirmado"],
        "ayuno": ["ayuno", "ayuno_confirmado"],
        "consentimiento": ["consentimiento", "consentimiento_ok"],
        "prequirurgico": ["prequirurgico", "prequirurgico_ok", "estudios_prequirurgicos"],
        "material": ["material", "material_ok", "material_especial"],
        "created_at": ["created_at", "creado", "fecha_creacion"],
        "updated_at": ["updated_at", "actualizado", "fecha_actualizacion"],
    }
    ...
    def _state(value: Any) -> str:
        ...
        return mapping.get(key, raw if raw else "Consulta")

    def _yes(value: Any) -> bool:
        return _norm(_clean(value)) in {"si", "s", "yes", "true", "1", "ok", "confirmado", "hecho", "completo"}

    def _date(value: Any):
        text = _clean(value)
        if not text:
            return pd.NaT
        # Primero formatos no ambiguos comunes de la planilla.
        for fmt in ("%d/%m/%Y", "%d-%m-%Y", "%Y-%m-%d", "%Y/%m/%d"):
            try:
                return pd.Timestamp(_datetime.strptime(text, fmt))
            except Exception:
                pass
        try:
    ...
    def _time_text(value: Any, default: str = "") -> str:
        ...
        if not text:
            return default
        # Excel/Sheets puede entregar una fracción de día.
        try:
            number = float(text.replace(",", "."))
            if 0 <= number < 1:
                mins = int(round(number * 24 * 60)) % (24 * 60)
                return f"{mins // 60:02d}:{mins % 60:02d}"
            if number.is_integer() and 0 <= number <= 23:
                return f"{int(number):02d}:00"
        except Exception:
            pass
        match = re.search(r"(?<!\d)([01]?\d|2[0-3])[:.]([0-5]\d)(?!\d)", text)
        if match:
            return f"{int(match.group(1)):02d}:{int(match.group(2)):02d}"
        match = re.fullmatch(r"\s*([01]?\d|2[0-3])\s*", text)
        if match:
    ...
    def _fmt_date(ts) -> str:
        return ts.strftime("%d/%m/%Y") if pd.notna(ts) else "Sin fecha"

    def _short(value: Any, n: int = 46) -> str:
        text = _clean(value)
        return text if len(text) <= n else text[: n - 1] + "…"

    def _html_text(value: Any) -> str:
        return _html.escape(_clean(value))

    original = pd.DataFrame() if df_original is None else df_original.copy()
    original = original.reset_index(drop=True)

    # Mapa por POSICIÓN: funciona incluso con encabezados repetidos.
    norm_columns = [_norm(col) for col in original.columns]
    field_pos: dict[str, int | None] = {}
    for field, aliases in ALIASES.items():
        options = {_norm(v) for v in aliases + [field]}
        field_pos[field] = next((i for i, col in enumerate(norm_columns) if col in options), None)

    def _ensure_field(field: str) -> int:
        pos = field_pos.get(field)
        if pos is not None:
            return int(pos)
        original[field] = ""
        pos = len(original.columns) - 1
        field_pos[field] = pos
        norm_columns.append(_norm(field))
        return pos

    def _series(field: str, default: Any = "") -> pd.Series:
        pos = field_pos.get(field)
        if pos is None:
            return pd.Series([default] * len(original), index=original.index, dtype="object")
        return original.iloc[:, int(pos)]

    def _set_cell(row_pos: int, field: str, value: Any) -> None:
        col_pos = _ensure_field(field)
        # Pandas 2.x: una columna numérica no acepta texto sin conversión explícita.
        safe = original.iloc[:, col_pos].astype("object")
        original.isetitem(col_pos, safe)
        original.iat[int(row_pos), col_pos] = value

    def _touch(row_pos: int) -> None:
        if field_pos.get("updated_at") is not None:
            _set_cell(row_pos, "updated_at", pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S"))

    def _save(message: str = "Cambios guardados en Google Sheets.") -> bool:
        if guardar_callback is None:
            st.warning("La agenda está operativa en pantalla, pero no hay función de guardado configurada.")
            return False
        try:
            guardar_callback(original.copy())
            _vitae_invalidate_data_cache()
            st.success(message)
            return True
        except Exception as exc:
            _vitae_render_error(
                "No se pudo guardar la Agenda de Quirófano en Google Sheets.",
                exc,
                context="agenda_quirofano_x1000_guardado",
            )
            return False

    def _save_updates(row_pos: int, updates: dict[str, Any], message: str = "Paciente actualizado.") -> None:
        for field, value in updates.items():
            _set_cell(row_pos, field, value)
        _touch(row_pos)
        if _save(message):
            st.rerun()

    # DataFrame canónico de trabajo, sin modificar la estructura fuente.
    work = pd.DataFrame({"_row_pos": range(len(original))})
    for field in ALIASES:
        work[field] = _series(field).astype("object")
    work["paciente"] = work["paciente"].map(_clean)
    work["procedimiento"] = work["procedimiento"].map(_clean)
    work["medico"] = work["medico"].map(_clean)
    work["anestesista"] = work["anestesista"].map(_clean)
    work["sala"] = work["sala"].map(_clean)
    work["obra_social"] = work["obra_social"].map(_clean)
    work["numero_afiliado"] = work["numero_afiliado"].map(_clean)
    work["autorizacion"] = work["autorizacion"].map(_clean)
    work["telefono"] = work["telefono"].map(_clean)
    work["observaciones"] = work["observaciones"].map(_clean)
    work["estado"] = work["estado"].map(_state)
    work["prioridad"] = work["prioridad"].map(lambda v: _clean(v) or "Normal")
    work["_fecha"] = pd.to_datetime(work["fecha"].map(_date), errors="coerce")
    work["_hora_inicio"] = work["hora_inicio"].map(lambda v: _time_text(v, ""))
    work["_hora_fin"] = work["hora_fin"].map(lambda v: _time_text(v, ""))

    def _duration(row) -> int:
        start = _mins(row["_hora_inicio"])
        end = _mins(row["_hora_fin"])
        if start is not None and end is not None and end > start:
            return int(end - start)
        try:
            raw = float(str(row.get("duracion_min", "")).replace(",", "."))
            if raw > 0:
                return int(raw)
        except Exception:
            pass
    ...
    work["_duracion"] = work.apply(_duration, axis=1) if len(work) else pd.Series(dtype="int64")
    work["_start_min"] = work["_hora_inicio"].map(_mins)
    work["_end_min"] = work.apply(
        lambda r: (_mins(r["_hora_fin"]) if _mins(r["_hora_fin"]) is not None else (
            (r["_start_min"] + r["_duracion"]) if pd.notna(r["_start_min"]) else None
        )), axis=1
    ) if len(work) else pd.Series(dtype="object")
    work["_confirmado"] = work["confirmado"].map(_yes)
    ...
    # ---------------------------
    # Conflictos de sala/horario
    # ---------------------------
    conflict_rows: set[int] = set()
    conflict_pairs: list[tuple[int, int]] = []
    sched = work[
        work["estado"].isin(ESTADOS_PROGRAMACION)
        & work["_fecha"].notna()
        & work["sala"].ne("")
        & work["_start_min"].notna()
        & work["_end_min"].notna()
    ].copy()
    if not sched.empty:
        sched["_day"] = sched["_fecha"].dt.date
        for _, group in sched.groupby(["_day", "sala"], dropna=False):
            group = group.sort_values(["_start_min", "_end_min", "_row_pos"])
            active: list[tuple[int, int, int]] = []  # (end, start, row)
            for _, row in group.iterrows():
                start = int(row["_start_min"])
                end = int(row["_end_min"])
                pos = int(row["_row_pos"])
                active = [item for item in active if item[0] > start]
                for prev_end, prev_start, prev_pos in active:
                    if start < prev_end and end > prev_start:
                        conflict_rows.update({pos, prev_pos})
                        conflict_pairs.append((prev_pos, pos))
                active.append((end, start, pos))

    # ---------------------------
    # Alertas clínicas/operativas
    # ---------------------------
    ...
    for _, row in work.iterrows():
        pos = int(row["_row_pos"])
        state = row["estado"]
        patient = row["paciente"] or "Paciente sin nombre"
        ts = row["_fecha"]
        day = ts.normalize() if pd.notna(ts) else pd.NaT

        def _alert(level: str, kind: str, detail: str):
            alerts.append({
                "Nivel": level,
                "Tipo": kind,
                "Paciente": patient,
        ...
        if pos in conflict_set:
            _alert("Crítica", "Conflicto de quirófano", "Se superpone con otro procedimiento en la misma sala.")
        if state not in ESTADOS_CERRADOS and state != "Consulta":
            missing = []
            for field, label in (("paciente", "paciente"), ("fecha", "fecha"), ("procedimiento", "procedimiento"), ("medico", "médico"), ("sala", "quirófano")):
                value = row[field]
                if field == "fecha":
                    if pd.isna(ts):
                    ...
                    missing.append(label)
            if missing:
                _alert("Alta", "Datos incompletos", "Falta completar: " + ", ".join(missing) + ".")
        if state == "Pendiente de autorización" and pd.notna(day) and day <= today + pd.Timedelta(days=3):
            _alert("Alta", "Autorización", "La cirugía está próxima y la autorización sigue pendiente.")
        if state in {"Programado", "Autorizado", "Reprogramado"} and pd.notna(day) and today <= day <= tomorrow:
            _alert("Media", "Confirmación", "Turno dentro de 24–48 h todavía no marcado como Confirmado.")
        if state not in ESTADOS_CERRADOS and state != "Consulta" and pd.notna(day) and day < today:
            _alert("Alta", "Seguimiento vencido", "La fecha ya pasó y el circuito continúa abierto.")
        if row["_start_min"] is not None and row["_end_min"] is not None:
            try:
                if int(row["_end_min"]) <= int(row["_start_min"]):
                    _alert("Alta", "Horario inválido", "La hora de fin no es posterior a la hora de inicio.")
            except Exception:
                pass
    ...
    st.markdown(
        """
        <style>
        .qx-hero{padding:20px 22px;border:1px solid rgba(177,86,126,.22);border-radius:20px;
          background:linear-gradient(135deg,rgba(177,86,126,.12),rgba(255,255,255,.60));margin:4px 0 14px}
        .qx-kicker{font-size:12px;font-weight:800;letter-spacing:.11em;text-transform:uppercase;color:#8F3F66;margin-bottom:5px}
        .qx-title{font-size:30px;font-weight:900;letter-spacing:-.03em;line-height:1.05;color:#2F2530}
        .qx-sub{font-size:14px;color:#6F6470;margin-top:7px;line-height:1.45}
        .qx-stage{padding:9px 10px;border-radius:12px;color:white;text-align:center;font-weight:850;font-size:14px;margin:4px 0 10px}
        .qx-card{border:1px solid rgba(90,70,85,.15);border-radius:15px;padding:12px 13px;margin:0 0 10px;background:rgba(255,255,255,.78);
          box-shadow:0 4px 14px rgba(68,45,58,.05)}
        .qx-card.critical{border-color:rgba(220,38,38,.45);box-shadow:0 0 0 2px rgba(220,38,38,.06)}
        .qx-patient{font-size:15px;font-weight:900;color:#272129;line-height:1.25;margin-bottom:5px}
        .qx-meta{font-size:12.5px;color:#665E67;line-height:1.5}
        .qx-pill{display:inline-block;padding:3px 8px;border-radius:999px;font-size:11px;font-weight:800;margin:4px 4px 0 0;background:#F4E8EE;color:#7A3152}
        .qx-pill.warn{background:#FFF1D6;color:#9A5B00}.qx-pill.danger{background:#FEE5E5;color:#A31919}.qx-pill.ok{background:#E4F7E9;color:#147A35}
        .qx-empty{border:1px dashed rgba(90,70,85,.20);border-radius:13px;padding:18px;text-align:center;color:#847985;font-size:13px;background:rgba(255,255,255,.35)}
        .qx-dayhead{font-weight:850;font-size:12px;text-align:center;color:#756B76;padding:4px}
        .qx-day{min-height:88px;border:1px solid rgba(90,70,85,.14);border-radius:12px;padding:8px;background:rgba(255,255,255,.62);margin-bottom:5px}
        .qx-day.today{border-color:#B1567E;box-shadow:0 0 0 2px rgba(177,86,126,.08)}
        .qx-daynum{font-size:13px;font-weight:900;color:#403741}.qx-count{font-size:20px;font-weight:900;color:#8F3F66;margin-top:8px}
        .qx-mini{font-size:11px;color:#7B717C}.qx-alert{border-left:4px solid #D97706;padding:10px 12px;background:rgba(255,255,255,.66);border-radius:10px;margin-bottom:8px}
        div[data-testid="stMetric"]{background:rgba(255,255,255,.58);border:1px solid rgba(90,70,85,.10);padding:10px 12px;border-radius:14px}
        </style>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        """
        <div class="qx-hero">
          <div class="qx-kicker">VITAE · Centro operativo quirúrgico</div>
          <div class="qx-title">Agenda de Quirófano X1000</div>
          <div class="qx-sub">Un solo tablero para programar, autorizar, confirmar, detectar conflictos, operar y cerrar cada paciente sin perder el control del circuito.</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    # Filtros globales
    # ---------------------------
    f1, f2, f3, f4, f5 = st.columns([2.2, 1.35, 1.4, 1.4, .75])
    with f1:
        search = st.text_input(
            "Buscar",
            placeholder="Paciente, médico, práctica, cobertura…",
            key="qx_search",
        ).strip().lower()
    with f2:
        scope = st.selectbox(
            "Alcance",
            ["Todo", "Activos", "Hoy", "Próximos 7 días", "Este mes"],
            key="qx_scope",
        )
    salas = sorted(v for v in work["sala"].dropna().astype(str).unique() if v.strip())
    medicos = sorted(v for v in work["medico"].dropna().astype(str).unique() if v.strip())
    with f3:
        selected_rooms = st.multiselect("Quirófano", salas, key="qx_rooms")
    with f4:
        selected_docs = st.multiselect("Médico", medicos, key="qx_docs")
    with f5:
        st.write("")
        if st.button("↻", key="qx_refresh", help="Actualizar datos", use_container_width=True):
            _vitae_invalidate_data_cache()
            st.rerun()
    ...
    if search:
        haystack = (
            filtered[["paciente", "procedimiento", "medico", "obra_social", "sala", "observaciones"]]
            .fillna("").astype(str).agg(" ".join, axis=1).str.lower()
        )
        filtered = filtered[haystack.str.contains(re.escape(search), na=False)]
    if selected_rooms:
        filtered = filtered[filtered["sala"].isin(selected_rooms)]
    if selected_docs:
        filtered = filtered[filtered["medico"].isin(selected_docs)]
    if scope == "Activos":
        filtered = filtered[~filtered["estado"].isin(ESTADOS_CERRADOS)]
    elif scope == "Hoy":
        filtered = filtered[filtered["_fecha"].dt.normalize().eq(today)]
    elif scope == "Próximos 7 días":
        filtered = filtered[
            filtered["_fecha"].notna()
            & filtered["_fecha"].dt.normalize().between(today, today + pd.Timedelta(days=7))
        ]
    elif scope == "Este mes":
        filtered = filtered[
            filtered["_fecha"].notna()
            & filtered["_fecha"].between(month_start, month_end)
        ]
    ...
    # ---------------------------
    today_cases = work[
        work["_fecha"].dt.normalize().eq(today)
        & ~work["estado"].isin({"Suspendido", "Cancelado"})
    ]
    next7 = work[
        work["_fecha"].notna()
        & work["_fecha"].dt.normalize().between(today, today + pd.Timedelta(days=7))
        & ~work["estado"].isin({"Suspendido", "Cancelado"})
    ]
    pending_auth = work["estado"].eq("Pendiente de autorización").sum()
    unconfirmed = work[
        work["estado"].isin({"Autorizado", "Programado", "Reprogramado"})
        & work["_fecha"].notna()
        & work["_fecha"].dt.normalize().between(today, tomorrow)
    ]
    done_month = work[
        work["estado"].eq("Realizado")
        & work["_fecha"].notna()
        & work["_fecha"].between(month_start, month_end)
    ]
    k1, k2, k3, k4, k5, k6 = st.columns(6)
    k1.metric("Hoy", int(len(today_cases)), help="Procedimientos del día no suspendidos/cancelados")
    k2.metric("Próx. 7 días", int(len(next7)))
    k3.metric("Autorización pendiente", int(pending_auth))
    k4.metric("Sin confirmar 48 h", int(len(unconfirmed)))
    k5.metric("Conflictos", int(len(conflict_rows)))
    k6.metric("Realizados mes", int(len(done_month)))

    if conflict_rows:
        st.error(f"⚠️ Hay {len(conflict_rows)} pacientes involucrados en superposiciones de quirófano. Revisalos antes de confirmar la agenda.")
    elif not alerts_df.empty and (alerts_df["Nivel"] == "Alta").any():
        st.warning(f"Hay {(alerts_df['Nivel'] == 'Alta').sum()} alertas de prioridad alta pendientes de revisión.")
    ...
    section = _vitae_lazy_selector("Agenda", sections, key="qx_section")

    def _row_alert_tags(pos: int) -> list[str]:
        if alerts_df.empty:
            return []
        return alerts_df.loc[alerts_df["_row_pos"].eq(pos), "Tipo"].drop_duplicates().tolist()

    def _render_patient_card(row, key_prefix: str, detailed: bool = True):
        pos = int(row["_row_pos"])
        state = row["estado"]
        critical = pos in conflict_set
        patient = _html_text(row["paciente"] or "Paciente sin nombre")
        procedure = _html_text(_short(row["procedimiento"] or "Procedimiento sin definir", 58))
        doctor = _html_text(_short(row["medico"] or "Médico sin definir", 38))
        room = _html_text(row["sala"] or "Sin quirófano")
        date_txt = _fmt_date(row["_fecha"])
        time_txt = row["_hora_inicio"] or "--:--"
        end_txt = row["_hora_fin"] or ""
        alerts_here = _row_alert_tags(pos)
        if state in {"Realizado", "Confirmado"}:
            state_class = "ok"
        elif state in {"En quirófano", "Suspendido", "Cancelado"}:
            state_class = "danger"
        elif state in {"Pendiente de autorización", "Reprogramado"}:
            state_class = "warn"
        else:
            state_class = ""
        tags = f'<span class="qx-pill {state_class}">{_html.escape(state)}</span>'
        if row["prioridad"] in {"Alta", "Urgente"}:
            tags += f'<span class="qx-pill danger">{_html.escape(row["prioridad"])}</span>'
        if alerts_here:
            tags += f'<span class="qx-pill danger">{len(alerts_here)} alerta(s)</span>'
        if row["obra_social"]:
            tags += f'<span class="qx-pill">{_html.escape(_short(row["obra_social"], 22))}</span>'
        st.markdown(
            f"""
            <div class="qx-card {'critical' if critical else ''}">
              <div class="qx-patient">{patient}</div>
              <div class="qx-meta"><b>{_html.escape(date_txt)}</b> · {time_txt}{('–' + end_txt) if end_txt else ''}<br>{procedure}<br>👨‍⚕️ {doctor} · 🏥 {room}</div>
              <div>{tags}</div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        if detailed:
            exp_label = "⚙️ Gestionar" + (" · CONFLICTO" if critical else "")
            with st.expander(exp_label, expanded=False):
                if alerts_here:
                    for tag in alerts_here:
                        st.caption("⚠️ " + tag)
                ...
                with c1:
                    new_state = st.selectbox(
                        "Estado",
                        ESTADOS,
                        index=ESTADOS.index(state) if state in ESTADOS else 0,
                        key=f"{key_prefix}_state_{pos}",
                    )
                with c2:
                    st.write("")
                    st.write("")
                    if st.button("Guardar estado", key=f"{key_prefix}_save_state_{pos}", use_container_width=True):
                        _save_updates(pos, {"estado": new_state}, "Estado actualizado.")
                a, b = st.columns(2)
                if state not in {"En quirófano", "Realizado", "Suspendido", "Cancelado"}:
                    if a.button("🔴 Ingresó a quirófano", key=f"{key_prefix}_or_{pos}", use_container_width=True):
                        _save_updates(pos, {"estado": "En quirófano"}, "Paciente marcado en quirófano.")
                if state == "En quirófano":
                    if b.button("✅ Finalizar", key=f"{key_prefix}_done_{pos}", use_container_width=True):
                        _save_updates(pos, {"estado": "Realizado"}, "Procedimiento finalizado.")
                if row["telefono"]:
                    st.caption(f"📞 {row['telefono']}")
                if row["observaciones"]:
                    st.caption("📝 " + row["observaciones"])

    # =====================================================
    # OPERACIÓN DEL DÍA
    # =====================================================
    if section == "🏥 Operación":
        st.markdown("### Centro de mando del día")
        d1, d2, d3 = st.columns([1.2, 1.4, 1.4])
        with d1:
            op_date = st.date_input("Día operativo", value=today.date(), key="qx_operation_date")
        day_ts = pd.Timestamp(op_date)
        day_df = work[
            work["_fecha"].dt.normalize().eq(day_ts)
            & ~work["estado"].isin({"Cancelado"})
        ].copy()
        day_rooms = sorted(v for v in day_df["sala"].dropna().astype(str).unique() if v.strip())
        with d2:
            op_rooms = st.multiselect("Salas del día", day_rooms, default=day_rooms, key="qx_operation_rooms")
        if op_rooms:
            day_df = day_df[day_df["sala"].isin(op_rooms)]
        with d3:
            op_states = st.multiselect("Estados visibles", ESTADOS, default=[], key="qx_operation_states", placeholder="Todos")
        if op_states:
            day_df = day_df[day_df["estado"].isin(op_states)]

        planned_minutes = int(day_df.loc[~day_df["estado"].isin({"Suspendido", "Cancelado"}), "_duracion"].sum()) if len(day_df) else 0
        rooms_count = max(len({r for r in day_df["sala"] if r}), 1)
        theoretical = rooms_count * 12 * 60  # ventana operativa de referencia 08–20, sólo indicador
        occupancy = min(planned_minutes / theoretical, 1.0) if theoretical else 0
        op_conflicts = len(set(day_df["_row_pos"]) & conflict_set)
        op1, op2, op3, op4 = st.columns(4)
        op1.metric("Pacientes", len(day_df))
        op2.metric("Minutos programados", planned_minutes)
        op3.metric("Carga estimada", f"{occupancy:.0%}", help="Referencia: 12 h por sala; no reemplaza la disponibilidad real configurada.")
        op4.metric("En conflicto", op_conflicts)

        if day_df.empty:
            st.info("No hay procedimientos cargados para ese día con los filtros actuales.")
        else:
            day_df = day_df.sort_values(["sala", "_start_min", "paciente"], na_position="last")
            grouped_rooms = list(day_df.groupby(day_df["sala"].replace("", "Sin quirófano"), dropna=False))
            for room_name, room_df in grouped_rooms:
                st.markdown(f"#### 🏥 {room_name} · {len(room_df)} paciente(s)")
                for _, row in room_df.iterrows():
                    _render_patient_card(row, "op", detailed=True)

        with st.expander("📌 Resumen compacto para reunión / pase", expanded=False):
            if day_df.empty:
                st.caption("Sin datos para mostrar.")
            else:
                compact = pd.DataFrame({
                    "Hora": day_df["_hora_inicio"],
                    "Paciente": day_df["paciente"],
                    "Procedimiento": day_df["procedimiento"],
                    "Médico": day_df["medico"],
                    "Quirófano": day_df["sala"],
                    "Estado": day_df["estado"],
                    "Alerta": day_df["_row_pos"].map(lambda p: " · ".join(_row_alert_tags(int(p)))),
                })
                st.dataframe(compact, use_container_width=True, hide_index=True)

    # =====================================================
    # FLUJO VISUAL 3 + 3 (no seis columnas angostas)
    # =====================================================
    elif section == "🧭 Flujo de pacientes":
        st.markdown("### Gestión visual del circuito quirúrgico")
        st.caption("Seis etapas, distribuidas 3 + 3 para que cada paciente sea legible y gestionable también en iPad. Los filtros superiores controlan todo el tablero.")
        sort_mode = st.selectbox(
            "Orden de tarjetas",
            ["Fecha más próxima", "Prioridad", "Paciente"],
            key="qx_flow_sort",
            label_visibility="collapsed",
        )

        flow_df = filtered.copy()
        priority_rank = {"Urgente": 0, "Alta": 1, "Normal": 2}
        flow_df["_priority_rank"] = flow_df["prioridad"].map(priority_rank).fillna(3)
        if sort_mode == "Fecha más próxima":
            flow_df = flow_df.sort_values(["_fecha", "_start_min", "paciente"], na_position="last")
        elif sort_mode == "Prioridad":
            flow_df = flow_df.sort_values(["_priority_rank", "_fecha", "_start_min"], na_position="last")
        else:
            flow_df = flow_df.sort_values("paciente")

        for start_idx in (0, 3):
            cols = st.columns(3)
            for offset, (label, states, color) in enumerate(ETAPAS[start_idx:start_idx + 3]):
                with cols[offset]:
                    lane = flow_df[flow_df["estado"].isin(states)].copy()
                    st.markdown(f'<div class="qx-stage" style="background:{color}">{_html.escape(label)} · {len(lane)}</div>', unsafe_allow_html=True)
                    if lane.empty:
                        st.markdown('<div class="qx-empty">Sin pacientes</div>', unsafe_allow_html=True)
                    else:
                        limit = 30
                        for _, row in lane.head(limit).iterrows():
                            _render_patient_card(row, f"flow_{start_idx}_{offset}", detailed=True)
                        if len(lane) > limit:
                            st.caption(f"Mostrando {limit} de {len(lane)}. Usá los filtros para acotar el tablero.")

    # =====================================================
    # CALENDARIO MENSUAL + DETALLE DEL DÍA
    # =====================================================
    elif section == "📅 Calendario":
        st.markdown("### Calendario quirúrgico")
        cm1, cm2 = st.columns([1, 2.3])
        with cm1:
            anchor = st.date_input("Mes", value=today.date(), key="qx_calendar_anchor")
        anchor_ts = pd.Timestamp(anchor)
        year, month = anchor_ts.year, anchor_ts.month
        month_name = ["", "Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"][month]
        with cm2:
            st.markdown(f"#### {month_name} {year}")
            st.caption("El número grande es la cantidad de procedimientos no cancelados. Debajo se indica cuántos están confirmados.")

        month_df = work[
            work["_fecha"].notna()
            & work["_fecha"].dt.year.eq(year)
            & work["_fecha"].dt.month.eq(month)
            & ~work["estado"].eq("Cancelado")
        ].copy()
        counts = month_df.groupby(month_df["_fecha"].dt.day).size().to_dict()
        confirmed_month = month_df[month_df["estado"].eq("Confirmado")].copy()
        conflict_month = month_df[month_df["_row_pos"].isin(conflict_set)].copy()
        conf_counts = confirmed_month.groupby(confirmed_month["_fecha"].dt.day).size().to_dict() if not confirmed_month.empty else {}
        conflict_counts = conflict_month.groupby(conflict_month["_fecha"].dt.day).size().to_dict() if not conflict_month.empty else {}
        headers = st.columns(7)
        for col, name in zip(headers, ["Lun", "Mar", "Mié", "Jue", "Vie", "Sáb", "Dom"]):
            with col:
                st.markdown(f'<div class="qx-dayhead">{name}</div>', unsafe_allow_html=True)
        for week in _calendar.Calendar(firstweekday=0).monthdayscalendar(year, month):
            week_cols = st.columns(7)
            for col, day_num in zip(week_cols, week):
                with col:
                    if not day_num:
                        st.markdown('<div class="qx-day" style="opacity:.25"></div>', unsafe_allow_html=True)
                        continue
                    is_today = year == today.year and month == today.month and day_num == today.day
                    n = int(counts.get(day_num, 0))
                    nc = int(conf_counts.get(day_num, 0))
                    nx = int(conflict_counts.get(day_num, 0))
                    extra = f" · ⚠ {nx}" if nx else ""
                    st.markdown(
                        f'<div class="qx-day {"today" if is_today else ""}"><div class="qx-daynum">{day_num}</div><div class="qx-count">{n if n else "–"}</div><div class="qx-mini">{nc} confirmados{extra}</div></div>',
                        unsafe_allow_html=True,
                    )

        st.markdown("#### Ver detalle de un día")
        inspect_day = st.date_input("Fecha a revisar", value=anchor, key="qx_calendar_day")
        inspect_ts = pd.Timestamp(inspect_day)
        inspect_df = work[work["_fecha"].dt.normalize().eq(inspect_ts)].sort_values(["_start_min", "sala"], na_position="last")
        if inspect_df.empty:
            st.info("No hay pacientes en esa fecha.")
        else:
            for _, row in inspect_df.iterrows():
                _render_patient_card(row, "cal", detailed=True)

    # =====================================================
    # ALERTAS
    # =====================================================
    elif section == "🚨 Alertas":
        st.markdown("### Centro de alertas y riesgos")
        if alerts_df.empty:
            st.success("No se detectan conflictos, vencimientos ni datos críticos incompletos con la información actual.")
        else:
            a1, a2, a3, a4 = st.columns(4)
            a1.metric("Críticas", int((alerts_df["Nivel"] == "Crítica").sum()))
            a2.metric("Altas", int((alerts_df["Nivel"] == "Alta").sum()))
            a3.metric("Medias", int((alerts_df["Nivel"] == "Media").sum()))
            a4.metric("Pacientes afectados", int(alerts_df["_row_pos"].nunique()))
            severity = st.multiselect("Prioridad", ["Crítica", "Alta", "Media"], default=["Crítica", "Alta", "Media"], key="qx_alert_severity")
            show_alerts = alerts_df[alerts_df["Nivel"].isin(severity)].copy()
            order = {"Crítica": 0, "Alta": 1, "Media": 2}
            show_alerts["_order"] = show_alerts["Nivel"].map(order)
            show_alerts = show_alerts.sort_values(["_order", "Fecha", "Paciente"])
            st.dataframe(show_alerts[["Nivel", "Tipo", "Paciente", "Fecha", "Detalle"]], use_container_width=True, hide_index=True)

            st.markdown("#### Gestionar paciente con alerta")
            affected = work[work["_row_pos"].isin(show_alerts["_row_pos"].unique())].copy()
            if not affected.empty:
                affected["_label"] = affected.apply(
                    lambda r: f"{r['paciente'] or 'Sin nombre'} · {_fmt_date(r['_fecha'])} · {r['estado']}", axis=1
                )
                selected_label = st.selectbox("Paciente", affected["_label"].tolist(), key="qx_alert_patient")
                selected = affected[affected["_label"].eq(selected_label)].iloc[0]
                _render_patient_card(selected, "alert", detailed=True)

    # =====================================================
    # CARGAR NUEVO PACIENTE
    # =====================================================
    elif section == "➕ Cargar":
        st.markdown("### Nuevo ingreso a agenda")
        st.caption("Carga guiada y no destructiva. Si la planilla todavía no tiene un campo opcional, se agrega sólo al guardar este registro.")
        with st.form("qx_new_case", clear_on_submit=False):
            r1c1, r1c2, r1c3, r1c4 = st.columns([1.1, 1, 1, 1.25])
            with r1c1:
                new_date = st.date_input("Fecha *", value=today.date())
            with r1c2:
                new_start = st.time_input("Inicio *", value=_time(8, 0))
            with r1c3:
                new_end = st.time_input("Fin *", value=_time(9, 0))
            with r1c4:
                new_room = st.text_input("Quirófano *", placeholder="Quirófano 1")

            r2c1, r2c2 = st.columns(2)
            with r2c1:
                new_patient = st.text_input("Paciente *", placeholder="APELLIDO, Nombre")
                new_procedure = st.text_input("Procedimiento *", placeholder="Histeroscopia, LEEP, aspiración…")
                new_doctor = st.text_input("Médico *")
                new_anesth = st.text_input("Anestesista")
            with r2c2:
                new_insurance = st.text_input("Obra social / cobertura")
                new_member = st.text_input("N.º afiliado")
                new_phone = st.text_input("Teléfono / WhatsApp")
                new_auth = st.text_input("Autorización", placeholder="Número / estado / referencia")

            r3c1, r3c2, r3c3 = st.columns(3)
            with r3c1:
                new_state = st.selectbox("Estado", ESTADOS, index=ESTADOS.index("Programado"))
            with r3c2:
                new_priority = st.selectbox("Prioridad", PRIORIDADES, index=0)
            with r3c3:
                new_confirmed = st.selectbox("Confirmación paciente", ["No", "Sí"], index=0)

            st.markdown("**Checklist prequirúrgico**")
            q1, q2, q3, q4 = st.columns(4)
            new_fasting = q1.checkbox("Ayuno confirmado")
            new_consent = q2.checkbox("Consentimiento")
            new_preop = q3.checkbox("Prequirúrgico OK")
            new_material = q4.checkbox("Material OK")
            new_notes = st.text_area("Observaciones", height=90)
            submitted = st.form_submit_button("Guardar en agenda", use_container_width=True, type="primary")

        if submitted:
            errors = []
            if not new_patient.strip(): errors.append("paciente")
            if not new_procedure.strip(): errors.append("procedimiento")
            if not new_doctor.strip(): errors.append("médico")
            if not new_room.strip(): errors.append("quirófano")
            start_m = new_start.hour * 60 + new_start.minute
            end_m = new_end.hour * 60 + new_end.minute
            if end_m <= start_m:
                errors.append("hora de fin posterior al inicio")
            if errors:
                st.error("Falta completar/corregir: " + ", ".join(errors) + ".")
            else:
                values = {
                    "fecha": new_date.strftime("%d/%m/%Y"),
                    "hora_inicio": new_start.strftime("%H:%M"),
                    "hora_fin": new_end.strftime("%H:%M"),
                    "duracion_min": end_m - start_m,
                    "sala": new_room.strip(),
                    "paciente": new_patient.strip(),
                    "procedimiento": new_procedure.strip(),
                    "medico": new_doctor.strip(),
                    "estado": new_state,
                    "anestesista": new_anesth.strip(),
                    "obra_social": new_insurance.strip(),
                    "numero_afiliado": new_member.strip(),
                    "autorizacion": new_auth.strip(),
                    "telefono": new_phone.strip(),
                    "observaciones": new_notes.strip(),
                    "prioridad": new_priority,
                    "confirmado": new_confirmed,
                    "ayuno": "Sí" if new_fasting else "No",
                    "consentimiento": "Sí" if new_consent else "No",
                    "prequirurgico": "Sí" if new_preop else "No",
                    "material": "Sí" if new_material else "No",
                }
                # Crear las columnas requeridas antes de formar la nueva fila.
                for field in values:
                    _ensure_field(field)
                if field_pos.get("created_at") is not None:
                    values["created_at"] = pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S")
                if field_pos.get("updated_at") is not None:
                    values["updated_at"] = pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S")
                new_row = [""] * len(original.columns)
                for field, value in values.items():
                    pos = _ensure_field(field)
                    # _ensure_field puede agrandar la tabla después de crear new_row.
                    if len(new_row) < len(original.columns):
                        new_row.extend([""] * (len(original.columns) - len(new_row)))
                    new_row[pos] = value
                original.loc[len(original)] = new_row
                if _save("Paciente agregado a la Agenda de Quirófano."):
                    st.rerun()

    # =====================================================
    # EDITOR OPERATIVO
    # =====================================================
    elif section == "✏️ Editar":
        st.markdown("### Editor operativo")
        st.caption("Editá varias filas de una sola vez. El guardado vuelve a la misma fila real de Google Sheets y conserva todas las columnas no mostradas.")
        edit_src = filtered.sort_values(["_fecha", "_start_min", "paciente"], na_position="last").copy()
        if edit_src.empty:
            st.info("No hay filas con los filtros actuales.")
        else:
            if len(edit_src) > 500:
                st.warning("Se muestran las primeras 500 filas para mantener el editor rápido. Acotá con los filtros superiores.")
                edit_src = edit_src.head(500)
            editor = pd.DataFrame({
                "_row_pos": edit_src["_row_pos"].astype(int),
                "Paciente": edit_src["paciente"],
                "Fecha": edit_src["_fecha"].map(lambda x: x.strftime("%d/%m/%Y") if pd.notna(x) else ""),
                "Inicio": edit_src["_hora_inicio"],
                "Fin": edit_src["_hora_fin"],
                "Quirófano": edit_src["sala"],
                "Procedimiento": edit_src["procedimiento"],
                "Médico": edit_src["medico"],
                "Anestesista": edit_src["anestesista"],
                "Estado": edit_src["estado"],
                "Prioridad": edit_src["prioridad"],
                "Cobertura": edit_src["obra_social"],
                "Autorización": edit_src["autorizacion"],
                "Confirmado": edit_src["confirmado"].map(_clean),
                "Observaciones": edit_src["observaciones"],
            })
            edited = st.data_editor(
                editor,
                use_container_width=True,
                ...
                num_rows="fixed",
                disabled=["_row_pos"],
                column_config={"_row_pos": None},
                key="qx_bulk_editor",
                height=min(650, 95 + 35 * max(len(editor), 5)),
            )
            ec1, ec2 = st.columns([1, 3])
            if ec1.button("💾 Guardar cambios", key="qx_save_editor", use_container_width=True, type="primary"):
                map_display = {
                    "Paciente": "paciente", "Fecha": "fecha", "Inicio": "hora_inicio", "Fin": "hora_fin",
                    "Quirófano": "sala", "Procedimiento": "procedimiento", "Médico": "medico",
                    "Anestesista": "anestesista", "Estado": "estado", "Prioridad": "prioridad",
                    "Cobertura": "obra_social", "Autorización": "autorizacion", "Confirmado": "confirmado",
                    "Observaciones": "observaciones",
                }
                for _, edrow in edited.iterrows():
                    row_pos = int(edrow["_row_pos"])
                    for display_col, field in map_display.items():
                        value = edrow[display_col]
                        if field == "estado":
                            value = _state(value)
                        _set_cell(row_pos, field, "" if pd.isna(value) else value)
                    _touch(row_pos)
                if _save("Cambios masivos guardados correctamente."):
                    st.rerun()
            ec2.caption("Tip: para mover rápidamente un paciente entre etapas, el tablero Flujo es más cómodo. Usá este editor para fechas, horarios y datos administrativos.")

    # =====================================================
    # LISTADO + EXPORTACIÓN
    # =====================================================
    elif section == "📋 Listado":
        st.markdown("### Listado maestro")
        list_df = filtered.sort_values(["_fecha", "_start_min", "paciente"], na_position="last").copy()
        if list_df.empty:
            st.info("No hay registros con los filtros actuales.")
        else:
            display = pd.DataFrame({
                "Fecha": list_df["_fecha"].map(lambda x: x.strftime("%d/%m/%Y") if pd.notna(x) else ""),
                "Hora": list_df["_hora_inicio"],
                "Fin": list_df["_hora_fin"],
                "Paciente": list_df["paciente"],
                "Procedimiento": list_df["procedimiento"],
                "Médico": list_df["medico"],
                "Anestesista": list_df["anestesista"],
                "Quirófano": list_df["sala"],
                "Estado": list_df["estado"],
                "Prioridad": list_df["prioridad"],
                "Cobertura": list_df["obra_social"],
                "Afiliado": list_df["numero_afiliado"],
                "Autorización": list_df["autorizacion"],
                "Teléfono": list_df["telefono"],
                "Alertas": list_df["_row_pos"].map(lambda p: " · ".join(_row_alert_tags(int(p)))),
                "Observaciones": list_df["observaciones"],
            })
            st.dataframe(display, use_container_width=True, hide_index=True, height=min(700, 105 + 35 * max(len(display), 6)))
            x1, x2, x3 = st.columns([1, 1, 2])
            x1.download_button(
                "⬇️ CSV",
                data=display.to_csv(index=False).encode("utf-8-sig"),
                file_name=f"agenda_quirofano_{pd.Timestamp.today().strftime('%Y%m%d')}.csv",
                mime="text/csv",
                use_container_width=True,
            )
            try:
                excel_bytes = _vitae_excel_bytes(display, "Agenda")
                x2.download_button(
                    "⬇️ Excel",
                    data=excel_bytes,
                    file_name=f"agenda_quirofano_{pd.Timestamp.today().strftime('%Y%m%d')}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True,
                )
            except Exception:
                x2.caption("Excel no disponible")
            x3.caption(f"{len(display)} registro(s) visibles · {len(work)} registro(s) totales en la fuente")
...
# =========================================================
# FACTURACIÓN ULTRA PRO · CORRECCIÓN DE FUENTE DE FECHA
# =========================================================
def _instalar_parche_fecha_centro_facturacion() -> None:
    """
    Hace que el Centro Inteligente externo (facturacion_ultra_pro.py) use la
    columna ``mes`` exactamente igual que "Editar tabla" de views.py.

    El Centro Inteligente vive en un archivo separado. Su implementación
    original construye ``_fecha_base`` priorizando ``fecha_factura`` por encima
    de ``mes``; por eso el detalle podía mostrar 01/09, 20/08, etc. aunque la
    tabla editable mostrara 11/08, 17/08, 16/08...

    Esta corrección no toca Google Sheets ni altera datos. Únicamente reemplaza
    la fecha analítica del centro por la fecha de ``mes`` leída con ``parse_mes``
    —la misma función que usa el editor—. Se aplica a VM y VMR porque ambos usan
    el mismo módulo ``facturacion_ultra_pro``.
    """
    try:
        import facturacion_ultra_pro as _fx_ultra
    except Exception:
        # La app puede funcionar sin el módulo Ultra Pro; no romper imports.
        return

    if getattr(_fx_ultra, "_VITAE_FECHA_MES_IGUAL_EDITOR", False):
        return

    preparar_original = getattr(_fx_ultra, "_prepare_facturacion", None)
    if not callable(preparar_original):
        return

    def _prepare_facturacion_igual_editor(df_original):
        data, cols = preparar_original(df_original)

        try:
            col_mes = (cols or {}).get("mes")
            if (
                col_mes is not None
                and isinstance(df_original, pd.DataFrame)
                and col_mes in df_original.columns
            ):
                serie_mes = df_original.loc[:, col_mes]
                if isinstance(serie_mes, pd.DataFrame):
                    serie_mes = serie_mes.iloc[:, 0]

                # MISMA LECTURA QUE "EDITAR TABLA".
                fecha_mes = parse_mes(serie_mes)
                fecha_mes = pd.to_datetime(fecha_mes, errors="coerce").dt.normalize()

                # Reindexa por seguridad para mantener correspondencia 1 a 1
                # con las filas preparadas por el centro.
                fecha_mes = fecha_mes.reindex(data.index)

                data["_fecha_mes"] = fecha_mes

                # FUENTE DE VERDAD DEL CENTRO:
                # NO fecha_factura, NO vencimiento, NO otra fecha inferida.
                # Si el editor muestra la fecha de ``mes``, el centro muestra esa.
                data["_fecha_base"] = fecha_mes

                # El resto del Centro (filtros, producción, proyección, detalle,
                # exportaciones e IA) trabaja sobre _fecha_base. Recalculamos el
                # período mensual para que toda la app quede sincronizada.
                data["_mes"] = (
                    data["_fecha_base"]
                    .dt.to_period("M")
                    .astype(str)
            ...
            # Nunca dejar inutilizable Facturación por una normalización visual.
            return data, cols

        return data, cols

    _fx_ultra._prepare_facturacion = _prepare_facturacion_igual_editor
    _fx_ultra._VITAE_FECHA_MES_IGUAL_EDITOR = True


# app.py importa views.py antes de importar render_facturacion_ultra_pro, por lo
# que este parche queda instalado sobre el mismo módulo que luego renderiza el
# Centro Inteligente.
_instalar_parche_fecha_centro_facturacion()


def _facturacion_mes_igual_editor(df: pd.DataFrame, *, ordenar: bool = False) -> pd.DataFrame:
    """
    ÚNICA fuente de verdad para la columna ``mes`` de Facturación VM/VMR.

    La regla es deliberadamente la misma que usa "Editar tabla": ``parse_mes``.
    El Centro Ejecutivo recibe esta misma representación, evitando que un segundo
    parser reinterprete día/mes o construya fechas distintas a las visibles en la
    planilla editable. Esta función trabaja sólo sobre una copia.
    """
    if df is None or not isinstance(df, pd.DataFrame):
        return pd.DataFrame()

    resultado = df.copy()
    if "mes" not in resultado.columns:
        return resultado

    fechas = parse_mes(resultado["mes"])

    if ordenar:
        resultado = resultado.assign(_orden=fechas)
        ...
        resultado["mes"] = resultado["_orden"].dt.strftime("%d/%m/%Y")
        resultado["mes"] = resultado["mes"].fillna("")
        resultado = resultado.drop(columns=["_orden"], errors="ignore")
    else:
        resultado["mes"] = fechas.dt.strftime("%d/%m/%Y").fillna("")
...
def render_honorarios_medicos_pro(df: pd.DataFrame) -> None:
    ...
    honorarios_section = _vitae_lazy_selector(
        "Vista de honorarios",
        ["Médicos", "Evolución", "Procedimientos y empresas", "Control y detalle"],
        key="honorarios_active_section",
    )

    # ---------------------------------------------------------
    # MÉDICOS
    # ---------------------------------------------------------
    if honorarios_section == "Médicos":
    ...
    # EVOLUCIÓN MENSUAL
    # ---------------------------------------------------------
    if honorarios_section == "Evolución":
    ...
    # ---------------------------------------------------------
    if honorarios_section == "Procedimientos y empresas":
    ...
    # ---------------------------------------------------------
    if honorarios_section == "Control y detalle":
...
def render_banco_pro_panel(
    ...
    st.markdown(
        ...
        .banco-pro-hero {
            ...
            border-radius: 19px;
            background:
                radial-gradient(circle at 92% 10%, rgba(210,100,148, .22), transparent 31%),
                linear-gradient(135deg, rgba(48,14,29, .98), rgba(79,24,48, .94));
    ...
    # Estado factual: no usa ponderaciones ni umbrales arbitrarios.
    if saldo_actual < 0:
        estado, nivel_estado = "Saldo negativo", "error"
    elif pendientes > 0 or alertas > 0:
        estado, nivel_estado = "Requiere revisión", "warning"
    else:
        estado, nivel_estado = "Sin incidencias detectadas", "success"
    ...
    s1.metric("Estado bancario", estado, f"{alertas} incidencias de datos")
    ...
    banco_section = _vitae_lazy_selector(
        "Vista bancaria",
        ["Resumen ejecutivo", "Evolución y cierres", "Conciliación", "Proyección", "Movimientos", "Auditoría"],
        key=f"{clave}_active_section",
    )

    if banco_section == "Resumen ejecutivo":
    ...
    if banco_section == "Evolución y cierres":
...
def render_caja_pro_panel(
    ...
    st.markdown(
        ...
        .caja-pro-hero {
            ...
            border-radius: 18px;
            background: linear-gradient(135deg, rgba(52,19,33, .96), rgba(78,28,50, .90));
    ...
    # Estado factual: prioriza hechos observables, sin un score ponderado inventado.
    if saldo_actual < 0:
        estado, nivel_estado = "Saldo negativo", "error"
    elif flujo_visible < 0 or problemas > 0:
        estado, nivel_estado = "Requiere revisión", "warning"
    else:
        estado, nivel_estado = "Sin incidencias detectadas", "success"
    ...
    e1.metric("Estado de caja", estado, f"{problemas} incidencias")
    ...
    e4.metric("Autonomía observada", f"{runway_dias:.1f} días" if runway_dias > 0 else "Sin cobertura")
    e5.metric("Incidencias de datos", f"{problemas}", f"{len(data)} movimientos")

    mensaje_estado = (
        f"**Caja: {estado}.** El período produjo un flujo de {moneda(flujo_visible)}; "
    ...
    caja_section = _vitae_lazy_selector(
        "Vista de caja",
        ["Resumen ejecutivo", "Evolución y cierres", "Proyección", "Conciliación", "Movimientos", "Auditoría"],
        key=f"{clave}_active_section",
    )

    if caja_section == "Resumen ejecutivo":
        ...
        with c2:
            ...
            elif flujo_visible > 0:
                st.success(f"El período generó un flujo positivo de {moneda(flujo_visible)}.")
            else:
                st.info("El período cerró con flujo neto neutro.")
    ...
    if caja_section == "Evolución y cierres":
...
def render_lectura_ejecutiva_facturacion_pro(
    ...
    # Estado ejecutivo basado exclusivamente en hechos observables.
    incidencias_datos = sin_fecha + sin_factura + sin_vencimiento
    if vencido > 0:
        estado = "ATENCIÓN · existen saldos vencidos"
        nivel_estado = "warning"
    elif incidencias_datos > 0:
        estado = "ATENCIÓN · faltan datos de control"
        nivel_estado = "warning"
    else:
        estado = "OK · sin vencidos ni faltantes críticos"
        nivel_estado = "success"

    st.markdown("### Lectura ejecutiva")
    e1, e2, e3, e4 = st.columns(4)
    e1.metric("Estado", estado)
    e2.metric(
        "Tendencia último mes",
        "Sin comparación" if crecimiento_mensual is None else f"{crecimiento_mensual:+.1f}%",
        ultimo_mes,
    )
    e3.metric("Saldo vencido", fmt_money(vencido), f"{mora:.1f}% del pendiente")
    e4.metric("Días promedio de cobro", f"{dso:.1f}" if dso else "Sin dato")

    resumen = (
        f"**{estado}.** La vista seleccionada registra {fmt_money(facturado)} facturados, "
        f"{fmt_money(cobrado)} cobrados y {fmt_money(pendiente)} pendientes. "
        f"La tasa de cobro observada es {tasa_cobro:.1f}%."
    )
    if vencido > 0:
        resumen += f" Del pendiente, {fmt_money(vencido)} ya está vencido."
    if nivel_estado == "warning":
        st.warning(resumen)
    else:
        st.success(resumen)
    ...
    with col_a:
        st.markdown("#### Qué explica el resultado")
        impulso = []
        ...
        impulso.append(
            f"**Cobranza observada:** {tasa_cobro:.1f}% del facturado"
            + (f", con {dso:.1f} días promedio de cobro." if dso else ".")
        )
        for texto in impulso:
            st.markdown(f"- {texto}")

    with col_b:
        st.markdown("#### Qué requiere seguimiento")
        riesgos = []
        if vencido > 0:
            riesgos.append(
                f"**Saldos vencidos:** {fmt_money(vencido)}, equivalentes al {mora:.1f}% de la cartera pendiente."
            )
        if not top_obra.empty:
            riesgos.append(
                f"**Concentración actual:** {nombre_top(top_obra)} representa {conc_obra:.1f}% de la facturación visible."
            )
        if not top_med.empty:
            riesgos.append(
                f"**Concentración profesional actual:** {nombre_top(top_med)} representa {conc_med:.1f}% del volumen económico visible."
            )
        if dso > 0:
            riesgos.append(f"**Ciclo de cobro observado:** {dso:.1f} días promedio.")
        if sin_vencimiento:
            riesgos.append(f"**Vencimiento faltante:** {sin_vencimiento} saldos pendientes no tienen fecha de vencimiento.")
        if sin_factura:
            riesgos.append(f"**Trazabilidad faltante:** {sin_factura} registros activos no tienen número de factura.")
        if sin_fecha:
            riesgos.append(f"**Fecha faltante:** {sin_fecha} registros activos no tienen fecha base utilizable.")
        if not riesgos:
            riesgos.append("No se detectaron saldos vencidos ni faltantes críticos en la vista actual.")
        for texto in riesgos:
            st.markdown(f"- {texto}")

    st.markdown("#### Proyección explicable")
    p1, p2, p3, p4 = st.columns(4)
    p1.metric("Ritmo anualizado", fmt_money(proyeccion_base))
    p2.metric("Último mes observado", fmt_money(facturacion_ultimo_mes), ultimo_mes)
    p3.metric("Promedio mensual observado", fmt_money(promedio_mensual))
    p4.metric("Meses con datos", f"{len(evolucion):,}".replace(",", "."))
    st.caption(
        "El ritmo anualizado es una extrapolación lineal de datos reales del período seleccionado; "
        "no incorpora supuestos de crecimiento ni escenarios porcentuales inventados."
    )

    st.markdown("#### Próximas acciones basadas en datos")
    acciones = []
    if vencido > 0:
        acciones.append({
            "Prioridad": "Cobranza",
            "Acción": "Revisar y gestionar los saldos vencidos, comenzando por el mayor financiador de deuda.",
            "Dato que la dispara": f"{fmt_money(vencido)} vencidos",
        })
    if not deuda_obra.empty and valor_top(deuda_obra) > 0:
        acciones.append({
            "Prioridad": "Seguimiento",
            "Acción": f"Priorizar la conciliación con {nombre_top(deuda_obra)}.",
            "Dato que la dispara": f"{fmt_money(valor_top(deuda_obra))} pendientes",
        })
    if sin_vencimiento or sin_factura or sin_fecha:
        acciones.append({
            "Prioridad": "Trazabilidad",
            "Acción": "Completar los campos faltantes que impiden controlar correctamente la cartera.",
            "Dato que la dispara": f"{incidencias_datos} faltantes detectados",
        })
    if not top_proc.empty:
        acciones.append({
            "Prioridad": "Análisis operativo",
            "Acción": f"Analizar margen, demanda, insumos y capacidad de {nombre_top(top_proc)} antes de decidir expansión.",
            "Dato que la dispara": f"{conc_proc:.1f}% de la facturación visible",
        })
    if not acciones:
        acciones.append({
            "Prioridad": "Control",
            "Acción": "Mantener seguimiento periódico y comparar contra esta línea base.",
            "Dato que la dispara": "Sin alertas factuales en la vista actual",
        })
    st.dataframe(pd.DataFrame(acciones), use_container_width=True, hide_index=True)

    with st.expander("Línea base medible para próximas comparaciones", expanded=False):
        linea_base = pd.DataFrame(
            [
                ["Tasa de cobro", f"{tasa_cobro:.1f}%"],
                ["Mora sobre pendiente", f"{mora:.1f}%"],
                ["Días promedio de cobro", f"{dso:.1f}" if dso else "Sin dato"],
                ["Completitud de campos de control", f"{completitud:.1f}%"],
                ["Concentración financiador principal", f"{conc_obra:.1f}%"],
            ],
            columns=["Indicador", "Situación actual"],
        )
        st.dataframe(linea_base, use_container_width=True, hide_index=True)
        st.caption(
            "La aplicación conserva la situación real como línea base. No asigna metas numéricas "
            "hasta que exista una regla u objetivo definido por el negocio."
        )
...
def render_facturacion_industrial(
    ...
    hoy = _facturacion_hoy_argentina()
    ...
    def serie_fecha(columna, *, permitir_futuras: bool = True):
        if columna is None or columna not in data.columns:
        ...
        return _facturacion_parse_fechas(
            original,
    ...
    # En Facturación VM/VMR, ``mes`` es la fecha canónica de la planilla.
    # Si existe, se usa EXACTAMENTE esa columna, igual que "Editar tabla".
    col_mes = "mes" if "mes" in data.columns else detectar_columna(
        "fecha", "fecha procedimiento", "fecha práctica"
    )
    ...
    # FECHA DEL PROCEDIMIENTO / MES:
    # se lee con la MISMA función que "Editar tabla". No se corrige, invierte
    # ni infiere día/mes mediante ningún parser alternativo.
    if col_mes is not None and col_mes in data.columns:
        _mes_origen = data.loc[:, col_mes]
        if isinstance(_mes_origen, pd.DataFrame):
            _mes_origen = _mes_origen.iloc[:, 0]
        data["_fecha_servicio"] = parse_mes(_mes_origen)
    else:
        data["_fecha_servicio"] = pd.Series(pd.NaT, index=data.index, dtype="datetime64[ns]")

    # Las demás fechas conservan su lógica específica.
    data["_fecha_factura"] = serie_fecha(col_fecha_factura, permitir_futuras=False)
    data["_vencimiento"] = serie_fecha(col_vencimiento, permitir_futuras=True)
    data["_fecha_pago"] = serie_fecha(col_fecha_pago, permitir_futuras=False)

    # Para filtros, producción, proyección y detalle la fecha base es ``mes``.
    # No se reemplaza silenciosamente por factura/vencimiento, porque eso hacía
    # que el Centro Ejecutivo mostrara un calendario distinto al editor.
    data["_fecha_base"] = data["_fecha_servicio"]
    ...
    # `hoy` ya fue calculado con zona horaria de Argentina al iniciar el módulo.
    ...
    # ---------------------------------------------------------
    fact_section = _vitae_lazy_selector(
        "Vista de facturación",
        ["Resumen ejecutivo", "Cobranza y vencimientos", "Producción médica", "Calidad de datos", "Detalle"],
        key=f"{key}_active_section",
    )

    if fact_section == "Resumen ejecutivo":
    ...
    if fact_section == "Cobranza y vencimientos":
    ...
    if fact_section == "Producción médica":
        p1, p2 = st.columns(2)
        with p1:
            por_medico = (
    ...
    if fact_section == "Calidad de datos":
    ...
    if fact_section == "Detalle":
        detalle = filtrado.copy().sort_values(
            "_fecha_base", ascending=False, na_position="last", kind="mergesort"
        )
        tabla_detalle = pd.DataFrame(
            {
            ...
            if not pregunta.strip():
                ...
                with st.spinner("Analizando la facturación filtrada…"):
                    try:
                        ...
                        _vitae_render_error(
                            "La IA no pudo responder. La facturación continúa operativa.",
                            exc,
                            context=f"ia:facturacion:{table}",
                        )
...
def _convenios_estado_calidad(df: pd.DataFrame, metadata: pd.DataFrame, estados_fuente: pd.DataFrame) -> pd.DataFrame:
    ...
    for convenio in convenios:
        ...
        sin_valor = max(total - valorizadas, 0)
        # Estado derivado de hechos observables. Los días desde vigencia y la
        # cobertura exacta siguen visibles como datos, sin umbrales inventados.
        if not conectado:
            semaforo = "CRÍTICO · Fuente no disponible"
        elif total == 0:
            semaforo = "ATENCIÓN · Sin prácticas detectadas"
        elif duplicados > 0:
            semaforo = f"ATENCIÓN · {duplicados} códigos duplicados"
        elif sin_valor > 0:
            semaforo = f"ATENCIÓN · {sin_valor} prácticas sin valor"
        elif pd.isna(ultima):
            semaforo = "ATENCIÓN · Sin vigencia detectada"
        else:
            semaforo = "OK · Información completa"
...
def render_convenios_pro() -> None:
    ...
    st.markdown(
        ...
        .cv-hero {
            padding: 1.35rem 1.55rem;
            border: 1px solid rgba(103,30,62, .20);
            border-radius: 22px;
            background: linear-gradient(135deg, rgba(103,30,62, .14), rgba(191,56,114, .07));
            box-shadow: 0 16px 38px rgba(42,15,27, .08);
            margin-bottom: 1rem;
        }
        ...
        .cv-pill {
            ...
            border:1px solid rgba(103,30,62, .22); background:rgba(103,30,62, .08);
        }
        .cv-note {
            padding: .85rem 1rem; border-radius: 14px;
            border-left: 4px solid #671E3E; background: rgba(103,30,62, .07);
        }
        </style>
        <div class="cv-hero">
    ...
    convenios_operativos = int(calidad["Estado"].str.startswith("OK", na=False).sum()) if not calidad.empty else 0
    alertas = int((~calidad["Estado"].str.startswith("OK", na=False)).sum()) if not calidad.empty else 0
    ...
    convenios_section = _vitae_lazy_selector(
        "Vista de convenios",
        ["Centro de control", "Buscador universal", "Comparador", "Ficha por convenio",
         "Reglas y directorio", "Auditoría de calidad", "Analista IA", "Exportar"],
        key="convenios_active_section",
    )

    if convenios_section == "Centro de control":
        ...
        if calidad.empty:
            ...
            criticos = calidad[~calidad["Estado"].str.startswith("OK", na=False)]
            if criticos.empty:
    ...
    if convenios_section == "Buscador universal":
    ...
    if convenios_section == "Comparador":
    ...
    if convenios_section == "Ficha por convenio":
    ...
    if convenios_section == "Reglas y directorio":
    ...
    if convenios_section == "Auditoría de calidad":
    ...
    if convenios_section == "Analista IA":
            ...
            if not pregunta.strip():
                ...
                with st.spinner("Analizando nomencladores, valores y vigencias..."):
                    try:
                        ...
                        _vitae_render_error(
                            "No se pudo consultar la IA. Convenios continúa operativo.",
                            exc,
                            context="ia:convenios",
                        )

    if convenios_section == "Exportar":
...
def _dt_css() -> None:
    st.markdown(
        ...
        .dt-ok {border-left-color: #8D2D57; background: #F0D1DE;}
...
def render_deuda_total_pro(
    ...
    deuda_section = _vitae_lazy_selector(
        "Vista de deuda total",
        ["Centro ejecutivo", "Cargar / pagos", "Gestionar", "Migrar histórico",
         "Plan de cancelación", "Analista IA", "Exportar"],
        key="deuda_total_active_section",
    )

    # ------------------------------------------------------------------
    # CENTRO EJECUTIVO
    # ------------------------------------------------------------------
    if deuda_section == "Centro ejecutivo":
        if data.empty:
            st.info(
    ...
    # ------------------------------------------------------------------
    if deuda_section == "Migrar histórico":
    ...
    # ------------------------------------------------------------------
    if deuda_section == "Plan de cancelación":
...
def _di_vm_render_actions(data: pd.DataFrame, table: str) -> None:
    actions_section = _vitae_lazy_selector(
        "Acción", ["Nueva obligación", "Registrar pago / plan"],
        key=f"{table}_deuda_actions_section",
    )
    if actions_section == "Nueva obligación":
    ...
    if actions_section == "Registrar pago / plan":
...
def render_deudas_impositivas_pro(
    ...
    st.markdown(
        ...
        .di-vm-hero {{
            ...
            border: 1px solid rgba(160,80,115, .22);
            border-radius: 20px;
            background:
                radial-gradient(circle at 92% 10%, rgba(216,122,163, .23), transparent 30%),
                linear-gradient(135deg, rgba(62,18,37, .99), rgba(95,28,57, .94));
            color: white;
            margin: .25rem 0 1rem 0;
            box-shadow: 0 16px 42px rgba(50,14,30, .18);
        }}
    ...
    deuda_imp_section = _vitae_lazy_selector(
        "Vista de obligaciones",
        ["Centro ejecutivo", "Base en Sheets", "Importar Excel", "Cargar / pagar", "Exportar"],
        key=f"{table}_deuda_impositiva_active_section",
    )
    if deuda_imp_section == "Centro ejecutivo":
        _di_vm_render_dashboard(data, table)
    elif deuda_imp_section == "Base en Sheets":
        _di_vm_render_editor(data, table)
    elif deuda_imp_section == "Importar Excel":
        _di_vm_render_import(data, table, module_name)
    elif deuda_imp_section == "Cargar / pagar":
...
def _pp_css() -> None:
    st.markdown(
        ...
        <style>
        .pp-hero {
            background: linear-gradient(135deg, #351221 0%, #631E3C 52%, #872F55 100%);
            color: white; border-radius: 22px; padding: 26px 28px; margin: 4px 0 18px 0;
            box-shadow: 0 16px 36px rgba(60,17,36, .22);
        }
        ...
        .pp-card {
            background: rgba(255,255,255,.97); border: 1px solid rgba(99,30,60, .14);
            border-radius: 17px; padding: 16px 18px; min-height: 128px;
            box-shadow: 0 8px 24px rgba(72,21,43, .07); margin-bottom: 10px;
        }
        .pp-card-label {font-size: .76rem; color: #62717c; font-weight: 750; text-transform: uppercase; letter-spacing: .06em;}
        .pp-card-value {font-size: 1.52rem; color: #47172C; font-weight: 850; margin-top: 5px; white-space: nowrap;}
        ...
        .pp-ok {border-left-color: #823154; background: #F1D0DE;}
        .pp-note {background:#FAF5F7; border:1px solid #EDDFE5; border-radius:12px; padding:12px 14px; color:#50616d;}
        div[data-testid="stMetric"] {border: 1px solid rgba(99,30,60, .12); padding: 12px 14px; border-radius: 14px; background: rgba(255,255,255,.94);}
        </style>
        """,
        unsafe_allow_html=True,
    )
...
def render_planes_pagos_prestamos_pro(
    ...
    planes_section = _vitae_lazy_selector(
        "Vista de planes y préstamos",
        ["Centro ejecutivo", "Cronograma", "Cargar / pagar", "Importar Excel",
         "Gestionar", "Vista original", "Analista IA", "Exportar"],
        key=f"{table}_planes_active_section",
    )

    # ---------------------------------------------------------------
    # CENTRO EJECUTIVO
    # ---------------------------------------------------------------
    if planes_section == "Centro ejecutivo":
        if data.empty:
            st.info(
    ...
    # CARGAR / PAGAR
    # ---------------------------------------------------------------
    if planes_section == "Cargar / pagar":
    ...
    # IMPORTAR EXCEL
    # ---------------------------------------------------------------
    if planes_section == "Importar Excel":
        ...
        if uploaded is not None:
            ...
            if read_errors:
                st.warning(
                    f"No se pudieron interpretar {len(read_errors)} hoja(s) del archivo. "
                    "El resto de las hojas válidas continúa disponible."
                )
                if _vitae_debug_enabled():
                    with st.expander("Detalle técnico de importación", expanded=False):
    ...
    # VISTA ORIGINAL
    # ---------------------------------------------------------------
    if planes_section == "Vista original":
            ...
            if not question.strip():
                ...
                with st.spinner("Analizando cuotas, saldos y vencimientos..."):
                    try:
                        ...
                        _vitae_render_error(
                            "No se pudo consultar la IA. El módulo continúa operativo.",
                            error,
                            context=f"ia:planes:{table}",
                        )

    # ---------------------------------------------------------------
    # EXPORTAR
    # ---------------------------------------------------------------
    if planes_section == "Exportar":
...
def _gv_css() -> None:
    st.markdown(
        ...
        <style>
        .gv-hero {
            background: linear-gradient(135deg, #46172B 0%, #691E3F 48%, #952F5B 100%);
            color: white; border-radius: 24px; padding: 27px 30px; margin: 4px 0 18px 0;
            box-shadow: 0 18px 42px rgba(74,22,44, .24);
        }
        ...
        .gv-card {
            background:rgba(255,255,255,.98); border:1px solid rgba(105,30,63, .14);
            border-radius:18px; padding:16px 18px; min-height:126px;
            box-shadow:0 9px 26px rgba(73,21,44, .07); margin-bottom:10px;
        }
        .gv-card-label {font-size:.74rem; color:#68777c; font-weight:800; letter-spacing:.055em; text-transform:uppercase;}
        .gv-card-value {font-size:1.50rem; color:#45152A; font-weight:900; margin-top:6px; white-space:nowrap;}
        ...
        .gv-ok {border-left-color:#862D54; background:#F1D0DE;}
        .gv-info {border-left-color:#AA3F6D; background:#F1D0DE;}
        </style>
        """,
        unsafe_allow_html=True,
    )
...
def render_gine_vitae_pro(
    ...
    gine_section = _vitae_lazy_selector(
        "Vista Gine Vitae",
        ["Centro ejecutivo", "Planes y pacientes", "Operación", "Caja",
         "Gestión", "Importar Excel", "Vista original", "IA y exportación"],
        key=f"{table}_gine_active_section",
    )

    if gine_section == "Centro ejecutivo":
        if data.empty:
    ...
    if gine_section == "Planes y pacientes":
        if data.empty:
    ...
    if gine_section == "Caja":
        ...
        # Filtro por concepto: afecta movimientos, tarjetas, gráfico y tabla de caja.
        concept_options = []
        if not visible_cash.empty and "concepto" in visible_cash.columns:
            concept_options = sorted({
                str(value).strip()
                for value in visible_cash["concepto"].dropna().tolist()
                if str(value).strip()
            }, key=str.casefold)

        selected_concept = st.selectbox(
            "Concepto",
            ["Todos"] + concept_options,
            key=f"gv_cash_concept_filter_{table}",
        )
        if selected_concept != "Todos" and not visible_cash.empty:
            visible_cash = visible_cash[
                visible_cash["concepto"].fillna("").astype(str).str.strip().eq(selected_concept)
            ]

        income = float(visible_cash["ingreso"].sum()) if not visible_cash.empty else (
            float(visible_summary["ingresos"].sum()) if selected_concept == "Todos" else 0.0
        )
    ...
    if gine_section == "Importar Excel":
    ...
    if gine_section == "Vista original":
    ...
    if gine_section == "IA y exportación":
                ...
                if not question.strip():
                    ...
                    with st.spinner("Analizando pacientes, cobros y pendientes..."):
                        try:
                            ...
                            _vitae_render_error(
                                "No se pudo consultar la IA. Gine Vitae continúa operativo.",
                                error,
                                context=f"ia:gine:{table}",
                            )
        with export_col:
...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    try:
        ...
        _vitae_render_error(
            f"No se pudo leer Google Sheets para {table}. El módulo se abrirá sin datos hasta que la fuente vuelva a estar disponible.",
            e,
            context=f"lectura_sheet:{table}",
        )
    ...
    # Navegación lazy: Streamlit ejecuta el contenido de todos los st.tabs en
    # cada rerun. En módulos grandes eso multiplicaba filtros, editores, imports
    # y exportaciones aunque estuvieran ocultos. Esta barra conserva todas las
    # funciones pero renderiza únicamente la sección activa.
    workspace_options = [
        "Panel PRO",
        "Cargar",
        "Importar",
    ...
    active_workspace = _vitae_lazy_selector(
        "Área de trabajo",
        workspace_options,
        key=f"workspace_{table}",
    )

    if active_workspace == "Panel PRO":
        ...
        if table in ["facturacion_vm", "facturacion_vmr"]:
            # El Centro Ejecutivo recibe la MISMA lectura de ``mes`` que
            # "Editar tabla". Así ambos módulos parten de una sola fuente
            # de verdad y no pueden volver a invertir día/mes por separado.
            df_facturacion_panel = _facturacion_mes_igual_editor(
                df_base.copy(),
                ordenar=False,
            )
            render_facturacion_industrial(
                df_original=df_facturacion_panel,
                module_name=module_name,
                table=table,
            )
        elif df_panel.empty:
                ...
                if errors:
                    ...
                    if _vitae_debug_enabled():
                        with st.expander("Detalle de validación", expanded=False):
                            st.write({key: value for key, value in data.items() if "password" not in str(key).lower()})
                else:
                    try:
                        ...
                        _vitae_exception_detail(e)
    if active_workspace == "Importar":
        render_importer(module_name, cfg)
    if active_workspace == "Editar tabla":
        ...
        if df.empty:
            ...
            if "mes" in df_edit.columns:
                if table in ["facturacion_vm", "facturacion_vmr"]:
                    # Misma función compartida por Editor y Centro Ejecutivo.
                    df_edit = _facturacion_mes_igual_editor(df_edit, ordenar=True)
                else:
            ...
            # -----------------------------------------------------------------
            # EDICIÓN + ELIMINACIÓN TOUCH-SAFE
            # -----------------------------------------------------------------
            # El basurero nativo de st.data_editor depende de una toolbar que
            # algunas versiones de Streamlit esconden detrás del hover. En iPad
            # y otros dispositivos táctiles eso puede dejar la fila marcada pero
            # sin un control de borrado visible. Para que eliminar sea SIEMPRE
            # posible, la tabla usa una columna explícita de selección y un
            # basurero permanente. La eliminación sigue siendo no destructiva:
            # recién se escribe en Google Sheets al pulsar "Guardar cambios".
            df_edit = df_edit.reset_index(drop=True)

            editor_work_key = f"_vitae_editor_work_{table}"
            editor_rev_key = f"_vitae_editor_rev_{table}"
            delete_col = "__vitae_eliminar__"

            if editor_rev_key not in st.session_state:
                st.session_state[editor_rev_key] = 0
            if editor_work_key not in st.session_state:
                st.session_state[editor_work_key] = df_edit.copy()

            editor_input = st.session_state[editor_work_key].copy()
            editor_input = editor_input.drop(columns=[delete_col], errors="ignore")
            editor_input.insert(0, delete_col, False)

            edited_df = st.data_editor(
                editor_input,
                use_container_width=True,
                hide_index=True,
                # Las altas se realizan desde "Cargar". Aquí dejamos editar y
                # borrar de forma explícita, evitando depender del toolbar nativo.
                num_rows="fixed",
                column_config={
                    delete_col: st.column_config.CheckboxColumn(
                        "🗑️",
                        help="Marcar esta fila para eliminarla",
                        default=False,
                        width="small",
                    ),
                },
                key=f"editor_{table}_{st.session_state[editor_rev_key]}",
            )

            delete_mask = edited_df.get(delete_col, pd.Series(False, index=edited_df.index))
            try:
                delete_mask = delete_mask.fillna(False).astype(bool)
            except Exception:
                delete_mask = pd.Series(False, index=edited_df.index)
            selected_delete = int(delete_mask.sum())

            delete_col_ui, save_col_ui = st.columns([1.15, 1])
            with delete_col_ui:
                delete_pressed = st.button(
                    f"🗑️ Eliminar seleccionadas ({selected_delete})",
                    key=f"eliminar_editor_{table}_{st.session_state[editor_rev_key]}",
                    disabled=selected_delete == 0,
                    use_container_width=True,
                )
            with save_col_ui:
                save_pressed = st.button(
                    "Guardar cambios",
                    type="primary",
                    key=f"guardar_editor_{table}_{st.session_state[editor_rev_key]}",
                    use_container_width=True,
                )

            if delete_pressed:
                # Conserva también cualquier edición hecha antes de borrar.
                restante = edited_df.loc[~delete_mask].copy()
                restante = restante.drop(columns=[delete_col], errors="ignore").reset_index(drop=True)
                st.session_state[editor_work_key] = restante
                st.session_state[editor_rev_key] += 1
                st.toast(
                    f"{selected_delete} registro(s) quitado(s) de la edición. "
                    "Pulsá Guardar cambios para confirmar.",
                    icon="🗑️",
                )
                st.rerun()
            ...
            if save_pressed:
                try:
                    ...
                    limpio = limpio.drop(
                        columns=[delete_col, "saldo", "saldo_usd", "saldo_movimiento"],
                        errors="ignore"
                    )
                    ...
                    st.session_state.pop(editor_work_key, None)
                    st.session_state[editor_rev_key] += 1
                    ...
                    _vitae_exception_detail(e)
    if active_workspace == "Columnas":
        _render_column_preferences(
            df_base.copy(),
            table=table,
            labels=labels,
        )
    if active_workspace == "Exportar":
        df = add_balance_columns(df_base.copy())
        if table == "cuenta_corriente_vm":
        ...
        if df.empty:
            ...
            export_df = _vitae_apply_export_preferences(df, table, labels)
            ...
            with export_left:
                st.download_button(
                    ...
                    mime="text/csv",
                    use_container_width=True,
                    key=f"export_csv_{table}",
                )
            with export_right:
                try:
                    xlsx_bytes = _vitae_excel_bytes(export_df, sheet_name=str(module_name)[:31])
                    st.download_button(
                        "Descargar Excel",
                        data=xlsx_bytes,
                        ...
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        use_container_width=True,
                        key=f"export_xlsx_{table}",
                    )
                except Exception as exc:
                    _vitae_render_error(
                        "No se pudo generar el Excel. El CSV continúa disponible.",
                        exc,
                        context=f"export:{table}",
                    )
...
def _render_cuenta_corriente_pro_legacy(df, table=""):
    ...
    legacy_cc_section = _vitae_lazy_selector(
        "Vista de cuenta corriente", ["Resumen", "Vencimientos", "Detalle"],
        key=f"{key}_legacy_section",
    )

    if legacy_cc_section == "Resumen":
        izquierda, derecha = st.columns(2)
    ...
    if legacy_cc_section == "Vencimientos":
...
def render_cuenta_corriente_vmr_pro(df):
    ...
    st.markdown(
        ...
        .ccvmr-hero {
            ...
            padding: 1.15rem 1.35rem;
            border-radius: 18px;
            background: linear-gradient(120deg, #2A0F1B 0%, #3A1726 55%, #4A2434 100%);
            border: 1px solid rgba(191,141,163, .24);
            box-shadow: 0 12px 30px rgba(42,15,27, .16);
            margin: .2rem 0 1rem 0;
            color: #EDD4DF !important;
        }
        .ccvmr-hero .ccvmr-kicker {
            color: #E6AAC4 !important;
        ...
        .ccvmr-note {
            ...
            box-shadow: 0 4px 14px rgba(42,15,27, .05);
    ...
    if top_entity:
        alerts.append(f"Concentración actual: {top_entity} representa {concentration * 100:.1f}% de la exposición consolidada.")
    if not alerts:
    ...
    ccvmr_section = _vitae_lazy_selector(
        "Vista de cuenta corriente",
        ["Panorama", "Vencimientos", "Entidades", "Evolución", "Detalle"],
        key=f"{module_name}_ccvmr_active_section",
    )

    if ccvmr_section == "Panorama":
        p1, p2 = st.columns(2)
        with p1:
    ...
    if ccvmr_section == "Detalle":
        # Estas tablas antes quedaban disponibles por efecto colateral de st.tabs
        # (Streamlit ejecutaba todas las pestañas). En navegación lazy se calculan
        # explícitamente sólo cuando el usuario abre el detalle/exportación.
        summary_rows = pd.DataFrame(
            [
                ["Cartera activa", len(active), active["_entidad"].nunique(), _money(total_pending_ars), _money(total_pending_usd, "USD")],
                ["Vencida", len(overdue), overdue["_entidad"].nunique(), _money(overdue_ars), _money(overdue_usd, "USD")],
...
def render_analisis_global_vitae(dfs):
    ...
    # =====================================================
    st.markdown("---")
    with st.spinner("Analizando información ejecutiva…"):
        try:
            ...
            _vitae_exception_detail(exc, context="director_ia:resumen_ejecutivo")
            resumen_director = _director_resumen_local(
                global_df=global_df,
                anio=ANIO_ANALISIS,
            )
...
def render_configuracion() -> None:
    ...
    st.caption("Centro de administración, seguridad y control de acceso de Vitae.")

    if not _auth_is_admin():
        st.warning("La gestión de usuarios y permisos requiere perfil Administrador.")
        return

    config_section = _vitae_lazy_selector(
        "Área de configuración", ["Usuarios", "Permisos", "Empresas", "Sistema"],
        key="config_active_section",
    )
    if config_section == "Usuarios":
        _auth_render_user_management()
    elif config_section == "Permisos":
        _auth_render_permissions_management()
    elif config_section == "Empresas":
        _auth_render_companies_management()
    else:
        _auth_render_security_status()
        st.divider()
        st.markdown("### Sincronización Google Sheets")
        col1, col2 = st.columns(2)
        with col1:
            if st.button("⬆️ Subir datos actuales a Google Sheets", use_container_width=True, key="cfg_sync_up"):
                try:
                    ...
                    _vitae_exception_detail(e)
        with col2:
            if st.button("⬇️ Leer datos desde Google Sheets", use_container_width=True, key="cfg_sync_down"):
                try:
                    restore_all_from_sheets()
                    _auth_load_users(force=True)
                    st.success("Datos restaurados desde Google Sheets.")
                    st.rerun()
                except Exception as e:
                    st.error("No se pudo leer desde Google Sheets.")
                    _vitae_exception_detail(e)

        st.divider()
        st.markdown("### Mantenimiento de módulos")
        st.warning("Zona sensible. Borrar una base elimina los registros del módulo seleccionado.")
        ...
        # Se conserva la selección y confirmación que ya existían, sin ejecutar
        # un borrado automático. La app original no tenía una acción de borrado
        # conectada y este panel evita introducir una operación destructiva nueva.
        if confirmar:
            st.caption("Confirmación registrada. No se ejecuta ningún borrado automático desde esta pantalla.")

def seed_examples() -> None:
    """Compatibilidad heredada: en producción nunca inserta datos ficticios."""
    _VITAE_LOGGER.warning("seed_examples() fue invocada, pero la carga de datos de ejemplo está deshabilitada.")
    st.info("La carga de datos de ejemplo está deshabilitada para proteger la integridad de Google Sheets.")
    return None
# =========================================================
...
def render_resumen_empresas_pro(
    ...
    def calcular_empresa(
        ...
        # ESTADO BASADO EN HECHOS OBSERVABLES

        # -------------------------------------------------

        # No se asigna un puntaje 0–100 con ponderaciones arbitrarias.
        # El badge comunica la condición concreta que surge de los datos.
        if disponible < 0:

            estado = "Disponibilidad negativa"
            ...
            estado = "Resultado mensual negativo"
            ...
            estado = "Con vencimientos pendientes"

            emoji_estado = "●"

            clase_estado = "estado-atencion"

        elif pendiente_mes > 0:

            estado = "Con saldo por cobrar"
        ...
        <div class="empresa-card">
            ...
            <div class="empresa-header">
                ...
                <div class="empresa-estado {datos["clase_estado"]}">
                    ...
                    <span>{datos["vencidos"]} vencidos</span>
    ...
    st.html(
        ...
        <style>
            ...
            .empresa-card {
                ...
                border: 1px solid rgba(79,44,59, 0.12);
            ...
            .empresa-participacion {
                ...
                border-bottom: 1px solid rgba(79,44,59, 0.1);
            ...
            .metrica-box {
                ...
                border: 1px solid rgba(79,44,59, 0.09);
...
def render_briefing_automatico_vitae(
    ...
    with st.spinner("Analizando toda la información de Vitae…"):
        try:
            briefing = generar_briefing_automatico(dfs=dfs)
        except Exception as exc:
            _vitae_exception_detail(exc, context="director_ia:briefing")
            briefing = _director_briefing_local(dfs=dfs)
