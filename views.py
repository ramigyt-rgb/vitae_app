...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    with tab_panel:
        ...
        if df.empty:
            ...
            total_facturado = 0
            for c in cols_monto:
                total_facturado += sum_money_col(filtered[c])
            ...
            if module_name in ["Cuenta Corriente VMR", "Cuenta Corriente VM"]:
                total_compras = sum_money_col(filtered["importe"]) if "importe" in filtered.columns else 0
                pagado = sum_money_col(filtered["pagado"]) if "pagado" in filtered.columns else 0
                a_pagar = max(0, money(total_compras) - money(pagado))
                deuda_pesos = filtered["saldo"].apply(money).sum() if "saldo" in filtered.columns else a_pagar
                deuda_usd = (
                    filtered["saldo_usd"].apply(money).sum()
                    if "saldo_usd" in filtered.columns
                    else 0
                    )
                c1, c2, c3, c4 = st.columns(4)
                c1.metric("💸 Pagado", fmt_money(pagado))
                c2.metric("🟡 A pagar", fmt_money(a_pagar))
                c3.metric("💰 Deuda $", fmt_money(deuda_pesos))
                c4.metric("💵 Deuda USD", f"USD {money(deuda_usd):,.2f}")
        st.divider()
