...
def render_gine_vitae_pro(
    ...
    with tabs[0]:
        if data.empty:
            ...
            with r2[3]:
                position = float(summary[summary["hoja_origen"].isin(selected_sheets)]["saldo_movimientos"].sum()) if selected_sheets and not summary.empty else float(summary["saldo_movimientos"].sum())
                _gv_card("Posición de caja", _gv_money(position), "Ingresos menos egresos")
    ...
    with tabs[3]:
        ...
        with cm[4]: _gv_card("Cuenta", _gv_money(account_declared), "Saldo − caja declarada")
