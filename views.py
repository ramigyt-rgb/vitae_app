...
def _dg_date_series(df: pd.DataFrame, candidates: list[str]) -> pd.Series:
    ...
    raw = _dg_series(df, column).copy().replace("", pd.NA)

    # Una columna llamada "mes" muchas veces contiene 1..12. Pandas puede
    # interpretar esos enteros como nanosegundos de 1970, generando períodos
    # ficticios. Un mes aislado no es una fecha auditable: se descarta.
    if _dg_norm(column) == "mes":
        as_text = raw.astype("string").str.strip()
        month_only = as_text.str.fullmatch(r"(?:0?[1-9]|1[0-2])", na=False)
...
def _dg_build_model(
    ...
    for module_name, cfg in MODULES.items():
        ...
        if category == "Facturación":
            invoice_date = _dg_date_series(df, ["fecha_factura", "fecha", "mes", "created_at"])
            ...
            # Nunca inventar la fecha de cobro usando la fecha de factura.
            # Si el registro está cobrado pero no tiene fecha de pago, el saldo
            # queda correctamente en cero, aunque no se atribuye a ningún mes.
            ...
            if balance_col is not None:
                ...
                dated_valid = balance_series.notna() & movement_date.notna()
                if dated_valid.any():
                    # Último saldo con fecha real. Las filas sin fecha jamás
                    # pueden desplazar al último movimiento fechado.
                    latest_index = movement_date.loc[dated_valid].idxmax()
                    balance = float(balance_series.loc[latest_index])
                else:
                    valid_values = balance_series.dropna()
                    balance = float(valid_values.iloc[-1]) if not valid_values.empty else 0.0
            else:
    ...
    if not billing.empty:
        ...
        # Cobros efectivamente fechados dentro del período, sin imputaciones
        # artificiales a la fecha de factura.
        collected_period = float(billing.loc[collection_mask, "Cobrado"].sum())
        # Cobrado de las facturas emitidas en el período y con cobro ocurrido
        # hasta el cierre seleccionado. Este es el numerador correcto para la
        # tasa de cobranza de la cohorte facturada.
        cohort_collection_mask = (
            invoice_mask
            & billing["Cobrado"].gt(0)
            & billing["Fecha cobro"].notna()
            & billing["Fecha cobro"].le(end.normalize())
        )
        cohort_collected_period = float(billing.loc[cohort_collection_mask, "Cobrado"].sum())
        # Pendiente correspondiente EXCLUSIVAMENTE a las facturas emitidas
        # dentro del período seleccionado. Este es el valor comparable con
        # "Facturado en período" y evita mezclar cartera histórica con el mes.
        billing_pending_period = float(billing.loc[invoice_mask, "Pendiente"].sum())
        # Cartera histórica de facturación: se conserva como dato de gestión,
        # pero nunca se presenta como si perteneciera al período seleccionado.
        ...
        billed_period = collected_period = cohort_collected_period = billing_pending_period = billing_pending = 0.0
    ...
    for column, company in zip(columns, companies):
        with column:
            ...
            if not company_billing.empty:
                ...
                cohort_mask = (
                    invoice_mask
                    & company_billing["Cobrado"].gt(0)
                    & company_billing["Fecha cobro"].notna()
                    & company_billing["Fecha cobro"].le(end.normalize())
                )
                cohort_collected = float(company_billing.loc[cohort_mask, "Cobrado"].sum())
            ...
            b.metric("Cobrado en período", fmt_money(collected), delta=f"{rate:.1f}% cobrado de facturas del período")
            c, d = st.columns(2)
...
def _dg_render_overview(model: dict[str, Any], start: pd.Timestamp, end: pd.Timestamp) -> None:
    ...
    k3.metric(
        ...
        delta=f"{model['collection_rate']:.1f}% cobrado de facturas del período" if model["billed_period"] > 0 else None,
    )
...
def _dg_render_finance(model: dict[str, Any], start: pd.Timestamp, end: pd.Timestamp, today: pd.Timestamp) -> None:
    ...
    f4.metric("Cuentas por pagar · actuales", fmt_money(model["account_payables"]))
...
def render_dashboard() -> None:
    ...
    with export_col:
        snapshot = pd.DataFrame([{
            ...
            "Cobrado en período": model["collected_period"],
            "Cobrado de facturas del período": model["cohort_collected_period"],
