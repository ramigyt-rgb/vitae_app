...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    if tabla_limpia is None or tabla_limpia.empty:
        st.warning("No hay registros para mostrar con estos filtros.")
    else:
        tabla_limpia = tabla_limpia.drop(
            columns=["id", "created_at", "updated_at"],
            errors="ignore"
        )
        # Arreglar columna mes para Tabla limpia
        if "mes" in tabla_limpia.columns:
            tabla_limpia["mes"] = tabla_limpia["mes"].replace(
                ["None", "nan", "NaT", "", None],
                pd.NA
            )
            # Si mes está vacío, usar otras fechas disponibles
            for col in ["fecha_factura", "fecha_pago", "vencimiento", "fecha"]:
                if col in tabla_limpia.columns:
                    tabla_limpia[col] = tabla_limpia[col].replace(
                        ["None", "nan", "NaT", "", None],
                        pd.NA
                    )
                    tabla_limpia["mes"] = tabla_limpia["mes"].fillna(tabla_limpia[col])
            # Ordenar correctamente
            tabla_limpia["_orden_fecha"] = pd.to_datetime(
                tabla_limpia["mes"],
                errors="coerce",
                dayfirst=False
            )
            ...
            tabla_limpia["mes"] = tabla_limpia["_orden_fecha"].dt.strftime("%d/%m/%Y")
            # Que no muestre None/NaT
            tabla_limpia["mes"] = tabla_limpia["mes"].fillna("")
