...
# =========================================================
# DEUDAS IMPOSITIVAS VM · CENTRO DE CONTROL ULTRA PRO
# =========================================================
_DEUDA_VM_COLUMNS = [
    "id",
    "empresa",
...
def _deuda_vm_texto(valor: Any) -> str:
    """Texto seguro para celdas de Google Sheets, Excel y CSV."""
    import math
...
def _deuda_vm_normalizar_texto(valor: Any) -> str:
    import re
    import unicodedata

    texto = _deuda_vm_texto(valor).lower()
    texto = unicodedata.normalize("NFKD", texto)
    texto = "".join(c for c in texto if not unicodedata.combining(c))
    texto = re.sub(r"[^a-z0-9%+./-]+", " ", texto)
    return " ".join(texto.split())
...
def _deuda_vm_numero(valor: Any) -> float:
    """Interpreta importes argentinos: 1.234.567,89, $ 12.500 y números puros."""
    import re
    ...
    try:
        if "," in texto and "." in texto:
            ...
            partes = texto.split(".")
            if len(partes) > 2 or (
                len(partes[-1]) == 3 and all(p.isdigit() for p in partes)
            ):
                texto = "".join(partes)
        numero = float(texto)
...
def _deuda_vm_fecha(valor: Any):
    """Convierte fechas de Excel/Sheets, incluidas fechas seriales."""
    if valor is None or _deuda_vm_texto(valor) == "":
        return pd.NaT
    if isinstance(valor, (int, float)) and not isinstance(valor, bool):
        try:
            numero = float(valor)
...
def _deuda_vm_periodo(valor: Any) -> str:
    """Normaliza 202601, ene-26, 01/2026 y fechas al formato YYYY-MM."""
    import re
    ...
    if isinstance(valor, (int, float)) and not isinstance(valor, bool):
        try:
            ...
            pass
        entero = str(int(float(valor)))
        if re.fullmatch(r"20\d{4}", entero):
            anio, mes = int(entero[:4]), int(entero[4:])
            if 1 <= mes <= 12:
                return f"{anio:04d}-{mes:02d}"

    texto = _deuda_vm_normalizar_texto(valor)
    if not texto:
        return ""

    compacto = re.sub(r"\D", "", texto)
    if re.fullmatch(r"20\d{4}", compacto):
        anio, mes = int(compacto[:4]), int(compacto[4:])
        if 1 <= mes <= 12:
            return f"{anio:04d}-{mes:02d}"
    ...
    match = re.search(r"\b([a-z]+)[-/. ](\d{2,4})\b", texto)
    if match and match.group(1) in meses:
        anio = int(match.group(2))
        if anio < 100:
            anio += 2000
        return f"{anio:04d}-{meses[match.group(1)]:02d}"

    fecha = _deuda_vm_fecha(valor)
    if pd.notna(fecha):
        return fecha.strftime("%Y-%m")
    return ""


def _deuda_vm_id(*partes: Any) -> str:
    import hashlib

    base = "|".join(_deuda_vm_normalizar_texto(p) for p in partes)
    return "DVM-" + hashlib.sha1(base.encode("utf-8")).hexdigest()[:16].upper()


def _deuda_vm_clasificar(grupo: Any, descripcion: Any) -> tuple[str, str, str]:
    """Traduce la estructura visual de la planilla histórica a campos corporativos."""
    grupo_txt = _deuda_vm_texto(grupo)
    desc_txt = _deuda_vm_texto(descripcion)
    combinado = _deuda_vm_normalizar_texto(f"{grupo_txt} {desc_txt}")

    if "f931" in combinado or any(
        token in combinado
        for token in [
            "seguro de vida colectivo",
            "aportes obras sociales",
            "aseg riesgo de trabajo",
            "contribuciones obra social",
            "aportes seg social",
            "contribuciones seg social",
        ]
    ):
        return "ARCA", "F931", desc_txt or "Seguridad social"
    if "iva" in combinado or "ddjj mensual" in combinado:
        return "ARCA", "IVA", desc_txt or "IVA DDJJ mensual"
    if any(token in combinado for token in ["municip", "tissh", "tasa de inspeccion"]):
        return "Municipalidad de Salta", "TISSH", desc_txt or "Tasa municipal"
    if "fatsa" in combinado:
        return "FATSA", "Aportes sindicales", desc_txt or "FATSA"
    if "atsa" in combinado:
        return "ATSA", "Cuota sindical", desc_txt or "ATSA"
    if "autonom" in combinado:
        return "ARCA", "Autónomos", desc_txt or grupo_txt or "Autónomos"

    organismo = grupo_txt or "Otros"
    tributo = desc_txt or grupo_txt or "Obligación impositiva"
    return organismo, tributo, desc_txt or tributo


def _deuda_vm_alias_columna(columnas: list[str], aliases: list[str]) -> str | None:
    normalizadas = {_deuda_vm_normalizar_texto(c): c for c in columnas}
    for alias in aliases:
        alias_n = _deuda_vm_normalizar_texto(alias)
        for normalizada, original in normalizadas.items():
            if normalizada == alias_n or alias_n in normalizada:
                return original
    return None


def _deuda_vm_normalizar(df: pd.DataFrame, fuente: str = "Google Sheets") -> pd.DataFrame:
    """Convierte cualquier tabla razonablemente tabular al modelo único del módulo."""
    if df is None or (df.empty and len(df.columns) == 0):
        return pd.DataFrame(columns=_DEUDA_VM_COLUMNS)

    data = df.copy()
    data.columns = [_deuda_vm_texto(c) for c in data.columns]
    columnas = data.columns.tolist()

    alias = {
        "id": ["id", "registro_id", "deuda_id"],
        "empresa": ["empresa"],
        "tipo_registro": ["tipo_registro", "tipo de registro"],
        "organismo": ["organismo", "entidad", "acreedor", "categoria"],
        "tributo": ["tributo", "impuesto", "sindicato", "tasa"],
        "concepto": ["concepto", "descripcion", "detalle", "obligacion"],
        "periodo": ["periodo", "periodos", "mes"],
        "fecha_vencimiento": [
            "fecha_vencimiento",
        ...
        "capital": ["capital", "importe", "monto", "valor"],
        "intereses": ["intereses", "interes", "recargos"],
        "deuda_total": ["deuda_total", "deuda total", "total", "importe total"],
        "pagado": ["pagado", "regularizado", "abonado"],
        "saldo": ["saldo", "saldo pendiente", "pendiente"],
        "estado": ["estado", "situacion"],
        "fecha_pago": ["fecha_pago", "fecha de pago", "pago fecha"],
        "plan_id": ["plan_id", "id plan"],
        "plan_nombre": ["plan_nombre", "plan", "nombre plan"],
        "nro_cuota": ["nro_cuota", "numero cuota", "cuota nro"],
        "total_cuotas": ["total_cuotas", "cantidad cuotas"],
        "fecha_origen": ["fecha_origen", "fecha carga", "fecha"],
        "fuente": ["fuente", "origen"],
        "observaciones": ["observaciones", "notas", "comentarios"],
        "actualizado_en": ["actualizado_en", "updated_at", "actualizacion"],
    }
    encontrados = {
        destino: _deuda_vm_alias_columna(columnas, candidatos)
        for destino, candidatos in alias.items()
    }

    # Sin período ni importes reconocibles no es una tabla normalizada.
    tiene_periodo = encontrados["periodo"] is not None
    tiene_importe = any(
        encontrados[c] is not None
        for c in ["capital", "deuda_total", "saldo", "pagado"]
    )
    if not (tiene_periodo and tiene_importe):
        return pd.DataFrame(columns=_DEUDA_VM_COLUMNS)

    salida = pd.DataFrame(index=data.index)
    for destino in _DEUDA_VM_COLUMNS:
        origen = encontrados.get(destino)
        salida[destino] = data[origen] if origen in data.columns else ""

    salida["empresa"] = salida["empresa"].apply(_deuda_vm_texto).replace("", "VM")
    salida["tipo_registro"] = (
        salida["tipo_registro"].apply(_deuda_vm_texto).replace("", "DEUDA")
    )
    salida["organismo"] = salida["organismo"].apply(_deuda_vm_texto)
    salida["tributo"] = salida["tributo"].apply(_deuda_vm_texto)
    salida["concepto"] = salida["concepto"].apply(_deuda_vm_texto)
    salida["periodo"] = salida["periodo"].apply(_deuda_vm_periodo)

    for columna in ["capital", "intereses", "deuda_total", "pagado", "saldo"]:
        salida[columna] = salida[columna].apply(_deuda_vm_numero)

    salida["deuda_total"] = salida.apply(
        lambda r: r["deuda_total"]
        if abs(r["deuda_total"]) > 0.001
        else r["capital"] + r["intereses"],
        axis=1,
    )
    salida["capital"] = salida.apply(
        lambda r: r["capital"]
        if abs(r["capital"]) > 0.001
        else max(0.0, r["deuda_total"] - r["intereses"]),
        axis=1,
    )
    salida["saldo"] = salida.apply(
        lambda r: max(0.0, r["deuda_total"] - r["pagado"])
        if abs(r["saldo"]) <= 0.001
        else max(0.0, r["saldo"]),
        axis=1,
    )

    for columna in ["fecha_vencimiento", "fecha_pago", "fecha_origen", "actualizado_en"]:
        salida[columna] = salida[columna].apply(_deuda_vm_fecha)

    hoy = pd.Timestamp.today().normalize()
    salida["fecha_origen"] = salida["fecha_origen"].fillna(hoy)
    salida["actualizado_en"] = hoy
    salida["fuente"] = salida["fuente"].apply(_deuda_vm_texto).replace("", fuente)
    salida["observaciones"] = salida["observaciones"].apply(_deuda_vm_texto)
    salida["plan_id"] = salida["plan_id"].apply(_deuda_vm_texto)
    salida["plan_nombre"] = salida["plan_nombre"].apply(_deuda_vm_texto)
    salida["nro_cuota"] = pd.to_numeric(salida["nro_cuota"], errors="coerce").fillna(0).astype(int)
    salida["total_cuotas"] = pd.to_numeric(
        salida["total_cuotas"], errors="coerce"
    ).fillna(0).astype(int)

    # Completa organismo/tributo usando el concepto cuando la tabla anterior era simple.
    for idx, fila in salida.iterrows():
        organismo, tributo, concepto = _deuda_vm_clasificar(
            fila["organismo"] or fila["tributo"],
            fila["concepto"] or fila["tributo"],
        )
        if not fila["organismo"]:
            salida.at[idx, "organismo"] = organismo
        if not fila["tributo"]:
            salida.at[idx, "tributo"] = tributo
        if not fila["concepto"]:
            salida.at[idx, "concepto"] = concepto

    estados_originales = salida["estado"].apply(_deuda_vm_texto)
    estados = []
    for i, fila in salida.iterrows():
        estado_original = _deuda_vm_normalizar_texto(estados_originales.loc[i])
        if fila["saldo"] <= 0.01:
            estado = "Pagado"
        elif "plan" in estado_original or fila["plan_id"]:
            estado = "En plan"
        elif "judicial" in estado_original:
            estado = "Judicial"
        elif "intim" in estado_original:
            estado = "Intimado"
        elif pd.notna(fila["fecha_vencimiento"]) and fila["fecha_vencimiento"] < hoy:
            estado = "Vencido"
        else:
            estado = "Pendiente"
        estados.append(estado)
    salida["estado"] = estados

    ids = []
    for _, fila in salida.iterrows():
        existente = _deuda_vm_texto(fila["id"])
        ids.append(
            existente
            or _deuda_vm_id(
                fila["empresa"],
    ...
    salida = salida[
        (salida["periodo"].ne(""))
        | (salida["concepto"].ne(""))
        | (salida["deuda_total"].abs() > 0.001)
    ].copy()
    salida = salida.drop_duplicates(subset=["id"], keep="last")
    return salida[_DEUDA_VM_COLUMNS].reset_index(drop=True)


def _deuda_vm_detectar_segmentos(matriz: list[list[Any]]) -> list[dict[str, Any]]:
    """Detecta todos los bloques aun cuando FATSA y ATSA estén en la misma fila."""
    ...
    matriz_n = [
        [_deuda_vm_normalizar_texto(v) for v in fila] + [""] * (ancho - len(fila))
        for fila in matriz
    ]

    for r, fila in enumerate(matriz_n):
        posiciones_periodo = [
            c
            for c, valor in enumerate(fila)
            if valor in {"periodo", "periodos"} or valor.startswith("periodo ")
        ]
        for pos_idx, periodo_col in enumerate(posiciones_periodo):
            inicio_busqueda = max(0, periodo_col - 4)
            descriptor_col = None
            for c in range(periodo_col - 1, inicio_busqueda - 1, -1):
                if any(
                    token in fila[c]
                    for token in ["impuesto", "sindicato", "tributo", "concepto"]
                ):
                    descriptor_col = c
                    break

            # AUTÓNOMOS comienza directamente con "Periodos".
            inicio_col = descriptor_col if descriptor_col is not None else periodo_col
            hay_proximo_periodo = pos_idx + 1 < len(posiciones_periodo)
            proximo_periodo = (
                posiciones_periodo[pos_idx + 1]
                if hay_proximo_periodo
                else None
            )
            if hay_proximo_periodo:
                fin_col = min(periodo_col + 5, int(proximo_periodo) - 2, ancho - 1)
            else:
                fin_col = min(periodo_col + 5, ancho - 1)
            if fin_col < periodo_col:
                fin_col = min(periodo_col + 5, ancho - 1)

            headers = {c: fila[c] for c in range(inicio_col, fin_col + 1)}
            due_col = next(
                (
                    c
                    for c, valor in headers.items()
                    if "venc" in valor or "fecha de vt" in valor
                ),
                None,
            )
            capital_col = next(
                (
                    ...
                    if any(
                        token in valor
                        for token in ["capital", "monto", "importe", "cuota sindical"]
                    )
                    and "total" not in valor
                ),
                None,
            )
            intereses_col = next(
                (c for c, valor in headers.items() if "interes" in valor),
                None,
            )
            total_col = next(
                (
                    c
                    for c, valor in headers.items()
                    if valor == "total" or "deuda total" in valor
                ),
                None,
            )

            tiene_importe = any(
                c is not None for c in [capital_col, intereses_col, total_col]
            ) or any(
                any(
                    token in valor
                    for token in [
            ...
            group_col = inicio_col - 1 if inicio_col > 0 else None
            titulo = ""
            if group_col is not None:
                titulo = _deuda_vm_texto(matriz[r][group_col]) if group_col < len(matriz[r]) else ""
            if not titulo and r > 0:
                candidatos_arriba = matriz[r - 1][max(0, inicio_col - 1) : fin_col + 1]
                titulo = next(
                    (_deuda_vm_texto(v) for v in candidatos_arriba if _deuda_vm_texto(v)),
                    "",
                )
...
def _deuda_vm_parsear_legacy_matriz(
    matriz_original: list[list[Any]],
    fuente: str,
) -> pd.DataFrame:
    """Parser específico para la planilla histórica mostrada por el usuario."""
    if not matriz_original:
        return pd.DataFrame(columns=_DEUDA_VM_COLUMNS)

    ancho = max((len(fila) for fila in matriz_original), default=0)
    matriz = [fila + [""] * (ancho - len(fila)) for fila in matriz_original]
    segmentos = _deuda_vm_detectar_segmentos(matriz)
    registros: list[dict[str, Any]] = []
    hoy = pd.Timestamp.today().normalize()

    for segmento in segmentos:
        r0 = segmento["header_row"]
        inicio = segmento["inicio_col"]
        fin = segmento["fin_col"]
        periodo_col = segmento["periodo_col"]
        descriptor_col = segmento["descriptor_col"]
        group_col = segmento["group_col"]
        grupo_actual = _deuda_vm_texto(segmento["titulo"])
        descriptor_actual = ""
        vacias = 0

        for r in range(r0 + 1, min(len(matriz), r0 + 110)):
            fila = matriz[r]
            celdas_segmento = fila[inicio : fin + 1]
            normalizadas = [_deuda_vm_normalizar_texto(v) for v in celdas_segmento]

            # Nuevo encabezado o pie TOTAL finaliza el bloque horizontal.
            if any(v in {"periodo", "periodos"} for v in normalizadas):
                break
            if any(v == "total" for v in normalizadas):
                break
            if not any(_deuda_vm_texto(v) for v in celdas_segmento):
                vacias += 1
                if vacias >= 3:
                    break
                continue
            ...
            periodo = _deuda_vm_periodo(fila[periodo_col])
            if not periodo:
                continue

            if group_col is not None and group_col < len(fila):
                posible_grupo = _deuda_vm_texto(fila[group_col])
                if posible_grupo and _deuda_vm_normalizar_texto(posible_grupo) not in {
                    "total",
                    "periodo",
                }:
                    grupo_actual = posible_grupo

            if descriptor_col is not None and descriptor_col < len(fila):
                posible_descriptor = _deuda_vm_texto(fila[descriptor_col])
                if posible_descriptor and _deuda_vm_normalizar_texto(posible_descriptor) not in {
                    "impuesto",
                    "sindicato",
                    "tributo",
                    "concepto",
                }:
                    descriptor_actual = posible_descriptor

            vencimiento = (
                _deuda_vm_fecha(fila[segmento["due_col"]])
                if segmento["due_col"] is not None
                else pd.NaT
            )
            capital = (
                _deuda_vm_numero(fila[segmento["capital_col"]])
                if segmento["capital_col"] is not None
                else 0.0
            )
            intereses = (
                _deuda_vm_numero(fila[segmento["intereses_col"]])
                if segmento["intereses_col"] is not None
                else 0.0
            )
            deuda_total = (
                _deuda_vm_numero(fila[segmento["total_col"]])
                if segmento["total_col"] is not None
                else 0.0
            )

            # Para FATSA hay dos componentes y una columna TOTAL.
            componentes = []
            for c in range(periodo_col + 1, fin + 1):
                if c == segmento["due_col"] or c == segmento["total_col"]:
                    continue
                valor = _deuda_vm_numero(fila[c])
                if abs(valor) > 0.001:
                    componentes.append(valor)
            if abs(capital) <= 0.001 and componentes:
                capital = sum(componentes)
            if abs(deuda_total) <= 0.001:
                deuda_total = capital + intereses
            if abs(deuda_total) <= 0.001:
                continue
            ...
            observacion_componentes = ""
            if len(componentes) > 1:
                observacion_componentes = "Componentes históricos: " + " + ".join(
                    f"{v:.2f}" for v in componentes
                )
            ...
            registro = {
                ...
                "intereses": max(0.0, intereses),
                "deuda_total": max(0.0, deuda_total),
                "pagado": 0.0,
                "saldo": max(0.0, deuda_total),
                "estado": "Vencido"
                if pd.notna(vencimiento) and vencimiento < hoy
                else "Pendiente",
                "fecha_pago": pd.NaT,
                "plan_id": "",
                "plan_nombre": "",
                ...
                "fecha_origen": hoy,
                "fuente": fuente,
                "observaciones": observacion_componentes,
                "actualizado_en": hoy,
            }
...
def _deuda_vm_df_a_matriz(df: pd.DataFrame) -> list[list[Any]]:
    if df is None or (df.empty and len(df.columns) == 0):
        return []
    filas = [df.columns.tolist()]
    filas.extend(df.astype(object).where(pd.notna(df), "").values.tolist())
    ancho = max((len(f) for f in filas), default=0)
    return [f + [""] * (ancho - len(f)) for f in filas]


def _deuda_vm_importar_archivo(archivo) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Lee XLSX/XLS/CSV y encuentra automáticamente la hoja histórica o normalizada."""
    import io

    if archivo is None:
        return pd.DataFrame(columns=_DEUDA_VM_COLUMNS), pd.DataFrame()

    nombre = _deuda_vm_texto(getattr(archivo, "name", "archivo"))
    ...
    try:
        ...
        if extension == "csv":
            ...
            libro = pd.ExcelFile(archivo)
            hojas = {
                hoja: pd.read_excel(libro, sheet_name=hoja, header=None, dtype=object)
                for hoja in libro.sheet_names
            }
    except Exception as exc:
        return pd.DataFrame(columns=_DEUDA_VM_COLUMNS), pd.DataFrame(
            [{"Hoja": nombre, "Estado": "Error", "Detalle": str(exc)[:240]}]
        )

    # Cuando se descarga un Google Sheets completo, evita leer Facturación, Caja,
    # Cuenta Corriente u otras pestañas. Prioriza exclusivamente la hoja fiscal VM.
    if len(hojas) > 1:
        candidatas = {}
        for hoja, raw_hoja in hojas.items():
            nombre_hoja = _deuda_vm_normalizar_texto(hoja)
            es_deuda = "deuda" in nombre_hoja
            es_fiscal = any(token in nombre_hoja for token in ["imposit", "fiscal", "tribut"] )
            es_vmr = "vmr" in nombre_hoja or "reproduct" in nombre_hoja
            es_vm = (
                nombre_hoja.endswith(" vm")
                or "vitae medical" in nombre_hoja
                or (" vm " in f" {nombre_hoja} ")
            )
            if es_deuda and es_fiscal and es_vm and not es_vmr:
                candidatas[hoja] = raw_hoja
        if candidatas:
            hojas = candidatas

    for hoja, raw in hojas.items():
        raw = raw.astype(object).where(pd.notna(raw), "")
        matriz = raw.values.tolist()
        fuente = f"{nombre} · {hoja}"

        # Distingue una tabla profesional de la planilla histórica visual.
        # Esto evita sumar dos veces las mismas obligaciones cuando una fila de
        # encabezados histórica también contiene palabras como período y total.
        fila_profesional = None
        claves_profesionales = {
            "id", "empresa", "tipo registro", "organismo", "tributo",
            "concepto", "periodo", "fecha vencimiento", "capital",
            "intereses", "deuda total", "pagado", "saldo", "estado",
            "fecha pago", "plan id", "plan nombre",
        }
        for fila_header in range(min(20, len(raw))):
            valores_header = {
                _deuda_vm_normalizar_texto(v).replace("_", " ")
                for v in raw.iloc[fila_header].tolist()
                if _deuda_vm_texto(v)
            }
            coincidencias = len(valores_header & claves_profesionales)
            if (
                coincidencias >= 5
                and "periodo" in valores_header
                and ("saldo" in valores_header or "deuda total" in valores_header)
            ):
                fila_profesional = fila_header
                break

        normalizada = pd.DataFrame(columns=_DEUDA_VM_COLUMNS)
        legacy = pd.DataFrame(columns=_DEUDA_VM_COLUMNS)
        detalle_tipo = ""

        if fila_profesional is not None:
            encabezados = [
                _deuda_vm_texto(v) for v in raw.iloc[fila_profesional].tolist()
            ]
            candidato = raw.iloc[fila_profesional + 1 :].copy()
            candidato.columns = encabezados
            candidato = candidato.loc[
                :, [c for c in candidato.columns if _deuda_vm_texto(c)]
            ]
            normalizada = _deuda_vm_normalizar(candidato, fuente)
            combinado = normalizada
            detalle_tipo = "Tabla normalizada"
        else:
            legacy = _deuda_vm_parsear_legacy_matriz(matriz, fuente)
            if not legacy.empty:
                combinado = legacy
                detalle_tipo = "Planilla histórica"
            else:
                # Último intento para hojas tabulares antiguas con pocos campos.
                for fila_header in range(min(20, len(raw))):
                    encabezados = [
                        _deuda_vm_texto(v) for v in raw.iloc[fila_header].tolist()
                    ]
                    candidato = raw.iloc[fila_header + 1 :].copy()
                    candidato.columns = encabezados
                    candidato = candidato.loc[
                        :, [c for c in candidato.columns if _deuda_vm_texto(c)]
                    ]
                    prueba = _deuda_vm_normalizar(candidato, fuente)
                    if len(prueba) > len(normalizada):
                        normalizada = prueba
                combinado = normalizada
                detalle_tipo = "Tabla tabular antigua"
            ...
            estados.append(
                {
                    ...
                    "Estado": "Ignorada",
                    "Registros": 0,
                    "Detalle": "No se detectaron obligaciones impositivas",
                }
            )

    if not resultados:
        return pd.DataFrame(columns=_DEUDA_VM_COLUMNS), pd.DataFrame(estados)
    salida = pd.concat(resultados, ignore_index=True)
    salida = salida.drop_duplicates(subset=["id"], keep="last")
    return salida[_DEUDA_VM_COLUMNS].reset_index(drop=True), pd.DataFrame(estados)


def _deuda_vm_preparar_sheet(df: pd.DataFrame) -> pd.DataFrame:
    """Deja la tabla exactamente lista para sync_df_to_sheet."""
    data = _deuda_vm_normalizar(df, "Gestión VITAE")
    hoy = pd.Timestamp.today().normalize()
    data["actualizado_en"] = hoy
    for columna in ["fecha_vencimiento", "fecha_pago", "fecha_origen", "actualizado_en"]:
        fechas = pd.to_datetime(data[columna], errors="coerce")
        data[columna] = fechas.dt.strftime("%Y-%m-%d").replace("NaT", "").fillna("")
    for columna in ["capital", "intereses", "deuda_total", "pagado", "saldo"]:
        data[columna] = pd.to_numeric(data[columna], errors="coerce").fillna(0).round(2)
    for columna in ["nro_cuota", "total_cuotas"]:
        data[columna] = pd.to_numeric(data[columna], errors="coerce").fillna(0).astype(int)
    return data[_DEUDA_VM_COLUMNS].astype(object).where(pd.notna(data), "")


def _deuda_vm_merge(existente: pd.DataFrame, importado: pd.DataFrame) -> pd.DataFrame:
    """Actualiza importes sin perder pagos, planes, estados ni notas ya gestionados."""
    base = _deuda_vm_normalizar(existente, "Google Sheets")
    nuevo = _deuda_vm_normalizar(importado, "Importación")
    if base.empty:
        return nuevo
    if nuevo.empty:
        return base
    ...
    indices = {fila["id"]: i for i, fila in base.iterrows()}
    for _, fila_nueva in nuevo.iterrows():
        deuda_id = fila_nueva["id"]
        if deuda_id not in indices:
            base = pd.concat([base, fila_nueva.to_frame().T], ignore_index=True)
            indices[deuda_id] = len(base) - 1
            continue
        i = indices[deuda_id]
        preservado = {c: base.at[i, c] for c in gestion}
        for columna in _DEUDA_VM_COLUMNS:
            base.at[i, columna] = fila_nueva[columna]
        for columna, valor in preservado.items():
            if columna == "observaciones":
                if _deuda_vm_texto(valor):
                    base.at[i, columna] = valor
            else:
                base.at[i, columna] = valor

    return _deuda_vm_normalizar(base, "Gestión VITAE")


def _deuda_vm_enriquecer(df: pd.DataFrame) -> pd.DataFrame:
    data = _deuda_vm_normalizar(df, "Google Sheets")
    if data.empty:
        return data
    hoy = pd.Timestamp.today().normalize()
    data["_vencimiento"] = pd.to_datetime(data["fecha_vencimiento"], errors="coerce")
    data["_pago"] = pd.to_datetime(data["fecha_pago"], errors="coerce")
    data["_dias"] = (data["_vencimiento"] - hoy).dt.days
    data["_situacion"] = "Pendiente"
    data.loc[data["saldo"] <= 0.01, "_situacion"] = "Pagado"
    data.loc[
        (data["saldo"] > 0.01) & data["estado"].eq("En plan"), "_situacion"
    ] = "En plan"
    data.loc[
        (data["saldo"] > 0.01) & data["estado"].eq("Intimado"), "_situacion"
    ] = "Intimado"
    data.loc[
        (data["saldo"] > 0.01) & data["estado"].eq("Judicial"), "_situacion"
    ] = "Judicial"
    data.loc[
        (data["saldo"] > 0.01)
        & data["_vencimiento"].notna()
        & (data["_vencimiento"] < hoy)
        & ~data["estado"].isin(["En plan", "Intimado", "Judicial"]),
        "_situacion",
    ] = "Vencido"
    data.loc[
        (data["saldo"] > 0.01)
        & data["_dias"].between(0, 7, inclusive="both")
        & data["estado"].eq("Pendiente"),
        "_situacion",
    ] = "Vence en 7 días"
    data.loc[
        (data["saldo"] > 0.01)
        & data["_dias"].between(8, 30, inclusive="both")
        & data["estado"].eq("Pendiente"),
        "_situacion",
    ] = "Vence en 30 días"
    data["_anio"] = data["periodo"].str[:4]
    data["_mes_periodo"] = pd.to_datetime(data["periodo"] + "-01", errors="coerce")
    return data


def _deuda_vm_moneda(valor: Any) -> str:
    numero = _deuda_vm_numero(valor)
    return "$ " + f"{numero:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")


def _deuda_vm_auditoria(df: pd.DataFrame) -> pd.DataFrame:
    data = _deuda_vm_enriquecer(df)
    hallazgos: list[dict[str, Any]] = []
    if data.empty:
        return pd.DataFrame(columns=["Severidad", "ID", "Hallazgo", "Detalle"])

    duplicados = data[data["id"].duplicated(keep=False)]
    for _, fila in duplicados.iterrows():
        hallazgos.append(
            {
                "Severidad": "Alta",
                "ID": fila["id"],
                "Hallazgo": "ID duplicado",
                "Detalle": f"{fila['tributo']} · {fila['periodo']}",
            }
        )
            ...
            hallazgos.append(
                {
                    "Severidad": "Media",
                    "ID": fila["id"],
                    "Hallazgo": "Período faltante",
                    "Detalle": fila["concepto"],
                }
            )
        if pd.isna(fila["_vencimiento"]) and fila["saldo"] > 0.01:
            hallazgos.append(
                {
                    "Severidad": "Media",
                    "ID": fila["id"],
                    "Hallazgo": "Vencimiento faltante",
                    "Detalle": f"{fila['organismo']} · {fila['tributo']}",
                }
            )
        if fila["deuda_total"] < -0.01 or fila["capital"] < -0.01:
            hallazgos.append(
                {
                    "Severidad": "Alta",
                    "ID": fila["id"],
                    "Hallazgo": "Importe negativo",
                    "Detalle": _deuda_vm_moneda(fila["deuda_total"]),
                }
            )
        if fila["pagado"] - fila["deuda_total"] > 0.01:
            hallazgos.append(
                {
                    "Severidad": "Alta",
                    "ID": fila["id"],
                    "Hallazgo": "Pagado mayor que deuda",
                    "Detalle": f"Pagado {_deuda_vm_moneda(fila['pagado'])}",
                }
            )
        diferencia = abs(fila["deuda_total"] - (fila["capital"] + fila["intereses"]))
        if diferencia > 1.0:
            hallazgos.append(
                {
                    "Severidad": "Baja",
                    "ID": fila["id"],
                    "Hallazgo": "Capital + intereses no coincide",
                    "Detalle": f"Diferencia {_deuda_vm_moneda(diferencia)}",
                }
            )
    return pd.DataFrame(hallazgos)


def _deuda_vm_excel(df: pd.DataFrame) -> bytes:
    import io

    data = _deuda_vm_enriquecer(df)
    auditoria = _deuda_vm_auditoria(data)
    resumen = pd.DataFrame()
    vencimientos = pd.DataFrame()
    if not data.empty:
        resumen = (
            data.groupby(["organismo", "tributo"], as_index=False)
            .agg(
        ...
        vencimientos = data[data["saldo"] > 0.01][
            [
                "fecha_vencimiento",
                "organismo",
                "tributo",
        ...
        ].sort_values("fecha_vencimiento", na_position="last")
        vencimientos = vencimientos.rename(columns={"_situacion": "situacion"})

    detalle = _deuda_vm_preparar_sheet(data)
    buffer = io.BytesIO()
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        detalle.to_excel(writer, sheet_name="Deuda normalizada", index=False)
        resumen.to_excel(writer, sheet_name="Resumen", index=False)
        vencimientos.to_excel(writer, sheet_name="Vencimientos", index=False)
...
def _deuda_vm_plan(
    ...
    cuotas: int,
    primer_vencimiento: Any,
) -> pd.DataFrame:
    capital_financiado = max(0.0, float(capital_financiado))
    cuotas = max(1, int(cuotas))
    tasa = max(0.0, float(tasa_mensual_pct)) / 100.0
    fecha_inicial = _deuda_vm_fecha(primer_vencimiento)
    if pd.isna(fecha_inicial):
        fecha_inicial = pd.Timestamp.today().normalize() + pd.offsets.MonthEnd(1)

    if tasa > 0:
        valor_cuota = capital_financiado * tasa / (1 - (1 + tasa) ** (-cuotas))
    else:
        valor_cuota = capital_financiado / cuotas

    saldo = capital_financiado
    filas = []
    for numero in range(1, cuotas + 1):
        interes = saldo * tasa
        amortizacion = min(saldo, max(0.0, valor_cuota - interes))
        if numero == cuotas:
            amortizacion = saldo
            cuota_real = amortizacion + interes
        else:
            cuota_real = valor_cuota
        saldo = max(0.0, saldo - amortizacion)
        vencimiento = fecha_inicial + pd.DateOffset(months=numero - 1)
        filas.append(
            {
                "Cuota": numero,
                "Vencimiento": vencimiento,
                ...
                "Valor cuota": round(cuota_real, 2),
                "Saldo posterior": round(saldo, 2),
            }
        )
    return pd.DataFrame(filas)
...
def render_deudas_impositivas_vm_pro(
    ...
    """Centro integral de deuda impositiva de Vitae Medical."""
    import io
    ...
    st.markdown(
        ...
        .dvm-hero {
            ...
            border: 1px solid rgba(49, 99, 206, .22);
            border-radius: 18px;
            background: linear-gradient(135deg, rgba(24, 54, 105, .10), rgba(35, 145, 120, .08));
            margin-bottom: .8rem;
        }
        .dvm-hero h2 { margin: 0 0 .25rem 0; }
        .dvm-note {
            padding: .85rem 1rem;
            border-left: 4px solid #3977d5;
            border-radius: 8px;
            background: rgba(57, 119, 213, .08);
        }
        .dvm-alert {
            padding: .85rem 1rem;
            border-radius: 12px;
        ...
        <div class="dvm-hero">
          <h2>🏛️ Deudas Impositivas VM · Centro de Control</h2>
          <div>ARCA · F931 · IVA · Municipalidad · FATSA · ATSA · Autónomos · Planes de pago</div>
        </div>
        """,
        unsafe_allow_html=True,
    )

    normalizada = _deuda_vm_normalizar(df_original, "Google Sheets")
    legacy_actual = pd.DataFrame(columns=_DEUDA_VM_COLUMNS)
    # Solo intenta reconstruir bloques visuales cuando la hoja NO tiene ya la
    # estructura profesional. Así nunca ofrece una falsa reconversión.
    if normalizada.empty and df_original is not None and not df_original.empty:
        legacy_actual = _deuda_vm_parsear_legacy_matriz(
            _deuda_vm_df_a_matriz(df_original),
            "Hoja actual de Google Sheets",
        )
    if normalizada.empty and not legacy_actual.empty:
        normalizada = legacy_actual.copy()

    data = _deuda_vm_enriquecer(normalizada)

    c_accion1, c_accion2, c_accion3 = st.columns([1, 1, 2])
    with c_accion1:
        if st.button("🔄 Recargar Sheet", use_container_width=True, key="dvm_recargar"):
            st.cache_data.clear()
            st.rerun()
    with c_accion2:
        backup = _deuda_vm_excel(data)
        st.download_button(
            "💾 Backup Excel",
            data=backup,
            file_name=f"backup_deudas_impositivas_vm_{date.today().isoformat()}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True,
            key="dvm_backup_superior",
        )
    with c_accion3:
        st.caption(
            "La pestaña queda normalizada y el módulo calcula saldo, vencimientos, alertas y planes sin tocar otros módulos."
        )
    ...
    tabs = st.tabs(
        [
            ...
            "💳 Gestión y pagos",
            "➕ Cargar deuda",
            "📥 Migrar planilla histórica",
            "🧮 Planes de pago",
            "🛡️ Auditoría y exportación",
        ]
    )
    ...
    with tabs[0]:
        if data.empty:
            st.info(
                "La hoja todavía no tiene obligaciones normalizadas. Abrí “Migrar planilla histórica”, "
                "subí el Excel que usás hoy y guardalo una sola vez."
            )
        else:
            f1, f2, f3, f4 = st.columns([1.2, 1.2, 1.2, 1.6])
            with f1:
                anios = sorted([a for a in data["_anio"].dropna().unique().tolist() if a], reverse=True)
                anio = st.selectbox(
                    "Período fiscal",
                    ["Todos"] + anios,
                    key="dvm_filtro_anio",
            ...
            with f2:
                organismos = st.multiselect(
                    "Organismo",
                    sorted(data["organismo"].dropna().unique().tolist()),
                    key="dvm_filtro_organismo",
                )
            with f3:
                situaciones = st.multiselect(
                    ...
                    key="dvm_filtro_situacion",
                )
            with f4:
                buscar = st.text_input(
                    "Buscar",
                    placeholder="F931, período, concepto, organismo...",
                    key="dvm_buscar",
                ).strip().lower()

            filtrado = data.copy()
            if anio != "Todos":
                filtrado = filtrado[filtrado["_anio"].eq(anio)]
            if organismos:
                filtrado = filtrado[filtrado["organismo"].isin(organismos)]
            if situaciones:
                filtrado = filtrado[filtrado["_situacion"].isin(situaciones)]
            if buscar:
                texto = (
                    filtrado["organismo"].astype(str)
                    + " "
                    + filtrado["tributo"].astype(str)
                    + " "
                    + filtrado["concepto"].astype(str)
                    + " "
                    + filtrado["periodo"].astype(str)
                    + " "
                    + filtrado["plan_nombre"].astype(str)
                ).str.lower()
                filtrado = filtrado[texto.str.contains(buscar, regex=False, na=False)]

            if filtrado.empty:
                st.warning("No hay obligaciones para esos filtros.")
            else:
                total = filtrado["deuda_total"].sum()
                capital = filtrado["capital"].sum()
                intereses = filtrado["intereses"].sum()
                pagado = filtrado["pagado"].sum()
                saldo = filtrado["saldo"].sum()
                vencido = filtrado.loc[filtrado["_situacion"].eq("Vencido"), "saldo"].sum()
                vence_30 = filtrado.loc[
                    filtrado["_situacion"].isin(["Vence en 7 días", "Vence en 30 días"]),
                    "saldo",
                ].sum()
                regularizacion = pagado / total if total > 0 else 0.0

                k1, k2, k3, k4 = st.columns(4)
                k1.metric("Deuda total", _deuda_vm_moneda(total))
                k2.metric("Saldo pendiente", _deuda_vm_moneda(saldo))
                k3.metric("Saldo vencido", _deuda_vm_moneda(vencido))
                k4.metric("Intereses acumulados", _deuda_vm_moneda(intereses))
                k5, k6, k7, k8 = st.columns(4)
                k5.metric("Capital original", _deuda_vm_moneda(capital))
                k6.metric("Regularizado", _deuda_vm_moneda(pagado))
                k7.metric("Próximos 30 días", _deuda_vm_moneda(vence_30))
                k8.metric("Obligaciones", f"{len(filtrado):,}".replace(",", "."))
                st.progress(min(max(regularizacion, 0.0), 1.0))
                st.caption(f"Nivel de regularización: {regularizacion * 100:.1f}%")

                if vencido > 0:
                    cantidad_vencida = int(filtrado["_situacion"].eq("Vencido").sum())
                    st.markdown(
                        f'<div class="dvm-alert"><b>Prioridad inmediata:</b> '
                        f'{cantidad_vencida} obligaciones vencidas por {_deuda_vm_moneda(vencido)}.</div>',
                        unsafe_allow_html=True,
                    )

                por_organismo = (
                    filtrado.groupby("organismo", as_index=False)["saldo"]
                    .sum()
                    .sort_values("saldo", ascending=False)
                )
                composicion = pd.DataFrame(
                    {
                        "Componente": ["Capital", "Intereses"],
                        "Importe": [capital, intereses],
                    }
                )
                izq, der = st.columns(2)
                with izq:
                    st.markdown("#### Exposición por organismo")
                    fig = px.bar(
                        por_organismo.sort_values("saldo"),
                        x="saldo",
                        y="organismo",
                        orientation="h",
                        labels={"saldo": "Saldo", "organismo": "Organismo"},
                        text_auto=".2s",
                    )
                    st.plotly_chart(fig, use_container_width=True)
                with der:
                    st.markdown("#### Composición de la deuda")
                    if composicion["Importe"].sum() > 0:
                        fig = px.pie(
                            composicion,
                            names="Componente",
                            values="Importe",
                ...
                pendientes = filtrado[filtrado["saldo"] > 0.01].copy()
                pendientes["Bucket"] = pd.cut(
                    pendientes["_dias"],
                    bins=[-10**9, -91, -31, -1, 7, 30, 90, 10**9],
                    labels=[
                        "> 90 días vencida",
                        "31–90 días vencida",
                        "1–30 días vencida",
                ...
                pendientes.loc[pendientes["_vencimiento"].isna(), "Bucket"] = "Sin vencimiento"
                aging = (
                    pendientes.groupby("Bucket", as_index=False, observed=False)["saldo"]
                    .sum()
                    .rename(columns={"saldo": "Saldo"})
                )

                izq, der = st.columns([1.05, 1.4])
                with izq:
                    st.markdown("#### Antigüedad del saldo")
                ...
                with der:
                    ...
                    ].sort_values("fecha_vencimiento", na_position="last").head(20)
                    proximos = proximos.rename(
                        columns={
                            "fecha_vencimiento": "Vencimiento",
                            "organismo": "Organismo",
                            "tributo": "Tributo",
                ...
                top = por_organismo.iloc[0] if not por_organismo.empty else None
                concentracion = (top["saldo"] / saldo * 100) if top is not None and saldo > 0 else 0
                sin_fecha = int(
                    ((filtrado["saldo"] > 0.01) & filtrado["_vencimiento"].isna()).sum()
                )
                st.markdown("#### Lectura ejecutiva")
                st.markdown(
                    f"""
                    <div class="dvm-note">
                    <b>Mayor exposición:</b> {top['organismo'] if top is not None else 'Sin datos'} concentra
                    {concentracion:.1f}% del saldo. <b>Costo financiero:</b> los intereses representan
                    {(intereses / total * 100) if total else 0:.1f}% de la deuda registrada.
                    <b>Calidad de agenda:</b> {sin_fecha} obligaciones pendientes no tienen vencimiento cargado.
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
    ...
    with tabs[1]:
        if data.empty:
            st.info("No hay obligaciones cargadas.")
        else:
            pendientes = data[data["saldo"] > 0.01].copy()
            etiquetas = {
                f"{fila['organismo']} · {fila['tributo']} · {fila['periodo']} · "
                f"{_deuda_vm_moneda(fila['saldo'])} · {fila['id'][-6:]}": fila["id"]
            ...
            with p1:
                ...
                if etiquetas:
                    ...
                    deuda_id_pago = etiquetas[seleccion_pago]
                    fila_pago = data[data["id"].eq(deuda_id_pago)].iloc[0]
                    c1, c2 = st.columns(2)
                    with c1:
                        importe_pago = st.number_input(
                            "Importe a registrar",
                            min_value=0.0,
                            value=float(fila_pago["saldo"]),
                            step=1000.0,
                            key="dvm_pago_importe",
                        )
                    with c2:
                    ...
                    pago_total = st.checkbox(
                        "Cancelar saldo completo",
                        value=True,
                        key="dvm_pago_total",
                    )
                    if st.button(
                        ...
                        type="primary",
                        use_container_width=True,
                        key="dvm_guardar_pago",
                    ):
                        actualizado = data[_DEUDA_VM_COLUMNS].copy()
                        mask = actualizado["id"].eq(deuda_id_pago)
                        saldo_actual = float(actualizado.loc[mask, "saldo"].iloc[0])
                        monto = saldo_actual if pago_total else min(float(importe_pago), saldo_actual)
                        actualizado.loc[mask, "pagado"] = (
                            pd.to_numeric(actualizado.loc[mask, "pagado"], errors="coerce").fillna(0)
                            + monto
                        )
                        actualizado.loc[mask, "saldo"] = (
                            pd.to_numeric(actualizado.loc[mask, "deuda_total"], errors="coerce").fillna(0)
                            - pd.to_numeric(actualizado.loc[mask, "pagado"], errors="coerce").fillna(0)
                        ).clip(lower=0)
                        actualizado.loc[mask, "fecha_pago"] = pd.Timestamp(fecha_pago)
                        actualizado.loc[mask, "estado"] = actualizado.loc[mask, "saldo"].apply(
                            lambda v: "Pagado" if float(v) <= 0.01 else "Pendiente"
                        )
                        sync_df_to_sheet(table, _deuda_vm_preparar_sheet(actualizado))
                        st.cache_data.clear()
                        st.success("Pago guardado y saldo recalculado.")
                        st.rerun()
                else:
                    st.success("Todas las obligaciones están canceladas.")

            with p2:
                st.markdown("#### Estado legal / plan")
                todas_etiquetas = {
                    f"{fila['organismo']} · {fila['tributo']} · {fila['periodo']} · {fila['id'][-6:]}": fila["id"]
                    for _, fila in data.iterrows()
                }
                seleccion_estado = st.selectbox(
                    "Registro",
                    list(todas_etiquetas.keys()),
                    key="dvm_estado_id",
                )
                deuda_id_estado = todas_etiquetas[seleccion_estado]
                estado_nuevo = st.selectbox(
                    "Estado",
                    ["Pendiente", "En plan", "Intimado", "Judicial", "Pagado"],
                    key="dvm_estado_nuevo",
                )
                nota_estado = st.text_area(
                    "Nota de gestión",
                    placeholder="Ej.: plan presentado, intimación recibida, expediente...",
                    key="dvm_estado_nota",
                )
                if st.button(
                    "Guardar estado",
                    use_container_width=True,
                    key="dvm_guardar_estado",
                ):
                    actualizado = data[_DEUDA_VM_COLUMNS].copy()
                    mask = actualizado["id"].eq(deuda_id_estado)
                    actualizado.loc[mask, "estado"] = estado_nuevo
                    if estado_nuevo == "Pagado":
                        actualizado.loc[mask, "pagado"] = actualizado.loc[mask, "deuda_total"]
                        actualizado.loc[mask, "saldo"] = 0.0
                        actualizado.loc[mask, "fecha_pago"] = pd.Timestamp.today().normalize()
                    if nota_estado.strip():
                        anterior = _deuda_vm_texto(actualizado.loc[mask, "observaciones"].iloc[0])
                        marca = pd.Timestamp.today().strftime("%d/%m/%Y")
                        actualizado.loc[mask, "observaciones"] = (
                            f"{anterior} | {marca}: {nota_estado.strip()}".strip(" |")
                        )
                    sync_df_to_sheet(table, _deuda_vm_preparar_sheet(actualizado))
                    st.cache_data.clear()
            ...
            st.caption(
                "Podés corregir períodos, vencimientos, capital, intereses, pagos y observaciones. "
                "El ID, el saldo y el estado se recalculan al guardar."
            )
            editor_cols = [
                "id",
                "organismo",
            ...
            editor = data[editor_cols].copy()
            edited = st.data_editor(
                editor,
                use_container_width=True,
                hide_index=True,
                num_rows="dynamic",
                key="dvm_editor_maestro",
                disabled=["id", "saldo"],
                column_config={
                    ...
                    "estado": st.column_config.SelectboxColumn(
                        "Estado",
                        options=["Pendiente", "Vencido", "En plan", "Intimado", "Judicial", "Pagado"],
                    ),
                },
            )
            if st.button(
                ...
                type="primary",
                use_container_width=True,
                key="dvm_guardar_editor",
            ):
                base_meta = data.set_index("id")
                filas = []
                for _, fila in edited.iterrows():
                    deuda_id = _deuda_vm_texto(fila.get("id"))
                    meta = base_meta.loc[deuda_id].to_dict() if deuda_id in base_meta.index else {}
                    registro = {c: meta.get(c, "") for c in _DEUDA_VM_COLUMNS}
                    for c in editor_cols:
                        registro[c] = fila.get(c, "")
                    registro["empresa"] = "VM"
                    registro["tipo_registro"] = registro.get("tipo_registro") or "DEUDA"
                    registro["fuente"] = registro.get("fuente") or "Editor maestro"
                    registro["fecha_origen"] = registro.get("fecha_origen") or pd.Timestamp.today().normalize()
                    filas.append(registro)
                sync_df_to_sheet(table, _deuda_vm_preparar_sheet(pd.DataFrame(filas)))
                st.cache_data.clear()
                st.success("Tabla completa guardada correctamente.")
                st.rerun()

    with tabs[2]:
        sub_individual, sub_f931 = st.tabs(["Obligación individual", "F931 completo"])
        with sub_individual:
            with st.form("dvm_nueva_deuda", clear_on_submit=False):
                c1, c2, c3 = st.columns(3)
                with c1:
                    organismo = st.selectbox(
                        "Organismo",
                        ["ARCA", "Municipalidad de Salta", "FATSA", "ATSA", "Otro"],
                    )
                    tributo = st.text_input("Tributo", placeholder="F931, IVA, TISSH...")
                with c2:
                    concepto = st.text_input("Concepto", placeholder="Aportes obras sociales")
                    periodo = st.text_input("Período", placeholder="2026-07 o jul-26")
                with c3:
                ...
                with m2:
                    intereses = st.number_input("Intereses", min_value=0.0, step=1000.0)
                with m3:
                    pagado = st.number_input("Pagado", min_value=0.0, step=1000.0)
                ...
                if guardar:
                    periodo_n = _deuda_vm_periodo(periodo)
                    if not tributo.strip() or not periodo_n:
                        st.error("Completá un tributo y un período válido.")
                    elif capital + intereses <= 0:
                        st.error("La deuda debe tener un importe mayor que cero.")
                    else:
                            ...
                            [
                                {
                                    ...
                                    "concepto": concepto.strip() or tributo.strip(),
                                    "periodo": periodo_n,
                                    "fecha_vencimiento": pd.Timestamp(vencimiento),
                                    "capital": capital,
                                    "intereses": intereses,
                                    "deuda_total": total,
                                    "pagado": min(pagado, total),
                                    "saldo": max(0.0, total - pagado),
                                    "estado": "Pagado" if pagado >= total else estado,
                                    "fecha_pago": pd.Timestamp.today().normalize()
                                    if pagado >= total
                                    else pd.NaT,
                                    "plan_id": "",
                                    "plan_nombre": "",
                                    "nro_cuota": 0,
                                    "total_cuotas": 0,
                                    "fecha_origen": pd.Timestamp.today().normalize(),
                                    "fuente": "Carga manual",
                                    "observaciones": observaciones.strip(),
                                    "actualizado_en": pd.Timestamp.today().normalize(),
                                }
                            ]
                        )
                        actualizado = _deuda_vm_merge(data[_DEUDA_VM_COLUMNS], registro)
                        sync_df_to_sheet(table, _deuda_vm_preparar_sheet(actualizado))
                        st.cache_data.clear()
                        st.success("Obligación guardada.")
                        st.rerun()

        with sub_f931:
            st.markdown("#### Carga rápida de una DDJJ F931")
            st.caption("Crea las cuatro líneas que usás en la planilla histórica en una sola operación.")
            with st.form("dvm_f931_completo", clear_on_submit=False):
                a, b = st.columns(2)
                with a:
                    periodo_f931 = st.text_input("Período F931", placeholder="2026-07")
                with b:
                    venc_f931 = st.date_input("Fecha de vencimiento F931", value=date.today())
                conceptos_f931 = [
                    ("28 - SEGURO DE VIDA COLECTIVO", "dvm_f931_28"),
                    ("302 - APORTES OBRAS SOCIALES", "dvm_f931_302"),
                    ("312 - ASEG. RIESGO DE TRABAJO L.24557", "dvm_f931_312"),
                    ("352 - CONTRIBUCIONES OBRA SOCIAL", "dvm_f931_352"),
                ]
                importes_f931 = {}
                cols = st.columns(2)
                for i, (concepto_f931, key_f931) in enumerate(conceptos_f931):
                    with cols[i % 2]:
                        importes_f931[concepto_f931] = st.number_input(
                            concepto_f931,
                            min_value=0.0,
                            step=1000.0,
                            key=key_f931,
                        )
                intereses_f931 = st.number_input(
                    "Intereses generales a distribuir (opcional)",
                    min_value=0.0,
                    step=1000.0,
                    key="dvm_f931_intereses",
                )
                guardar_f931 = st.form_submit_button(
                    "Guardar F931 completo",
                    type="primary",
                    use_container_width=True,
                )
                if guardar_f931:
                    periodo_n = _deuda_vm_periodo(periodo_f931)
                    activos = {k: v for k, v in importes_f931.items() if v > 0}
                    if not periodo_n:
                        st.error("Ingresá un período válido.")
                    elif not activos:
                        st.error("Cargá al menos un concepto con importe.")
                    else:
                        total_capital = sum(activos.values())
                        filas = []
                        for concepto_f931, importe in activos.items():
                            interes_linea = (
                                intereses_f931 * importe / total_capital
                                if total_capital > 0
                                else 0.0
                            )
                            total_linea = importe + interes_linea
                            filas.append(
                                {
                                    "id": _deuda_vm_id(
                                        "VM",
                                    ...
                                    "organismo": "ARCA",
                                    "tributo": "F931",
                                    "concepto": concepto_f931,
                                    "periodo": periodo_n,
                                    "fecha_vencimiento": pd.Timestamp(venc_f931),
                                    "capital": importe,
                                    "intereses": interes_linea,
                                    "deuda_total": total_linea,
                                    "pagado": 0.0,
                                    "saldo": total_linea,
                                    "estado": "Pendiente",
                                    "fecha_pago": pd.NaT,
                                    ...
                                    "total_cuotas": 0,
                                    "fecha_origen": pd.Timestamp.today().normalize(),
                                    "fuente": "Carga rápida F931",
                                    "observaciones": "",
                                    "actualizado_en": pd.Timestamp.today().normalize(),
                                }
                            )
                        actualizado = _deuda_vm_merge(
                            data[_DEUDA_VM_COLUMNS], pd.DataFrame(filas)
                        )
                        sync_df_to_sheet(table, _deuda_vm_preparar_sheet(actualizado))
                        st.cache_data.clear()
                        st.success(f"F931 guardado: {len(filas)} conceptos.")
                        st.rerun()

    with tabs[3]:
        st.markdown("#### Migración automática desde tu planilla actual")
        st.markdown(
            """
            <div class="dvm-note">
            En Google Sheets abrí la planilla que usás hoy y elegí
            <b>Archivo → Descargar → Microsoft Excel (.xlsx)</b>. Después subila acá.
            El sistema reconoce automáticamente los bloques F931, IVA, Municipalidad,
            FATSA, ATSA y Autónomos, aunque tengan títulos, celdas combinadas y totales.
            </div>
            """,
            unsafe_allow_html=True,
        )

        if not legacy_actual.empty:
            st.success(
                f"La hoja conectada parece tener el formato histórico: se detectaron {len(legacy_actual)} obligaciones."
            )
            if st.button(
                "⚡ Convertir la hoja conectada al formato profesional",
                type="primary",
                use_container_width=True,
                key="dvm_convertir_actual",
            ):
                convertido = _deuda_vm_merge(normalizada, legacy_actual)
                sync_df_to_sheet(table, _deuda_vm_preparar_sheet(convertido))
                st.cache_data.clear()
                st.success("Hoja convertida correctamente.")
                st.rerun()

        archivo = st.file_uploader(
            "Subir planilla histórica",
            type=["xlsx", "xls", "csv"],
            key="dvm_importador_historico",
        )
        importado, estado_hojas = _deuda_vm_importar_archivo(archivo)
        if archivo is not None:
            st.markdown("##### Resultado de lectura")
            st.dataframe(estado_hojas, use_container_width=True, hide_index=True)
            if importado.empty:
                st.error(
                    "No se detectaron filas. Verificá que el archivo incluya la pestaña de deudas impositivas."
                )
            else:
                i1, i2, i3, i4 = st.columns(4)
                i1.metric("Registros detectados", len(importado))
                i2.metric("Capital", _deuda_vm_moneda(importado["capital"].sum()))
                i3.metric("Intereses", _deuda_vm_moneda(importado["intereses"].sum()))
                i4.metric("Deuda total", _deuda_vm_moneda(importado["deuda_total"].sum()))
                vista_importada = importado[
                    [
                        "organismo",
                        "tributo",
                        "concepto",
                ...
                st.dataframe(
                    ...
                    height=420,
                    column_config={
                        "fecha_vencimiento": st.column_config.DateColumn(format="DD/MM/YYYY"),
                        "capital": st.column_config.NumberColumn(format="$ %.2f"),
                        "intereses": st.column_config.NumberColumn(format="$ %.2f"),
                        "deuda_total": st.column_config.NumberColumn(format="$ %.2f"),
                    },
                )
                modo = st.radio(
                    "Cómo guardar",
                    [
                        ...
                        "Reemplazar la hoja actual por esta migración",
                    ],
                    key="dvm_modo_importacion",
                )
                confirmar = st.checkbox(
                    "Confirmo que revisé la vista previa",
                    key="dvm_confirmar_importacion",
                )
                if st.button(
                    ...
                    key="dvm_guardar_importacion",
                ):
                    destino = (
                        _deuda_vm_merge(data[_DEUDA_VM_COLUMNS], importado)
                        if modo.startswith("Fusionar")
                        else importado
                    )
                    sync_df_to_sheet(table, _deuda_vm_preparar_sheet(destino))
                    st.cache_data.clear()
                    st.success(
                        f"Migración terminada. La hoja quedó con {len(destino)} registros profesionales."
                    )
                    st.rerun()

    with tabs[4]:
        pendientes = data[data["saldo"] > 0.01].copy() if not data.empty else data
        opciones_plan = {
            f"{fila['organismo']} · {fila['tributo']} · {fila['periodo']} · {_deuda_vm_moneda(fila['saldo'])}": fila["id"]
            for _, fila in pendientes.iterrows()
        }
        seleccion_plan = st.multiselect(
            "Obligaciones que se incluirán en el plan",
            list(opciones_plan.keys()),
            key="dvm_plan_obligaciones",
        )
        ids_plan = [opciones_plan[x] for x in seleccion_plan]
        deuda_seleccionada = (
            pendientes.loc[pendientes["id"].isin(ids_plan), "saldo"].sum()
            if ids_plan
            else 0.0
        )
        ...
        p1, p2, p3, p4 = st.columns(4)
        with p1:
            deuda_base = st.number_input(
                "Deuda a refinanciar",
                min_value=0.0,
                value=float(deuda_seleccionada),
                step=10000.0,
                key="dvm_plan_deuda",
            )
        with p2:
        ...
        q1, q2 = st.columns(2)
        with q1:
            primer_venc = st.date_input(
                "Primer vencimiento",
                value=(date.today() + timedelta(days=30)),
                key="dvm_plan_primer_venc",
            )
        with q2:
            nombre_plan = st.text_input(
                "Nombre del plan",
                value=f"Plan VM {date.today().strftime('%m-%Y')}",
                key="dvm_plan_nombre",
            )

        financiado = max(0.0, float(deuda_base) - float(anticipo))
        plan = _deuda_vm_plan(financiado, tasa, int(cuotas), primer_venc)
        total_plan = plan["Valor cuota"].sum() if not plan.empty else 0.0
        interes_plan = plan["Interés"].sum() if not plan.empty else 0.0
        cuota_promedio = plan["Valor cuota"].mean() if not plan.empty else 0.0
        r1, r2, r3, r4 = st.columns(4)
        r1.metric("Capital financiado", _deuda_vm_moneda(financiado))
        r2.metric("Cuota estimada", _deuda_vm_moneda(cuota_promedio))
        r3.metric("Interés del plan", _deuda_vm_moneda(interes_plan))
        r4.metric("Costo total + anticipo", _deuda_vm_moneda(total_plan + anticipo))
        st.dataframe(
            plan,
            use_container_width=True,
            hide_index=True,
            column_config={
                "Vencimiento": st.column_config.DateColumn(format="DD/MM/YYYY"),
                "Amortización": st.column_config.NumberColumn(format="$ %.2f"),
                "Interés": st.column_config.NumberColumn(format="$ %.2f"),
                "Valor cuota": st.column_config.NumberColumn(format="$ %.2f"),
                "Saldo posterior": st.column_config.NumberColumn(format="$ %.2f"),
            },
        )
        st.download_button(
            "⬇️ Descargar simulación CSV",
            data=plan.to_csv(index=False).encode("utf-8-sig"),
            file_name=f"{nombre_plan.strip().replace(' ', '_') or 'plan_vm'}.csv",
            mime="text/csv",
            use_container_width=True,
            key="dvm_descargar_plan",
        )

        st.warning(
            "Al confirmar, las obligaciones elegidas se cierran como “Regularizadas por plan” "
            "y se crean las cuotas nuevas. Así no se duplica la deuda en el total."
        )
        confirmar_plan = st.checkbox(
            "Confirmo convertir las obligaciones seleccionadas en este plan",
            key="dvm_confirmar_plan",
        )
        if st.button(
            "💾 Crear plan en Google Sheets",
            type="primary",
            use_container_width=True,
            disabled=not (confirmar_plan and ids_plan and financiado > 0),
            key="dvm_crear_plan",
        ):
            import hashlib

            actualizado = data[_DEUDA_VM_COLUMNS].copy()
            plan_id = "PLAN-" + hashlib.sha1(
                f"{nombre_plan}|{pd.Timestamp.now().isoformat()}".encode("utf-8")
            ).hexdigest()[:12].upper()
            mask = actualizado["id"].isin(ids_plan)
            actualizado.loc[mask, "pagado"] = actualizado.loc[mask, "deuda_total"]
            actualizado.loc[mask, "saldo"] = 0.0
            actualizado.loc[mask, "estado"] = "Pagado"
            actualizado.loc[mask, "fecha_pago"] = pd.Timestamp.today().normalize()
            actualizado.loc[mask, "plan_id"] = plan_id
            actualizado.loc[mask, "plan_nombre"] = nombre_plan
            actualizado.loc[mask, "observaciones"] = actualizado.loc[
                mask, "observaciones"
            ].apply(
                lambda x: (f"{_deuda_vm_texto(x)} | Regularizada por {nombre_plan}").strip(" |")
            )

            cuotas_filas = []
            for _, cuota_fila in plan.iterrows():
                numero = int(cuota_fila["Cuota"])
                venc = pd.Timestamp(cuota_fila["Vencimiento"])
                cuotas_filas.append(
                    {
                        "id": _deuda_vm_id(plan_id, numero, venc, cuota_fila["Valor cuota"]),
                        "empresa": "VM",
                        "tipo_registro": "CUOTA_PLAN",
                        "organismo": "Plan de pagos",
                        "tributo": nombre_plan,
                        "concepto": f"Cuota {numero}/{int(cuotas)}",
                        "periodo": venc.strftime("%Y-%m"),
                        "fecha_vencimiento": venc,
                        "capital": float(cuota_fila["Amortización"]),
                        "intereses": float(cuota_fila["Interés"]),
                        "deuda_total": float(cuota_fila["Valor cuota"]),
                        "pagado": 0.0,
                        "saldo": float(cuota_fila["Valor cuota"]),
                        "estado": "En plan",
                        "fecha_pago": pd.NaT,
                        "plan_id": plan_id,
                        "plan_nombre": nombre_plan,
                        "nro_cuota": numero,
                        "total_cuotas": int(cuotas),
                        "fecha_origen": pd.Timestamp.today().normalize(),
                        "fuente": "Simulador de planes VM",
                        "observaciones": f"Anticipo informado: {_deuda_vm_moneda(anticipo)}",
                        "actualizado_en": pd.Timestamp.today().normalize(),
                    }
                )
            actualizado = pd.concat(
                [actualizado, pd.DataFrame(cuotas_filas)], ignore_index=True
            )
            sync_df_to_sheet(table, _deuda_vm_preparar_sheet(actualizado))
            st.cache_data.clear()
            st.success(f"Plan creado con {len(cuotas_filas)} cuotas.")
            st.rerun()

    with tabs[5]:
        auditoria = _deuda_vm_auditoria(data)
        a1, a2, a3, a4 = st.columns(4)
        a1.metric("Registros", len(data))
        a2.metric(
            "Hallazgos altos",
            int(auditoria["Severidad"].eq("Alta").sum()) if not auditoria.empty else 0,
        )
        a3.metric(
            "Sin vencimiento",
            int(((data["saldo"] > 0.01) & data["_vencimiento"].isna()).sum())
            if not data.empty
            else 0,
        )
        a4.metric(
            "Fuentes",
            int(data["fuente"].nunique()) if not data.empty else 0,
        )
        if auditoria.empty:
            st.success("No se detectaron inconsistencias estructurales.")
        else:
            st.dataframe(auditoria, use_container_width=True, hide_index=True)

        st.markdown("#### Estructura oficial de la hoja")
        esquema = pd.DataFrame(
            {
                "Columna": _DEUDA_VM_COLUMNS,
                "Uso": [
                    "Identificador estable",
                    "Empresa",
                    "Deuda o cuota de plan",
                    "ARCA, Municipalidad, sindicato...",
                    "F931, IVA, TISSH...",
        ...
        with st.expander("Ver diccionario de datos"):
            st.dataframe(esquema, use_container_width=True, hide_index=True)

        excel = _deuda_vm_excel(data)
        e1, e2 = st.columns(2)
        with e1:
            st.download_button(
                "📗 Descargar informe Excel completo",
                data=excel,
                file_name=f"deudas_impositivas_vm_{date.today().isoformat()}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True,
                key="dvm_exportar_excel",
            )
        with e2:
            st.download_button(
                "📄 Descargar CSV normalizado",
                data=_deuda_vm_preparar_sheet(data).to_csv(index=False).encode("utf-8-sig"),
                file_name=f"deudas_impositivas_vm_{date.today().isoformat()}.csv",
                mime="text/csv",
                use_container_width=True,
                key="dvm_exportar_csv",
            )
...
def render_facturacion_pro(module_name: str, cfg: Dict[str, Any]) -> None:
    ...
    st.header(module_name)
    st.caption(cfg.get("descripcion", ""))
    # DEUDAS IMPOSITIVAS VM: módulo especializado con migración de la planilla histórica,
    # gestión de pagos, vencimientos, planes y auditoría. Se resuelve antes de las
    # pestañas genéricas para no mezclar su estructura con otros módulos.
    if table == "deudas_impositivas_vm" or (
        "deudas impositivas" in str(module_name).lower()
        and "vmr" not in str(module_name).lower()
    ):
