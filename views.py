...
def render_facturacion_industrial(
    ...
    def serie_fecha(columna, *, permitir_futuras: bool = True):
        """Convierte fechas de Google Sheets sin invertir día/mes.

        Además corrige un caso concreto de datos históricos de Facturación:
        cuando una fecha ambigua termina en el futuro (por ejemplo 01/09/2026)
        pero al invertir día/mes produce una fecha válida ya ocurrida
        (09/01/2026), se usa esa segunda interpretación.

        La corrección de fechas futuras se activa sólo en fechas que, por
        naturaleza, ya deberían haber ocurrido (procedimiento, factura y pago).
        Vencimientos sí pueden estar en el futuro y no se tocan.
        """
        if columna is None or columna not in data.columns:
        ...
        raw = original.astype("string").str.strip().replace("", pd.NA)
        fechas = pd.Series(pd.NaT, index=data.index, dtype="datetime64[ns]")

        # 1) ISO / año primero. Se toma sólo la parte de fecha para tolerar
        # timestamps como 2026-09-01 00:00:00 o 2026-09-01T00:00:00Z.
        ...
        # 2) Formato local argentino explícito: DD/MM/YYYY o DD-MM-YYYY.
        ...
        # 3) Serial Excel / Google Sheets. Se intenta antes del fallback para
        # evitar que un número sea interpretado como nanosegundos desde 1970.
        ...
        # 4) Fallback para cualquier otra representación textual.
        faltantes = fechas.isna() & raw.notna()
        if faltantes.any():
            fechas.loc[faltantes] = pd.to_datetime(
                raw.loc[faltantes], dayfirst=True, errors="coerce"
            )

        fechas = pd.to_datetime(fechas, errors="coerce").dt.normalize().copy(deep=True)

        # 5) Reparación de ambigüedad día/mes para hechos históricos.
        # Ejemplo observado: 01/09/2026 cuando aún estamos en agosto. Si al
        # invertir día/mes queda 09/01/2026, esa es la lectura coherente.
        if not permitir_futuras:
            hoy_local = pd.Timestamp.today().normalize()
            futuras = fechas.notna() & fechas.gt(hoy_local)
            if futuras.any():
                for idx in fechas.index[futuras]:
                    valor = fechas.at[idx]
                    if pd.isna(valor):
                        continue
                    dia = int(valor.day)
                    mes = int(valor.month)
                    # Sólo existe ambigüedad real si ambos componentes caben
                    # en día/mes. Nunca se fuerza una fecha imposible.
                    if dia <= 12 and mes <= 12:
                        try:
                            invertida = pd.Timestamp(
                                year=int(valor.year), month=dia, day=mes
                            ).normalize()
                        except ValueError:
                            continue
                        if invertida <= hoy_local:
                            fechas.at[idx] = invertida
