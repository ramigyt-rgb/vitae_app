...
def render_convenios_pro() -> None:
    ...
    col_fuente, col_archivo, col_actualizar = st.columns([1.35, 2.2, .9])
    with col_fuente:
    ...
    with col_archivo:
        archivo = st.file_uploader(
            ...
            type=["xlsx", "xlsm"],
            key="convenios_excel",
            help="Sirve para auditar una versión antes de llevarla a Google Sheets.",
        )
    with col_actualizar:
        st.write("")
        st.write("")
    ...
    if not matrices:
        st.error(
            "No se detectaron hojas de convenios. Verificá los nombres AVALIAN, OSDE, OSPE, "
            "OSSEG, BOREAL, IPS, PREVENCION S, ACLISASA y CIRCULO MED."
        )
