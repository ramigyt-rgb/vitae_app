...
def _css() -> None:
    st.markdown(
        """
        <style>
        .fx-hero{background:linear-gradient(135deg,#4A1830 0%,#7C2C53 55%,#B1567E 100%);padding:24px 26px;border-radius:24px;color:#fff;margin:.2rem 0 1rem 0;box-shadow:0 18px 50px rgba(74,24,48,.16);position:relative;overflow:hidden}
        .fx-hero:after{content:"";position:absolute;width:260px;height:260px;border-radius:50%;right:-75px;top:-145px;background:rgba(255,255,255,.10)}
        .fx-kicker{font-size:.78rem;letter-spacing:.14em;text-transform:uppercase;font-weight:800;opacity:.76}
        .fx-title{font-size:1.8rem;font-weight:850;letter-spacing:-.03em;margin:.18rem 0 .1rem 0}
        .fx-sub{font-size:.95rem;opacity:.88;max-width:900px;line-height:1.5}
        .fx-card{background:rgba(255,255,255,.93);border:1px solid rgba(177,86,126,.12);border-radius:18px;padding:15px 17px;min-height:110px;box-shadow:0 9px 28px rgba(74,24,48,.06)}
        ...
        .fx-chip{display:inline-block;padding:.22rem .55rem;border-radius:999px;background:#F8EAF0;color:#7C2C53;font-size:.76rem;font-weight:750;margin:.1rem .2rem .1rem 0}
        div[data-testid="stMetric"]{background:rgba(255,255,255,.93);border:1px solid rgba(177,86,126,.12);border-radius:18px;padding:13px 15px;box-shadow:0 9px 28px rgba(74,24,48,.05)}
            ...
            if st.button("Analizar", key=f"{key}_ai_btn", use_container_width=False):
                if not question.strip():
                    ...
                    try:
                        ...
                        answer = preguntar_ia(module_name, ai_df.head(1000), question)
                        st.markdown(answer)
                    except Exception as exc:
